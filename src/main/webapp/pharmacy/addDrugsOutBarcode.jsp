...
<%
	...
	PreparedStatement ps = conn.prepareStatement("delete from oc_drugsoutlist where oc_list_updatetime<?");
	ps.setTimestamp(1, new java.sql.Timestamp(new java.util.Date().getTime()-SH.getTimeDay()*SH.ci("drugsoutlistexpiryindays",30)));
	ps.execute();
	ps.close();
	ResultSet rs = null;
	...
	if(!servicestock.equalsIgnoreCase("-1") && loadonly.length()==0 && (servicestock.length()==0 || productstock==null)){
				...
				int nBatch=rs.getInt("OC_LIST_QUANTITY");
				 ...
				 if(SH.c(stock.getProduct().getPrestationcode()).length()>0){
					 ...
					 if(prestation!=null && insurance!=null){
						 dUnit=prestation.getPatientPrice(insurance, insurance.getInsuranceCategoryLetter(),encounter==null?"":encounter.getType());
						 if(SH.c(insurance.getExtraInsurarUid()).length()==0){
							 dPat=nBatch*stock.getProduct().getPrestationquantity()*dUnit;
							 dPatient+=dPat;
						 }
						 else{
							 dExtraInsurer+=nBatch*stock.getProduct().getPrestationquantity()*dUnit;
						 }
				 ...
				 drugs+= "<tr>"+
				         ...
				         "<td class='admin2'><b><span id='drugname."+stock.getUid()+"'>"+stock.getProduct().getName()+"<br><font style='"+fontstyle+"'>"+(SH.c(stock.getProduct().getRoute()).length()==0?"":"["+getTranNoLink("product.route",stock.getProduct().getRoute(),sWebLanguage)+"] ")+"["+nBatch+" x "+SH.getPriceFormat(dUnit)+" = "+SH.getPriceFormat(dPat)+" "+SH.cs("currency","EUR")+"]</font></span></b>"+stocklabel+"</td>"+
		...
		drugsCost+="<td>"+getTranNoLink("web","patient",sWebLanguage)+": <font style='color: red;font-size: 12px'><input type='hidden' name='amount' id='amount' value='"+new Double(dPatient).intValue()+"'/>"+SH.getPriceFormat(dPatient)+" "+SH.cs("currency","EUR")+"</font></td>";
