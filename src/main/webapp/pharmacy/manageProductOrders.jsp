...
<%=checkPermission(out,"pharmacy.manageproductorders","select",activeUser)%>
<%=sJSSORTTABLE%>

<%!
    //--- ORDERS TO HTML --------------------------------------------------------------------------
    private StringBuffer ordersToHtml(Vector orders, String sWebLanguage, User activeUser){
        ...
        for (int i = 0; i < orders.size(); i++){
            ...
            html.append("<tr class='list"+sClass+"' onmouseover=\"this.style.cursor='pointer';\" onmouseout=\"this.style.cursor='default';\" title='"+detailsTran+"'>")
                 .append(!activeUser.getAccessRight("pharmacy.manageproductorders.delete")?"<td>":"<td align='center'><img src='"+sCONTEXTPATH+"/_img/icons/icon_delete.png' border='0' title='"+deleteTran+"' onclick=\"doDelete('"+order.getUid()+"');\">")
    ...
    private StringBuffer undeliveredOrdersToHtml(Vector orders, String sWebLanguage,User activeUser){
        StringBuffer html = new StringBuffer();
        String sClass = "1", sProductStockUid = "", sPreviousProductStockUid = "", sImportance = "",
        ...
        for(int i=0; i<orders.size(); i++){
            ...
            //*** display order in one row ***
            html.append("<tr class='list"+sClass+"' onmouseover=\"this.style.cursor='pointer';\" onmouseout=\"this.style.cursor='default';\" title='"+detailsTran+"'>")
                 .append(!activeUser.getAccessRight("pharmacy.manageproductorders.delete")?"<td>":"<td><img src='"+sCONTEXTPATH+"/_img/icons/icon_delete.png' border='0' title='"+deleteTran+"' onclick=\"doDelete('"+order.getUid()+"');\">")
                 .append("<td onclick=\"doShowDetails('"+order.getUid()+"');\">&nbsp;"+checkString(order.getDescription())+"</td>")
                 .append("<td onclick=\"doShowDetails('"+order.getUid()+"');\">&nbsp;"+sServiceStockName+"</td>")
                 .append("<td>&nbsp;"+sOrderForm+"</td>")
                 .append("<td onclick=\"doShowDetails('"+order.getUid()+"');\">&nbsp;"+sProductName+"</td>")
...
Debug.println("a");
    ...
    if(sAction.startsWith("find")){
        ...
        if(displayDeliveredOrders) ordersHtml = ordersToHtml(orders,sWebLanguage,activeUser);
        if(displayUndeliveredOrders) ordersHtml = undeliveredOrdersToHtml(orders,sWebLanguage,activeUser);
