...
<%=checkPermission(out,"pharmacy.manageproducts","select",activeUser)%>
<%=sJSSORTTABLE%>
    ...
    private StringBuffer objectsToHtml(Vector objects, String sWebLanguage,User activeUser){
        ...
        for (int i = 0; i < objects.size(); i++){
            ...
            html.append("<tr class='list"+sClass+"' onmouseover=\"this.style.cursor='pointer';\" onmouseout=\"this.style.cursor='default';\" title='"+detailsTran+"'>")
                 .append(!activeUser.getAccessRight("pharmacy.manageproducts.delete")?"<td>":"<td><img src='"+sCONTEXTPATH+"/_img/icons/icon_delete.png' class='link' title='"+deleteTran+"' onclick=\"doDeleteProduct('"+sProductUid+"');\">")
...
<%
    ...
    String sEditProductUid       = checkString(request.getParameter("EditProductUid")),
           ...
           sEditProductNomenclature     = checkString(request.getParameter("EditProductNomenclature")),
    ...
    String msg = "", sFindProductName, sFindProductCode, sFindUnit, sFindUnitPriceMin,
           ...
           sFindSupplierUid, sSelectedProductName = "", sSelectedProductDose = "", sSelectedProductDHIS2 = "", sSelectedProductNomenclature = "", sSelectedProductCode = "", sSelectedUnit = "", sSelectedUnitPrice = "",
    ...
    if(Debug.enabled){
        ...
        Debug.println("sEditProductNomenclature      : "+sEditProductNomenclature);
    ...
    if(sAction.equals("save") && sEditProductUid.length() > 0){
        ...
        product.setNomenclature(sEditProductNomenclature);
    ...
    if(sAction.startsWith("find")){
        ...
        productsHtml = objectsToHtml(products,sWebLanguage,activeUser);
    ...
    if(sAction.startsWith("showDetails")){
        ...
        if(sAction.equals("showDetails") || sAction.equals("showDetailsAfterUpdateReject")){
            ...
            if(product!=null){
                ...
                sSelectedProductNomenclature = checkString(product.getNomenclature());
            ...
            sSelectedProductNomenclature = sEditProductNomenclature;
            ...
            %>
                <table class="list" width="100%" cellspacing="1">
                    ...
                    <tr>
                        ...
                        <td class="admin2">
                            <input type="text" class="text" name="EditProductNomenclature"  id="EditProductNomenclature" size="10" maxlength="20" value="<%=sSelectedProductNomenclature%>" onkeyup="getNomenclatureLabel();">
                            <input type='button' class='text' name='nomenclatureButton' value='<%=getTranNoLink("web","search",sWebLanguage) %>' onclick='searchNomenclature()'/>
...
<script>
  ...
  function getNomenclatureLabel(){
      document.getElementById('nomenclatureText').innerHTML = "<img src='<%=sCONTEXTPATH%>/_img/themes/<%=sUserTheme%>/ajax-loader.gif'/><br/>Loading";
	    var today = new Date();
	    var code=transactionForm.EditProductNomenclature.value;
  ...
  function searchNomenclature(){
	  <%if(SH.cs("pharmacyNomenclatureTable","nups").equalsIgnoreCase("nups")){%>
	      openPopup("/_common/search/searchNUPSPharma.jsp&PopupWidth=800&PopupHeight=500&ts=<%=getTs()%>&VarCode=EditProductNomenclature");
	  <%}%>
      transactionForm.EditProductNomenclature.focus();
  }
