...
<%=checkPermission(out,"pharmacy.viewproductstockfiches","select",activeUser)%>
<%=sJSSORTTABLE%>
                    ...
                    <tbody class="hand">
                        <%
                            ...
                            for(int i=0; i<12; i++){
								...
								java.util.Date date = ScreenHelper.getEndOfMonth(calendar.getTime());
                                // count units
                                unitsIn   = productStock.getTotalUnitsInForMonth(date);
                                unitsOut  = productStock.getTotalUnitsOutForMonth(date);
                                ...
                                <tr class="list<%=sClass%>" title="<%=detailsTran%>" onClick="showUnitsForMonth(<%=i%>,'<%=calendar.get(Calendar.YEAR)%>','<%=productStock.getServiceStockUid()%>','<%=productStock.getUid()%>');">
                                    ...
                                    <td style="text-align:right;"><%=productStock.getLevel(date)%>&nbsp;</td>
                                    <td/>
                                </tr>
                                <%
                            }
