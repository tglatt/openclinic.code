            ...
            borderWidth: 1,
	       	data: [
					<%
						...
						for(int n=1;n<25;n++){
							itemTypes+=",'be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_1_"+n+"'";
						}
						Vector data = MedwanQuery.getInstance().getItemsFordPeriod(Integer.parseInt(activePatient.personid), itemTypes, dBegin, dEnd);
						//Sort data
						SortedMap sData = new TreeMap();
						for(int n=0;n<data.size();n++){
							ItemVO itemVO = (ItemVO)data.elementAt(n);
							java.util.Date date = itemVO.getDate();
							try{
								if(itemVO.getType().startsWith("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_1_")){
									String h = itemVO.getType().replaceAll("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_1_", "");
									h=(Integer.parseInt(h)-1)+"";
									date= SH.parseDate(new SimpleDateFormat("yyyy/MM/dd").format(date)+" "+SH.padLeft(h, "0", 2)+":00","yyyy/MM/dd HH:mm");
								}
								sData.put(date,itemVO);
							}
							catch(Exception e){};
						}
						data = new Vector();
						Iterator iData = sData.keySet().iterator();
						while(iData.hasNext()){
							data.add(sData.get(iData.next()));
						}
						//End sort
						for(int n=0;n<data.size();n++){
							ItemVO itemVO = (ItemVO)data.elementAt(n);
							...
							try{
								...
								if(value>=35 && value<=43){
									if(itemVO.getType().startsWith("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_1_")){
										String h = itemVO.getType().replaceAll("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_1_", "");
										h=(Integer.parseInt(h)-1)+"";
										out.println("{x: new Date("+new SimpleDateFormat("yyyy").format(date)+","+(Integer.parseInt(new SimpleDateFormat("MM").format(date))-1)+","+new SimpleDateFormat("dd").format(date)+","+h+",0), y:"+value+"},");
									}
									else{
            ...
            borderWidth: 1,
	       	data: [
					<%
						...
						for(int n=1;n<25;n++){
							itemTypes+=",'be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_2_"+n+"'";
						}
						data = MedwanQuery.getInstance().getItemsFordPeriod(Integer.parseInt(activePatient.personid), itemTypes, dBegin, dEnd);
						//Sort data
						sData = new TreeMap();
						for(int n=0;n<data.size();n++){
							...
							java.util.Date date = itemVO.getDate();
							try{
								if(itemVO.getType().startsWith("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_2_")){
									String h = itemVO.getType().replaceAll("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_2_", "");
									h=(Integer.parseInt(h)-1)+"";
									date= SH.parseDate(new SimpleDateFormat("yyyy/MM/dd").format(date)+" "+SH.padLeft(h, "0", 2)+":00","yyyy/MM/dd HH:mm");
								}
								sData.put(date,itemVO);
            ...
            borderWidth: 1,
	       	data: [
					<%
						...
						for(int n=1;n<25;n++){
							itemTypes+=",'be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_3_"+n+"'";
						}
						data = MedwanQuery.getInstance().getItemsFordPeriod(Integer.parseInt(activePatient.personid), itemTypes, dBegin, dEnd);
						//Sort data
						sData = new TreeMap();
						for(int n=0;n<data.size();n++){
							...
							java.util.Date date = itemVO.getDate();
							try{
								if(itemVO.getType().startsWith("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_3_")){
									String h = itemVO.getType().replaceAll("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_NEONATOLOGY_SURV_3_", "");
									h=(Integer.parseInt(h)-1)+"";
									date= SH.parseDate(new SimpleDateFormat("yyyy/MM/dd").format(date)+" "+SH.padLeft(h, "0", 2)+":00","yyyy/MM/dd HH:mm");
								}
								sData.put(date,itemVO);
