...
<%
    ...
    //check existing same maintenance plan frequency
    if(plan.countPlanWithSameFrequency(
    		plan.getAssetUID() ,
    		plan.getFrequency(),
    		Integer.parseInt(plan.getUid().split("\\.")[1])
    		)){
    	 sMessage = "<font color='red'>"+getTranNoLink("web","error.samefrequencyplan.exist",sWebLanguage)+"</font>";
    	 sStatus  = "error.samefrequencyplan.exist";
    }else{
...

{
  ...
  "newUID":"<%=plan.getUid()%>",
  "status":"<%=HTMLEntities.htmlentities(sStatus)%>"
}