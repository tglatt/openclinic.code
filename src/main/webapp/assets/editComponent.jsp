...
<%
	...
	while(rs.next()){
		...
		type=checkString(rs.getString("oc_component_type2"));
	...
	if(request.getParameter("submit")!=null){
		...
		ps.execute();
		ps.close();
		ps = conn.prepareStatement("insert into oc_assetcomponents(oc_component_assetuid,oc_component_nomenclature,oc_component_type2,oc_component_status,oc_component_characteristics,oc_component_objectid) "+
				" values(?,?,?,?,?,?)");
		ps.setString(1, assetuid);
		ps.setString(2, nomenclature);
		ps.setString(3, type);
	...
	<table width='100%'>
		...
		<tr>
			...
			<td class='admin2'>
				...
				<!-- <input class='text' type='text' size='80' name='type' id='type' value='<%= type%>'/> -->
			 
				<select class='text' name='type' id='type'>
					<option/>
					<%=ScreenHelper.writeSelect(request,"component.type",type ,sWebLanguage) %>
				</select>
