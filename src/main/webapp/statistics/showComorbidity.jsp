<%@page import="java.util.Vector,
                be.openclinic.statistics.DiagnosisStats,
                java.util.Date,
                java.text.SimpleDateFormat,
                be.openclinic.statistics.Comorbidity,
...
<%
    ...
    try{
        ...
        Date dBegin = ScreenHelper.parseDate(request.getParameter("Start")),
             dEnd   = ScreenHelper.parseDate(request.getParameter("End"));
...
<%
        ...
        for(int n=0; n<comorbidities.size(); n++){
            ...
            out.print("<tr>"+
                       "<td class='admin2' nowrap>"+(n==0?"<b>":"")+comorbidity.cases+" ("+deci.format(comorbidity.cases * 100 / totalcases)+"%)"+(n==0?"</b>":"")+"</td>"+
                       "<td class='admin2'><a href='javascript:listcases(\""+comorbidity.diagnosisCode+"\");'>"+(n==0?"<b>":"")+comorbidity.diagnosisCode+" "+codeLabel+(n==0?"</b>":"")+"</a></td>"+
                      "</tr>");
        }
    }
    catch(Exception e){
