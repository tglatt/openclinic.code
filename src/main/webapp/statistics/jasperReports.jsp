					...
					while(iFields.hasNext()){
						...
						if(field.getName().equalsIgnoreCase("field")){
							...
							if(field.elementText("type").equalsIgnoreCase("text")){
			                    ...
			                    out.println("<td class='admin2'><input type='hidden'  id='fieldname_"+field.elementText("name")+"'  name='fieldname_"+field.elementText("name")+"' value='"+sServiceUid+"'>");
			                    out.println("<input class='text' type='text' name='servicename_"+field.elementText("name")+"' id='servicename_"+field.elementText("name")+"' size='40' value='"+getTranNoLink("service",sServiceUid,sWebLanguage)+"'>&nbsp;");
...
<script>
	...
	new Ajax.Autocompleter('servicename_Structure','autocomplete_service','assets/findService.jsp',{
		  minChars:1,
		  method:'post',
		...
		function afterAutoCompleteService(field,item){
		  ...
		  document.getElementById("fieldname_Structure").value = id;
		  document.getElementById("servicename_Structure").value=item.innerHTML.split("$")[1];
		}
		
		function composeCallbackURLService(field,item){
		  document.getElementById("fieldname_Structure").value
		  var url = "";
		  if(field.id=="servicename_Structure"){
