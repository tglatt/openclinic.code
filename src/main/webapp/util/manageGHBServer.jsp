...
<%
	if(checkString(request.getParameter("action")).equalsIgnoreCase("submit")){
		...
		if(baseurl.length()>0){
			...
			MedwanQuery.getInstance().setConfigString("ghb_ref_ackurl",baseurl+"/util/setGHBACK.jsp");
			MedwanQuery.getInstance().setConfigString("ghb_ref_getackurl",baseurl+"/util/getGHBACK.jsp");
