...
<table width='100%'>
	...
	if(sortedPatients.size()>0){
		%>
		<tr>
			<td colspan='5'><input type='button' class='button' name='newmpibutton' onclick='creatempi()' value='<%=getTranNoLink("web","createnewmpiid",sWebLanguage)%>'/><span id='creatempi'/></td>
		</tr>
		<%
		...
		<tr>
			<td><input type='button' class='button' name='newmpibutton' onclick='creatempi()' value='<%=getTranNoLink("web","createnewmpiid",sWebLanguage)%>'/><span id='creatempi'/></td>
		</tr>
		<%
	}
	%>
