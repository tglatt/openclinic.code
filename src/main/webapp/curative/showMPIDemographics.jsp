...
<%
		...
		if(checkString(person.updateStatus).length()>0){
			%>
			<form name='mpiTransactionForm' method='post' action='main.jsp?Page=curative/showMPI.jsp'>
				<table width='100%'>
					<tr>
						<td colspan='3' id='divmpi' width='100%'>
							<font style='font-size: 12px;color: red'><%=getTran(request,"web","mpiservererror",sWebLanguage) %></font><br/><br/><%=person.updateStatus %><br/>
							<input class='button' onclick='doReload();' type='button' name='retrybutton' id='retrybutton' value='<%=getTranNoLink("web","retry",sWebLanguage)%>'/>
						</td>
					</tr>
					<tr>
						<td class='admin' nowrap><%=SH.cs("mpiServerName","KHIN") %></td>
