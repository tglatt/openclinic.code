...
<form name='transactionForm' method='post' action='<%=sCONTEXTPATH%>/<%=request.getRequestURI().contains("main.jsp")?"main":"popup" %>.jsp?Page=dhis2/analyzeDHIS2.jsp'>
	...
	<table width='100%'>
		...
		if(request.getParameter("getOrganisationunitsButton")!=null || SH.c(request.getParameter("showchildren")).equalsIgnoreCase("1") || SH.c(request.getParameter("showancestor")).equalsIgnoreCase("1")){
			...
			out.println("</tr>");
			try{
				String sUrl="/organisationUnits.json?paging=false&fields=id,name,ancestors&filter=name:ilike:"+organisationunitname+"&filter=id:ilike:"+organisationunitname+"&rootJunction=OR";
				if(SH.c(request.getParameter("showchildren")).equalsIgnoreCase("1")){
					sUrl="/organisationUnits/"+organisationunitname+".json?paging=false&includeChildren=true&fields=id,name,ancestors";
				}
				Document document = server.getXml(sUrl);
				...
				if(orgunits!=null){
					...
					while(iSorts.hasNext()){
						...
						if(!request.getParameter("showchildren").equalsIgnoreCase("1") && organisationunitname.length()>0 && !new String(orgunit.attributeValue("id").getBytes()).equals(organisationunitname) && !new String(orgunit.attributeValue("name").getBytes()).toLowerCase().contains(organisationunitname.toLowerCase())){
							continue;
						}
						else if(request.getParameter("showchildren").equalsIgnoreCase("1") && new String(orgunit.attributeValue("id").getBytes()).equals(organisationunitname)){
							continue;
						}
						bEmpty=false;
						String ancestor = "";
						Element ancestors = orgunit.element("ancestors");
						if(ancestors!=null){
							Iterator<Element> iAncestors = ancestors.elementIterator("organisationUnit");
							while(iAncestors.hasNext()){
								ancestor = iAncestors.next().attributeValue("id");
							}
						}
						out.println("<tr>");
						out.println("<td class='admin'><b>"+new String(orgunit.attributeValue("name").getBytes())+"</b></td>");
						out.println("<td class='admin2' width='1%' id='"+orgunit.attributeValue("id")+"'>"+orgunit.attributeValue("id")+"&nbsp;</td><td class='admin2'><input class='button' type='button' value='"+getTranNoLink("web","select",sWebLanguage)+"' onclick='selectOrgUnit(\""+orgunit.attributeValue("id")+"\")'/><input class='button' type='button' value='"+getTranNoLink("web","descendants",sWebLanguage)+"' onclick='getDescendants(\""+orgunit.attributeValue("id")+"\")'/>"+(ancestor.length()>0?"<input class='button' type='button' value='"+getTranNoLink("web","ancestor",sWebLanguage)+"' onclick='getAncestor(\""+ancestor+"\")'/>":"")+"</td>");
						out.println("</tr>");
					}
				}
				if(bEmpty && request.getParameter("showchildren").equalsIgnoreCase("1")){
					sUrl="/organisationUnits.json?paging=false&fields=id,name,ancestors&filter=name:ilike:"+organisationunitname+"&filter=id:ilike:"+organisationunitname+"&rootJunction=OR";
					document = server.getXml(sUrl);
					root = document.getRootElement();
					orgunits = root.element("organisationUnits");
					if(orgunits!=null){
						Iterator<Element> iOrgunits = orgunits.elementIterator("organisationUnit");
						while(iOrgunits.hasNext()){
							Element orgunit = iOrgunits.next();
						...
						while(iSorts.hasNext()){
							...
							out.println("<tr>");
							out.println("<td class='admin'><b>"+new String(orgunit.attributeValue("name").getBytes())+"</b></td>");
							out.println("<td class='admin2' width='1%' id='"+orgunit.attributeValue("id")+"'>"+orgunit.attributeValue("id")+"&nbsp;</td><td class='admin2'><input class='button' type='button' value='"+getTranNoLink("web","select",sWebLanguage)+"' onclick='selectOrgUnit(\""+orgunit.attributeValue("id")+"\")'/>"+(ancestor.length()>0?"<input class='button' type='button' value='"+getTranNoLink("web","ancestor",sWebLanguage)+"' onclick='getAncestor(\""+ancestor+"\")'/>":"")+"</td>");
							out.println("</tr>");
						}
					}
				}
				...
				out.println("</tr>");
				try{
					Document document = server.getXml("/dataSets.json?paging=false&fields=id,name");
					Element root = document.getRootElement();
