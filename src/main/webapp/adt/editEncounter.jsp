	    ...
	    <table class='list' border='0' width='100%' cellspacing='1'>
	       ...
	       <tr id="Bed" style="visibility:visible;">
	        ...
	        <tr>
	            ...
	            <td class="admin2">
	                <select class="text" name="EditEncounterOutcome" id="EditEncounterOutcome" style="vertical-align:middle;" onchange="checkDeadLink()">
			        ...
			        %>
			        &nbsp;<a  id="deadlink" style="display: none" href="<c:url value='/healthrecord/createTransaction.do'/>?be.mxs.healthrecord.createTransaction.transactionType=be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_MSPLS_MEDICAL_CERTIFICATE_OFCAUSEOFDEATH&be.mxs.healthrecord.createTransaction.context=<%=activeUser.activeService.defaultContext%>&ts=<%=getTs()%>"><b><%=getTran(request,"web","mccod",sWebLanguage)%></b></a>
	            </td>
	        </tr>
	...
	<script>  
	  ...
	  setcategoryfields();
		...
		function checkDeadLink(){
			<%if(SH.ci("enableICD11",0)==1){%>
				const deadLink=document.getElementById("deadlink");
				if(document.getElementById("EditEncounterOutcome").value.indexOf('dead')>-1){
					deadLink.style.display="inline";
				}else{
					deadLink.style.display="none";
				}
			<%}%>
		}
