...
<body>
        ...
        <div id="container">
			...
			<%
				...
				String sSql = "delete from dc_monitorvalues where dc_monitorvalue_date<?";
				PreparedStatement ps=conn.prepareStatement(sSql);
				ps.setTimestamp(1,new java.sql.Timestamp(new java.util.Date().getTime()-MedwanQuery.getInstance().getConfigInt("gbhPublicInactivityPeriodInMonths",3)*2*month));
				ps.execute();
				ps.close();
  ...
  <script>
    ...
    function openGISMap(siteId,parameter,mapType){    
		...
		if(mapType=='clusterer'){
	    	  var url = "<c:url value='/datacenter/gis/showDefaultMap.jsp'/>?ts="+new Date().getTime();
