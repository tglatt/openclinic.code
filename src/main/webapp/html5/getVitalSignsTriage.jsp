...
<%
	if(checkString(request.getParameter("patientuid")).length()>0 && (activePatient==null || !activePatient.personid.equalsIgnoreCase(request.getParameter("patientuid")))){
		activePatient=AdminPerson.getAdminPerson(request.getParameter("patientuid"));
	}
    if(activeUser==null || activeUser.person==null || activePatient==null){
        out.println("<script>window.location.href='"+sCONTEXTPATH+"/html5/login.jsp';</script>");
        out.flush();
    }
    else{
