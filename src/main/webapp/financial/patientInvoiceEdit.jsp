                ...
                if (sPatientCreditUID.length()>0){
                    ...
                    if (patientcredit!=null){
                        ...
                        sReturn.append( "<tr class='list"+sClass+"'>"
                            ...
                            +"<td align='right'>"+SH.getTotalPriceString(patientcredit.getAmount())+"</td>"
                        +"</tr>");
                    }
                }
            }
...
<%
	...
	<script>
	    ...
	    function searchPatientInvoice(){
	        openPopup("/_common/search/searchPatientInvoice.jsp&FindInvoicePatient=<%=sPatientId%>&doFunction=doFind()&ReturnFieldInvoiceNr=FindPatientInvoiceUID&FindInvoicePatientId=<%=sPatientId%>&Action=search&header=false&PopupHeight=420&PopupWidth=600&ts=<%=getTs()%>");
	    }
	...
	<form name='EditForm' id="EditForm" method='POST'>
        ...
        <tr>
            ...
            <td class='admin2'>
                ...
                &nbsp;<%=getTran(request,"web","total",sWebLanguage) %>: <label id='invoiceValue'></label> <%=SH.cs("preferAlternateCurrency",SH.cs("currency","EUR")) %>
                &nbsp;<%=getTran(request,"web","paid",sWebLanguage) %>: <label id='invoicePaid'></label> <%=SH.cs("preferAlternateCurrency",SH.cs("currency","EUR")) %>
                &nbsp;100%: <label id='invoice100pct'></label> <%=SH.cs("preferAlternateCurrency",SH.cs("currency","EUR")) %>
            </td>
        ...
        <% 
        if(SH.ci("enableOpenIMIS",0)==1 && Insurance.getActiveInsurance(activePatient.personid, SH.cs("OpenIMISDefaultInsurerUID",""))!=null && (patientInvoice==null || !patientInvoice.hasValidUid() || patientInvoice.getDebets().size()==0)){
        %>
        ...
        if(SH.ci("enableOpenIMIS",0)==1 && patientInvoice!=null && patientInvoice.hasValidUid() && Insurance.getActiveInsurance(activePatient.personid, SH.cs("OpenIMISDefaultInsurerUID",""))!=null && patientInvoice.getDebetsForInsurer(SH.cs("OpenIMISDefaultInsurerUID","UNKNOWN")).size()>0){ %>
        <tr style='border:2px solid black'>
        	<td class='admin'><img style='vertical-align: middle' src='<%=sCONTEXTPATH%>/_img/icons/icon_openimis.png' height='16px'/> OpenIMIS</td>
        	<td class='admin2' id='openIMISMsg' colspan='3'>
        		<table width='100%'>
                ...
                if(checkString(patientInvoice.getUid()).length() > 0){
                    ...
                    %>
	                    ...
	                    <%
                            ...
                            if (!checkString(request.getParameter("nosave")).equalsIgnoreCase("1") && !isInsuranceAgent && !(checkString(patientInvoice.getStatus()).equalsIgnoreCase("canceled"))){
	  ...
	  <% 	}
	   ...
	   <script>
	    ...
	    function doBalance(){
	    	...
	    	<%
	    		if(SH.cs("preferAlternateCurrency","").length()>0){
	    			java.util.Date date = new java.util.Date();
	    			
	    			double conversionRate=be.mxs.common.util.io.ExportSAP_AR_INV.getExchangeRate(SH.cs("preferAlternateCurrency",""),patientInvoice.getDate());
	    			out.println("conversionrate="+conversionRate+";");
	    		}
	    		else{
	    			out.println("conversionrate=1;");
	    		}
	    	%>
	    	document.getElementById('invoiceValue').innerHTML='<b>'+(total/conversionrate).toFixed(<%=MedwanQuery.getInstance().getConfigInt("currencyDecimals",2)%>)+'</b>';
	    	document.getElementById('invoicePaid').innerHTML='<b>'+(paid/conversionrate).toFixed(<%=MedwanQuery.getInstance().getConfigInt("currencyDecimals",2)%>)+'</b>';
	    	document.getElementById('invoice100pct').innerHTML='<b>'+(((insurar+total)/conversionrate).toFixed(<%=MedwanQuery.getInstance().getConfigInt("currencyDecimals",2)%>))+'</b>';
			var reductions=document.getElementsByName('reduction');
			if(reductions[0] && reductions[0].type=='radio'){
				for(var n=0;n<reductions.length;n++){
					if(reductions[n].checked){
