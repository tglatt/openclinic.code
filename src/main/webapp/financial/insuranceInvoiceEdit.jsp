            				...
            				<table width='100%'>
		            			...
		            			<tr>
            						<td>
		            					...
		            					<img src="/openclinic/_img/icons/icon_error.gif" class="link" alt="Enlever" onclick="removeInvoiceToSelection();">
	             	...
	             	<%
                    	...
                    	if(MedwanQuery.getInstance().getConfigInt("enableBurundi",0)==1){
		                    %>
                        	    ...
                        	    <option value="cmckcsv2" <%=defaultmodel.equalsIgnoreCase("cmckcsv2")?"selected":""%>><%=getTranNoLink("web","cmckcsvmodel2",sWebLanguage)%></option>
...
<script>
	...
	var found_row = 0;
	  for (var i = 2; i < numb; i++) {
		...
		if(siglerow.childNodes[3].textContent==$('EditInvoiceAdd').value){
    	     if(siglerow.childNodes[0].firstElementChild.src.indexOf('/uncheck.gif') > 0){
  ...
  function removeInvoiceToSelection(){
		var content = $("divPrestations").firstElementChild;
		//var numb = content.firstElementChild.childElementCount;
		var numb = content.firstElementChild.childElementCount;
		var found_row = 0;
		  for (var i = 2; i < numb; i++) {
			...
			if(siglerow.childNodes[3].textContent==$('EditInvoiceAdd').value){
	    	     if(siglerow.childNodes[0].firstElementChild.src.indexOf('/check.gif') > 0){
				doBalance(siglerow.childNodes[0].firstElementChild,true);
	    	     }
	          }
			}else{
...
function doPrintPdf(invoiceUid) {
    if(EditForm.PrintModel.value=='ramacsv'){
		...
		location.href=url;
    } else if(EditForm.PrintModel.value=='cmckcsv2'){
		
		var url = "<c:url value='/util/csvDocs.jsp'/>?invoiceuid=" + invoiceUid + "&ts=<%=getTs()%>&docid=invoice.cmck2";
		location.href=url;
    }
