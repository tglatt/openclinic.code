            		...
            		<tr>
		            	...
		            	<%
			            	if(userWickets.size() > 0){
			                	...
			                	if(SH.ci("enableMobileMoney",0)==1 && activeUser.getAccessRight("financial.mobilepayment.select")){
...
<script>
  ...
  function doMobilePaymentMoov(){
	  var url = "financial/mobilemoney/mobileMoneyPayment.jsp&invoiceid="+document.getElementById("EditCreditInvoiceUid").value+"&operator=Moov&amount="+document.getElementById("EditCreditAmount").value+"&currency=<%=SH.cs("currency","RWF")%>&ts=<%=getTs()%>";
	  openPopup(url);
  }
