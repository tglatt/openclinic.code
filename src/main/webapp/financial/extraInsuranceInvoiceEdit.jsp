                     ...
                     <%	
                 	    ...
                 	    if(MedwanQuery.getInstance().getConfigInt("enableMFP",0)==1){
               	        ...
               	        if(MedwanQuery.getInstance().getConfigInt("enableBurundi",0)==1){
                            ...
                            <option value="TH" <%=defaultmodel.equalsIgnoreCase("TH")?"selected":""%>><%=getTranNoLink("web","Modele TH(excel)",sWebLanguage)%></option>
                            <option value="thpdf" <%=defaultmodel.equalsIgnoreCase("thpdf")?"selected":""%>><%=getTranNoLink("web","Modele TH(pdf)",sWebLanguage)%></option>
...
function doPrintPdf(invoiceUid){
    ...
    else if(EditForm.PrintModel.value=='TH'){
		var url = "<c:url value='/util/csvDocs.jsp'/>?invoiceuid=" + invoiceUid + "&ts=<%=getTs()%>&docid=invoice.TH.extra";
	    window.open(url, "InsurarInvoicePdf<%=new java.util.Date().getTime()%>", "height=600,width=900,toolbar=yes,status=no,scrollbars=yes,resizable=yes,menubar=yes");
    }
	  else{
