		...
		while(rs.next()){
			...
			<tr>
				...
				<%	if(SH.c(rs.getString("oc_momo_status")).equalsIgnoreCase("pending")){ %>
					<img id='image_<%=rs.getString("oc_momo_transactionid")%>' src='<%=sCONTEXTPATH %>/_img/icons/icon_reload.png' onclick="checkPaymentStatus('<%=rs.getString("oc_momo_transactionid")%>','<%=rs.getString("oc_momo_operator") %>')"/>
				<%	} %>
				</td>
