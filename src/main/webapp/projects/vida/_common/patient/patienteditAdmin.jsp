...
<%
    ...
    if((activePatient!=null)&&(request.getParameter("SavePatientEditForm")==null)) {
      ...
      String sStatute = "<select name='Comment4' select-one class='text'><option value=''/>"
              +ScreenHelper.writeSelect(request,"statute",activePatient.comment4,sWebLanguage)+"</select>";
      ...
      out.print(normalRow("Web","gender","Gender","Admin",sWebLanguage)+sGender+"</select></td></tr>"
          ...
          +normalRow("Web","headoffamily","Comment3Name","Admin",sWebLanguage)+"<input type='text' class='text' name='Comment3Name' id='Comment3Name' size='70' value='"+(checkString(activePatient.comment3).length()==0?"":AdminPerson.getFullName(activePatient.comment3))+" "+activePatient.comment3+"'/><input class='button' type='button' value="+getTranNoLink("Web","select",sWebLanguage)+" onclick='showSearchPatientPopup(\"Comment3\",\"Comment3Name\");'></td></tr>"
          +normalRow("Web","relationship","Relationship","Admin",sWebLanguage)+sRelationship+"</td></tr>"
          +normalRow("Web","statute","Comment4","Admin",sWebLanguage)+sStatute+"</td></tr>"
