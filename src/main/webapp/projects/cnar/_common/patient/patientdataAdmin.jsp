...
<%
    ...
    //CenterReasons
    if(checkString((String)activePatient.adminextends.get("centerreasons")).length()>0){
    	String[] reasons = ((String)activePatient.adminextends.get("centerreasons")).split(";");
    	for(int n=0;n<reasons.length;n++){
    		if(n>0){
    			sCenterreasons+=", ";
    		}
    		sCenterreasons+=getTran(request,"centerreasons",reasons[n],sWebLanguage);
    	}
    }
...
<table width='100%' cellspacing="1" class="list">
    <%=(
        ...
        +setRow(request,"Web","center",sCenterreasons,sWebLanguage)
