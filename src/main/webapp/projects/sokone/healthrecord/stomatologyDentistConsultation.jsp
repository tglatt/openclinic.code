...
%>
        ...
        <tr>
            ...
            <td style="vertical-align:top;" size="20%">
                <table cellspacing="1" cellpadding="0">
                    ...
                    <tr>
                        <td><div width="10" height="10" style="background-color: yellow;">&nbsp;&nbsp;</div></td><td>&nbsp;<%=getTran(request,"openclinic.chuk","tooth.mobile",sWebLanguage)%></td>
                    </tr>
                    <tr>
    ...
    <table class="list" cellspacing="1" cellpadding="0" width="100%">
            ...
            <td colspan="2">
                <table cellspacing="1" cellpadding="0" width="100%" id="tblTeeth">
                    ...
                    <tr>
                        ...
                        <td class="admin2" style="vertical-align:top;">
                            <textarea onkeyup="resizeTextarea(this,10);limitChars(this,255);" class="text" cols="40" rows="2" name="toothDescription"></textarea>
                        </td>
                        <td class="admin2" style="vertical-align:top;">
                            <textarea onkeyup="resizeTextarea(this,10);limitChars(this,255);" class="text" cols="40" rows="2" name="toothTreatment"></textarea>
                        </td>
                        <td class="admin2" style="vertical-align:top;">
                            <select class="text" name="toothStatus">
                                ...
                                <option value="tooth.mobile"><%=getTran(request,"openclinic.chuk","tooth.mobile",sWebLanguage)%></option>
        ...
        <%-- STATE --%>
            ...
            <tr>
			            <td class="admin"><%=getTran(request,"web", "nombresdents", sWebLanguage)%></td>
			            <td class="admin2" >
			                <%=SH.writeDefaultCountBoxes((TransactionVO)transaction, request, "msas.dentist.nombresdents", "ITEM_TYPE_DENTIST_NBRS_DENTS", sWebLanguage, true,"","|&nbsp;&nbsp;",32) %>
			            </td>
			        </tr>
          
        <tr>
         		  <td class="admin"><%=getTran(request,"Web","medecintretement",sWebLanguage)%> </td>
          		  <td class="admin2" ><%=SH.writeDefaultTextInput(session, (TransactionVO)transaction, "ITEM_TYPE_DENTIST_MEDECIN_TRAITANT", 40)%> </td>
           </tr>
            <tr>
         		<td class="admin" > <%=getTran(request,"web","etatgeneral",sWebLanguage)%></td>
        		<td class="admin2"> <%=SH.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_DENTIST_ETAT_GENERAL",36, 2)%></td>
  		  		
           </tr>
           <tr>
         		<td class="admin" ><%=getTran(request,"Web","tarteplaque",sWebLanguage)%> </td>
           		<td class='admin2'  ><%=SH.writeDefaultRadioButtons(tran, request, "yesno", "ITEM_TYPE_DENTIST_TARTE_PLAQUE", sWebLanguage, false, "", "") %></td>
         </tr>
         <tr>
         		<td class="admin" ><%=getTran(request,"Web","pregnantwomen",sWebLanguage)%> </td>
           		<td class='admin2'  ><%=SH.writeDefaultRadioButtons(tran, request, "yesno", "ITEM_TYPE_DENTIST_PREGNANTOMEN", sWebLanguage, false, "", "") %></td>
         </tr>
        
           <tr>
           		   <td class="admin"   ><%=getTran(request,"web", "daterdv",sWebLanguage)%></td>
					<td class="admin2" ><%=SH.writeDefaultDateInput(session, tran, "ITEM_TYPE_MSAS_ENFANTEXPOSEVIH_DATERDV", "", sCONTEXTPATH)%></td>		
			</tr> 
			<tr>
         		<td class="admin" > <%=getTran(request,"web","plantraitement",sWebLanguage)%></td>
        		<td class="admin2"> <%=SH.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_DENTIST_PLAN_TRAITEMENT",40, 2)%></td>
  		  		
           </tr>
        
             <tr>      
        <td class="admin" > <%=getTran(request,"openclinic.chuk","state",sWebLanguage)%></td>
        <td class="admin2"> <%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_STOMATOLOGY_DENTIST_STATE",40, 2)%></td>
  </tr>
...
function deleteRowFromArrayString(sArray,rowid){
  ...
  function draw(tooth, status){
    if(vHashtable.containsKey(tooth)){
      ...
      if(status!=""){
        ...
        else if(status=="tooth.mobile") 		jg.setColor("yellow");
  ...
  function translate(tmpLabel){
    ...
    else if(tmpLabel=="tooth.mobile"){
        return "<%=getTranNoLink("openclinic.chuk","tooth.mobile",sWebLanguage)%>";
      }
