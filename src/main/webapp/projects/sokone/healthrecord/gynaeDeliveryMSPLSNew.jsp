...
%>
    ...
    <table class="list" width="100%" cellspacing="1">
		...
		<tr>
			<td class="admin">
	               <%=getTran(request,"gynecology", "statusmother", sWebLanguage)%>
			</td>
			<td colspan="3" class='admin2'>
               	<select class="text" name="currentTransactionVO.items.<ItemVO[hashCode=<mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_DELIVERY_STATUS_MOTHER" property="itemId"/>]>.value">
               		<option/>
               		<%=ScreenHelper.writeSelect(request,"delivery.statusmother",((TransactionVO)transaction).getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_DELIVERY_STATUS_MOTHER"),sWebLanguage) %>
               	</select>
             </td>
		</tr>
		<tr>
 			...
 			<td class='admin2' colspan='4'>
 				<table width=100%'>
 					...
 					<tr>
			 			...
			 			<td>
			                 <%=SH.writeDefaultCheckBox((TransactionVO)transaction, request, "medwan.common.true", "ITEM_TYPE_DELIVERY_PRE_ECLAMPSIE", "") %>
			                 <%=getTran(request,"gynecology", "pre_eclampsie", sWebLanguage)%>
			 			</td>
			 			<td>
			                 <%=SH.writeDefaultCheckBox((TransactionVO)transaction, request, "medwan.common.true", "ITEM_TYPE_DELIVERY_PRUPTURE_UTERINE", "") %>
			                 <%=getTran(request,"gynecology", "rupture_uterine", sWebLanguage)%>
			 			</td>
			 			<td>
			                 <%=SH.writeDefaultCheckBox((TransactionVO)transaction, request, "medwan.common.true", "ITEM_TYPE_DELIVERY_TRANSFUSSION", "") %>
			                 <%=getTran(request,"gynecology", "transfussion", sWebLanguage)%>
			 			</td>	
			 			<td>
			                 <%=SH.writeDefaultCheckBox((TransactionVO)transaction, request, "medwan.common.true", "ITEM_TYPE_DELIVERY_PRE_OCYTICIQUES", "") %>
			                 <%=getTran(request,"gynecology", "ocytociques", sWebLanguage)%>
			 			</td>			 					 						 			
			 		</tr>
			 		<tr>
			 		                                          
			 			<td>
			                 <%=SH.writeDefaultCheckBox((TransactionVO)transaction, request, "medwan.common.true", "ITEM_TYPE_DELIVERY_OCYTICIQUES_PRE_PARTUM", "") %>
			                 <%=getTran(request,"gynecology", "ocytociques_pre_partum", sWebLanguage)%>
			 			</td>
			 			<td>
			                 <%=SH.writeDefaultCheckBox((TransactionVO)transaction, request, "medwan.common.true", "ITEM_TYPE_DELIVERY_ANTICONVULSIVANT", "") %>
			                 <%=getTran(request,"gynecology", "anticonvulsivant", sWebLanguage)%>
			 			</td>
			 			<td>
			                 <%=SH.writeDefaultCheckBox((TransactionVO)transaction, request, "medwan.common.true", "ITEM_TYPE_DELIVERY_ANTIBIOTIQUE", "") %>
			                 <%=getTran(request,"gynecology", "antibiotique", sWebLanguage)%>
			 			</td>	
			 			<td>
			                 <%=SH.writeDefaultCheckBox((TransactionVO)transaction, request, "medwan.common.true", "ITEM_TYPE_DELIVERY_OCYTICIQUES_POST_PARTUM", "") %>
			                 <%=getTran(request,"gynecology", "ocytociques_post_partum", sWebLanguage)%>
			 			</td>			 					 						 			
			 		</tr>
			 		...
			 		<tr>
            			...
            			<td colspan='1'>
							<%=ScreenHelper.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_DELIVERY_PLACENTACOMMENT", 30, 1)%>
			 			</td>
			 			<td>
            				<%=getTran(request,"web", "nbrproches", sWebLanguage)%>
            			</td>
            			<td colspan='1'>
            				<%=ScreenHelper.writeDefaultNumericInput(session, (TransactionVO)transaction, "ITEM_TYPE_DELIVERY_NBRS_POCHES", 5, 0, 100, sWebLanguage) %>g
			 			</td>
			 		</tr>
                    <tr>
                    	<td colspan="6">
