...
<%
    ...
    if(sAction.equals("save") && sEditOldServiceCode.length() > 0){
        ...
        if(sEditOldServiceCode.equals("-1")){
            ...
            String orgunitid = SH.p(request,"OrganisationUnitID");
            if(orgunitid.length()>0){
            	Pointer.deletePointers("dhis2.orgunitid."+sEditServiceCode);
            	Pointer.storePointer("dhis2.orgunitid."+sEditServiceCode, orgunitid);
            }        
        }
        //*** UPDATE SERVICE **************************************************
        else{
        ...
        %>
            ...
            <table width="100%" class="list" cellspacing="1">
                ...
                <% 	if(MedwanQuery.getInstance().getConfigInt("enableDHIS2",0)==1){ %>
	                ...
	                <tr>
	                    <td class="admin"> <%=getTran(request,"web","organisationuniid",sWebLanguage)%></td>
	                    <td class="admin2">
                        	<input type="text" class="text" name="OrganisationUnitID" id="OrganisationUnitID" value="<%=Pointer.getPointer("dhis2.orgunitid."+service.code)%>" size="20">
                        	<input type='button' name='orgunitidButton' value='<%=getTranNoLink("web","search",sWebLanguage) %>' onclick='exploreDHIS2()'/>
	                    </td>
	                </tr>
                <% 	}
                   	else{
	...
	var sAuthorizedUsers = "<%=authorizedUsersJS%>";
	  ...
	  function deleteRowFromArrayString(sArray,rowid){
		...
		function exploreDHIS2(){
			openPopup("dhis2/analyzeDHIS2JSON.jsp&updatefield=OrganisationUnitID&nostore=1&nodatasets=1",800,600);
		}
