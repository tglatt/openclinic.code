...
//            sBeginDate+= "&nbsp;<input type='button' name='buttonNewAPC' class='button' onclick='newAPC()' value='"+getTran(request,"Web","new",sWebLanguage)+"'>";
        ...
        out.print(sBeginDate
            ...
            +(SH.ci("gfmalariaEnabled",0)==0?
            	inputRow(request,"Web","function","PFunction","AdminPrivate",apc.businessfunction,"T",true,false,sWebLanguage)
            :"<tr><td class='admin'>"+getTran(request,"web","function",sWebLanguage)+"</td><td class='admin2'><select onchange='checkProfession()' class='text' name='PFunction' id='PFunction'><option/>"
            	+ScreenHelper.writeSelect(request,"gf.function",apc.businessfunction,sWebLanguage,false,true)
            	+"</select> <input style='display: none' class='text' type='text' name='PSanitaryDistrict' id='PSanitaryDistrict' size='48' value='"+SH.c(apc.sanitarydistrict)+"'/></td></tr>"
            	
            )
            +inputRow(request,"Web","businessorschool","PBusiness","AdminPrivate",apc.business,"T",true,false,sWebLanguage)
...
<script>
  function checkProfession(){
	  if(document.getElementById('PFunction').value=='6'){
		  document.getElementById('PSanitaryDistrict').style.display='';
	  }
	  else{
		  document.getElementById('PSanitaryDistrict').style.display='none';
		  document.getElementById('PSanitaryDistrict').value='';
	  }
  }
  function newAPC(){
