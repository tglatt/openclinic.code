...
<script>
	function changeRegion(){
	    var today = new Date();
	    var url= path + '/_common/search/searchByAjax/getSectorsByRegion.jsp?ts=' + today;
	    new Ajax.Request(url,{
	            method: "POST",
	            postBody: 'FindRegion=' + document.getElementById("PProvince").value,
	            onSuccess: function(resp){
	                var sSectors = resp.responseText;
	
	                if (sSectors.length>0){
	                    var aSectors = sSectors.split("$");
	                    $('PSector').options.length=1;
	                    $('PDistrict').value = "";
	                    $('PCity').value = "";
	                    $('PZipcode').value = "";
	                    for(var i=0; i<aSectors.length; i++){
	                    	aSectors[i] = aSectors[i].replace(/^\s+/,'');
	                    	aSectors[i] = aSectors[i].replace(/\s+$/,'');
	
	                        if ((aSectors[i].length>0)&&(aSectors[i]!=" ")){
	                            $("PSector").options[i] = new Option(aSectors[i], aSectors[i]);
	                        }
	                    }
	                }
	            },
	...
	function changeSector(){
	    ...
	    var url= path + '/_common/search/searchByAjax/getDistrictsByRegionAndSector.jsp?ts=' + today;
	    new Ajax.Request(url,{
	            method: "POST",
	            postBody: 'FindRegion=' + document.getElementById("PProvince").value+
	            		  '&FindSector=' + document.getElementById("PSector").value,
	            onSuccess: function(resp){
	...
	function changeDistrict(){
	    ...
	    var url= path + '/_common/search/searchByAjax/getCitiesByRegionAndSectorAndDistrict.jsp?ts=' + today;
	    new Ajax.Request(url,{
	            method: "POST",
	            postBody: 'FindRegion=' + document.getElementById("PProvince").value+
	            	  	  '&FindSector=' + document.getElementById("PSector").value+
	            		  '&FindDistrict=' + document.getElementById("PDistrict").value,
	            onSuccess: function(resp){
    ...
    function changeCity(){
        var today = new Date();
        var url= path + '/_common/search/searchByAjax/getZipcodeByRegionAndSectorAndDistrictAndCity.jsp?ts= + today';
        var params = 'FindDistrict=' + document.getElementById("PDistrict").value+
		  '&FindSector='+ document.getElementById("PSector").value+
		  '&FindCity='+ document.getElementById("PCity").value+
		  '&FindRegion='+ document.getElementById("PProvince").value;
        new Ajax.Request(url,{
                method: "POST",
                postBody: params,  
                onSuccess: function(resp){
...
//            sBeginDate+= "&nbsp;<input type='button' name='buttonNewAPC' class='button' onclick='newAPC()' value='"+getTran(request,"Web","new",sWebLanguage)+"'>";
        ...
        String sProvinces = "<select class='text' id='PProvince' name='PProvince' onchange='changeRegion();'><option/>";
        Vector vProvinces = Zipcode.getProvinces(MedwanQuery.getInstance().getConfigString("zipcodetable","RwandaZipcodes"));
        Collections.sort(vProvinces);
        String sTmpProvince;
        boolean bProvinceSelected = false;
        for (int i=0;i<vProvinces.size();i++){
        	sTmpProvince = (String)vProvinces.elementAt(i);

        	sProvinces += "<option value='"+sTmpProvince+"'";

            if (sTmpProvince.equalsIgnoreCase(apc.province)){
            	sProvinces+=" selected";
                bProvinceSelected = true;
            }
            sProvinces += ">"+sTmpProvince+"</option>";
        }

        if ((!bProvinceSelected)&&(checkString(apc.district).length()>0)){
        	sProvinces += "<option value='"+checkString(apc.province)+"' selected>"+checkString(apc.province)+"</option>";
        }
        sProvinces += "</select>";

        String sSectors = "<select class='text' id='PSector' name='PSector' onchange='changeSector()'><option/>";
        Vector vSectors = Zipcode.getSectors(apc.province,MedwanQuery.getInstance().getConfigString("zipcodetable","RwandaZipcodes"));
        Collections.sort(vSectors);
        String sTmpSector;
        boolean bSectorSelected = false;
        for (int i=0;i<vSectors.size();i++){
        	sTmpSector = (String)vSectors.elementAt(i);

        	sSectors += "<option value='"+sTmpSector+"'";

            if (sTmpSector.equalsIgnoreCase(apc.sector)){
            	sSectors+=" selected";
                bSectorSelected = true;
            }
            sSectors += ">"+sTmpSector+"</option>";
        }

        if ((!bSectorSelected)&&(checkString(apc.sector).length()>0)){
            sSectors += "<option value='"+checkString(apc.sector)+"' selected>"+checkString(apc.sector)+"</option>";
        }
        sSectors += "</select>";

        String sDistricts = "<select class='text' id='PDistrict' name='PDistrict' onchange='changeDistrict()'><option/>";
        Vector vDistricts = Zipcode.getDistricts(apc.province,apc.sector,MedwanQuery.getInstance().getConfigString("zipcodetable","RwandaZipcodes"));
        ...
        Vector vCities = Zipcode.getDistricts(apc.province,apc.sector,MedwanQuery.getInstance().getConfigString("zipcodetable","RwandaZipcodes"));
        ...
        out.print(sBeginDate
              ...
              +"<tr><td class='admin'>"+getTran(request,"gambia","region",sWebLanguage)+"</td><td class='admin2'>"+sProvinces+"</td></tr>"
              +"<tr><td class='admin'>"+getTran(request,"gambia","sector",sWebLanguage)+"</td><td class='admin2'>"+sSectors+"</td></tr>"
              +"<tr><td class='admin'>"+getTran(request,"gambia","district",sWebLanguage)+"</td><td class='admin2'>"+sDistricts+"</td></tr>"
              +"<tr><td class='admin'>"+getTran(request,"gambia","city",sWebLanguage)+"</td><td class='admin2'>"+sCities+"</td></tr>"
