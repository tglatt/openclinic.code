...
<form name="SearchForm" method="POST" onkeyup="if(enterEvent(event,13)){searchPrestations();}">
    ...
    <table width="100%" cellspacing="1" cellpadding="0" class="menu">
        ...
        <%-- TITLE --%>
        <tr class="admin">
            <td colspan="4"><%=getTran(request,"web", "searchprestation", sWebLanguage)%>
            </td>
        </tr>
        <tr>
            <td class="admin2" width="120" nowrap><%=getTran(request,"web", "code", sWebLanguage)%>
            </td>
        ...
        <tr>
            <td class="admin2"><%=getTran(request,"web", "description", sWebLanguage)%>
            </td>
            <td class="admin2">
        ...
        <tr>
            <td class="admin2"><%=getTran(request,"web", "type", sWebLanguage)%>
            </td>
            <td class="admin2">
                <select class="text" name="FindPrestationType" readonly>
                    <option value="con.openinsurance"><%=getTran(request,"prestation.type","con.openinsurance",sWebLanguage) %></option>
                </select>
            </td>
        </tr>
        <tr>
            <td class="admin2"><%=getTran(request,"web", "price", sWebLanguage)%>
            </td>
            <td class="admin2">
        ...
        <tr height="25">
            ...
            <td class="admin2">
                <input class="button" type="button" onClick="searchPrestations();" name="searchButton"
                       value="<%=getTran(null,"Web","search",sWebLanguage)%>">&nbsp;
                <input class="button" type="button" onClick="clearFields();" name="clearButton"
                       value="<%=getTran(null,"Web","clear",sWebLanguage)%>">
            </td>
        </tr>
        <%
            }
    ...
    <br>
    <center>
        <input type="button" class="button" name="buttonclose" value="<%=getTran(null,"Web","Close",sWebLanguage)%>"
...
<script>
    ...
    function searchPrestations() {
        ...
        ajaxChangeSearchResults('_common/search/searchByAjax/searchContributionShow.jsp', SearchForm);
    }
