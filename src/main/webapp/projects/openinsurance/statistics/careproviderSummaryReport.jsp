...
%>
<table width='100%'>
	<tr><td class='admin' nowrap><h3><%=getTran(request,"web","totalclaimedamount",sWebLanguage)%></h3></td><td class='admin2' colspan='2'><center><h3><%=new DecimalFormat(MedwanQuery.getInstance().getConfigString("priceFormatReport","#,###")).format(dTotalClaimedAmount)+" "+MedwanQuery.getInstance().getConfigString("currency","EUR")%></h3></center></td></tr>
	<tr><td class='admin' nowrap><%=getTran(request,"web","totalacceptedamount",sWebLanguage)%></td><td class='admin2' colspan='2'><b><%=new DecimalFormat(MedwanQuery.getInstance().getConfigString("priceFormatReport","#,###")).format(dTotalAcceptedAmount)+" "+MedwanQuery.getInstance().getConfigString("currency","EUR")%></b></td></tr>
	<tr><td class='admin' nowrap><%=getTran(request,"web","waitingclaims",sWebLanguage)%></td><td class='admin2' colspan='2'><b><%=new DecimalFormat(MedwanQuery.getInstance().getConfigString("priceFormatReport","#,###")).format(dTotalClaimedAmount-dTotalAcceptedAmount)+" "+MedwanQuery.getInstance().getConfigString("currency","EUR")%></b></td></tr>
	<tr><td class='admin' nowrap><%=getTran(request,"web","totalinsuredtreated",sWebLanguage)%></td><td class='admin2' colspan='2'><b><%=new DecimalFormat(MedwanQuery.getInstance().getConfigString("priceFormatReport","#,###")).format(dTotalCareProviderInsuredPeriod)%></b></td></tr>
	<tr><td class='admin' nowrap><h3><%=getTran(request,"web","averageinsuredcost",sWebLanguage)%></h3></td><td class='admin2' colspan='2'><center><h3><%=new DecimalFormat(MedwanQuery.getInstance().getConfigString("priceFormatReport","#,###")).format(dTotalClaimedAmount/dTotalCoveredPeriod)+" "+MedwanQuery.getInstance().getConfigString("currency","EUR")%></h3><center></td></tr>
	<tr><td class='admin' nowrap><%=getTran(request,"web","consumptionindex",sWebLanguage) %> (<%=getTran(request,"web","samecareprovidercategory",sWebLanguage) %>)</td><td class='admin2' width='1%' nowrap><b><%=new DecimalFormat("#0.00").format(sameCategoryConsumptionRiskindex) %> (-2 <%=getTran(request,"web","to",sWebLanguage) %> +2)</b></td><td class='admin2'><%=getBar((sameCategoryConsumptionRiskindex+2)*100/4)%></td></tr>
	<tr><td class='admin' nowrap><%=getTran(request,"web","consumptionindex",sWebLanguage) %> (<%=getTran(request,"web","allcareprovidercategories",sWebLanguage) %>)</td><td class='admin2' width='1%' nowrap><b><%=new DecimalFormat("#0.00").format(consumptionRiskindex) %> (-2 <%=getTran(request,"web","to",sWebLanguage) %> +2)</b></td><td class='admin2'><%=getBar((consumptionRiskindex+2)*100/4)%></td></tr>
</table>