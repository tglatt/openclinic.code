...
<%
    ...
    String sEditEncounterUID = checkString(request.getParameter("EditEncounterUID")),
           ...
           sEditEncounterEscortDNI = checkString(request.getParameter("EditEncounterEscortDNI")),
           ...
           sEditEncounterReferenceSheet = checkString(request.getParameter("EditEncounterReferenceSheet")),
           sEditEncounterCounterReference = checkString(request.getParameter("EditEncounterCounterReference")),
           sEditEncounterCounterReferenceSheet = checkString(request.getParameter("EditEncounterCounterReferenceSheet")),
    ...
    String sEditEncounterManager = checkString(request.getParameter("EditEncounterManager")),
           ...
           sEditEncounterCareModality = checkString(request.getParameter("EditEncounterCareModality")),
           sEditEncounterCareType = checkString(request.getParameter("EditEncounterCareType")),
    ...
    if(sAction.equals("SAVE")){
        ...
        tmpEncounter.setCareModality(sEditEncounterCareModality);
        tmpEncounter.setCareType(sEditEncounterCareType);
		...
		if(MedwanQuery.getInstance().getConfigInt("enableEncounterReference",0)==1){
			...
			tmpEncounter.setReferenceSheet(sEditEncounterReferenceSheet);
			tmpEncounter.setCounterReference(sEditEncounterCounterReference);
			tmpEncounter.setCounterReferenceSheet(sEditEncounterCounterReferenceSheet);
		}
		tmpEncounter.setEscortName(sEditEncounterEscortName);
		tmpEncounter.setEscortPhone(sEditEncounterEscortPhone);
		tmpEncounter.setEscortDNI(sEditEncounterEscortDNI);
		tmpEncounter.setComment(sEditEncounterComment);
        try{
        	tmpEncounter.setNewcase(Integer.parseInt(sEditEncounterNewcase));
        }
        catch(Exception e){}
    ...
    if(sEditEncounterUID.length() > 0){
		...
		sEditEncounterReferenceSheet = checkString(tmpEncounter.getReferenceSheet());
		sEditEncounterCounterReference = checkString(tmpEncounter.getCounterReference());
		sEditEncounterCounterReferenceSheet = checkString(tmpEncounter.getCounterReferenceSheet());
		...
		sEditEncounterEscortDNI = checkString(tmpEncounter.getEscortDNI());
        ...
        sEditEncounterCareModality = checkString(tmpEncounter.getCareModality());
        sEditEncounterCareType = checkString(tmpEncounter.getCareType());
        if(tmpEncounter.getManager()==null){
            sEditEncounterManager = "";
            sEditEncounterManagerName = "";
        }
        else{
            sEditEncounterManager = checkString(tmpEncounter.getManagerUID());
            sEditEncounterManagerName = checkString(User.getFullUserName(tmpEncounter.getManagerUID()));
        }
...
%>
    ...
    <table class='list' border='0' width='100%' cellspacing='1'>
        ...
        <tr>
            ...
            <td class="admin2">
                <select class="text" name="EditEncounterOrigin" id="EditEncounterOrigin" onchange="checkorigin()">
                    <option/>
        ...
        <tr id="Reference" style="display: none">
            ...
            <td class='admin2'>
            	<table>
            		<tr>
            			<td>
            				<%=getTran(request,"web","hospital",sWebLanguage)%>:
            				<select class='text' id='EditEncounterReference' name='EditEncounterReference'>
            					<option>
            					<%=ScreenHelper.writeSelect(request, "ipress.renaes", sEditEncounterReference, sWebLanguage, false, true, 50) %>
            				</select>
            			</td>
            			<td>
            				<%=getTran(request,"web","hoja",sWebLanguage)%>:
            				<input type='text' class='text' size='20' id='EditEncounterReferenceSheet' name='EditEncounterReferenceSheet' value='<%=sEditEncounterReferenceSheet %>'/>
            			</td>
            		</tr>
            	</table>
            </td>
       ...
       <tr id="Bed" style="visibility:visible;">
        ...
        <%-- NEWCASE --%>
        <tr>
            <td class="admin" width="<%=sTDAdminWidth%>"><%=getTran(request,"hep","patienttype",sWebLanguage)+(MedwanQuery.getInstance().getConfigInt("encounterSituationMandatory",0)==1?" *":"")%></td>
            <td class="admin2">
                <select class="text" name="EditEncounterNewcase" id="EditEncounterNewcase" style="vertical-align:top;">
                    <option/>
                    <%=ScreenHelper.writeSelectUnsorted(request,"hep.patienttype",sEditEncounterNewcase,sWebLanguage)%>
                </select>
            </td>
        </tr>
        
        <%-- SITUATION --%>
        <input type='hidden' name='EditEncounterSituation' value='1'/>
        <tr>
            <td class="admin" width="<%=sTDAdminWidth%>"><%=getTran(request,"fua","fua.caremodality",sWebLanguage)+(MedwanQuery.getInstance().getConfigInt("encounterSituationMandatory",0)==1?" *":"")%></td>
            <td class="admin2">
                <select class="text" name="EditEncounterCareModality" id="EditEncounterCareModality" style="vertical-align:top;">
                    <option/>
                    <%=ScreenHelper.writeSelectUnsorted(request,"fua.caremodality",sEditEncounterCareModality,sWebLanguage)%>
                </select>
            </td>
        </tr>
        
        <tr>
            <td class="admin" width="<%=sTDAdminWidth%>"><%=getTran(request,"fua","fua.prestationcode",sWebLanguage)+(MedwanQuery.getInstance().getConfigInt("encounterSituationMandatory",0)==1?" *":"")%></td>
            <td class="admin2">
                <select class="text" name="EditEncounterCareType" id="EditEncounterCareType" style="vertical-align:top;">
                    <option/>
                    <%=ScreenHelper.writeSelectUnsorted(request,"fua.prestationcode",sEditEncounterCareType,sWebLanguage)%>
                </select>
            </td>
        </tr>
        ...
        <%-- ACCOMPANIED BY --%>
        <tr id="Escort" style="visibility:visible;">
            <td class="admin"><%=getTran(request,"web","accompaniedby",sWebLanguage)%></td>
            <td class='admin2'>
	        <select class="text" name="EditEncounterEscortName" style="vertical-align:top;">
                <option value=""><%=getTran(request,"web","choose",sWebLanguage)%></option>
                    <%=ScreenHelper.writeSelectUnsorted(request,MedwanQuery.getInstance().getConfigString("encounterEscortName","encounter.escort"),sEditEncounterEscortName,sWebLanguage)%>
                </select>
               ...
               &nbsp;&nbsp;&nbsp;&nbsp;<%=getTran(request,"web","dni",sWebLanguage)%>:
				<input type='text' class='text' size='20' id='EditEncounterEscortDNI' name='EditEncounterEscortDNI' value='<%=sEditEncounterEscortDNI	 %>'/>
            
            </td>
        </tr>

        <%-- COUNTERREFERENCE --%>
        <tr id="CounterReference" style="visibility:visible;">
            <td class="admin"><%=getTran(request,"web","counterreference",sWebLanguage)%></td>
            <td class='admin2'>
            	<table>
            		<tr>
            			<td>
            				<%=getTran(request,"web","hospital",sWebLanguage)%>:
            				<select class='text' id='EditEncounterCounterReference' name='EditEncounterCounterReference'>
            					<option>
            					<%=ScreenHelper.writeSelect(request, "ipress.renaes", sEditEncounterCounterReference, sWebLanguage, false, true, 50) %>
            				</select>
            			</td>
            			<td>
            				<%=getTran(request,"web","hoja",sWebLanguage)%>:
            				<input type='text' class='text' size='20' id='EditEncounterCounterReferenceSheet' name='EditEncounterCounterReferenceSheet' value='<%=sEditEncounterReferenceSheet %>'/>
            			</td>
            		</tr>
            	</table>
            </td>
        ...
        <tr id="Category" style="visibility:visible;">
            ...
            <td class='admin2'>
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesA' name='EditEncounterCategories' value='A' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("A")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.disease",sWebLanguage,"EditEncounterCategoriesA")%>&nbsp;
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesB' name='EditEncounterCategories' value='B' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("B")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.accident",sWebLanguage,"EditEncounterCategoriesB")%>&nbsp;
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesC' name='EditEncounterCategories' value='C' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("C")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.accident.work",sWebLanguage,"EditEncounterCategoriesC")%>&nbsp;
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesD' name='EditEncounterCategories' value='D' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("D")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.accident.traffic",sWebLanguage,"EditEncounterCategoriesD")%>&nbsp;
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesE' name='EditEncounterCategories' value='E' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("E")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.agression",sWebLanguage,"EditEncounterCategoriesE")%>&nbsp;
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesF' name='EditEncounterCategories' value='F' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("F")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.other",sWebLanguage,"EditEncounterCategoriesF")%>&nbsp;
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesG' name='EditEncounterCategories' value='G' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("G")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.accident.casual",sWebLanguage,"EditEncounterCategoriesG")%>&nbsp;
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesH' name='EditEncounterCategories' value='H' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("H")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.medical.care",sWebLanguage,"EditEncounterCategoriesH")%>&nbsp;
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesI' name='EditEncounterCategories' value='I' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("I")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.diagnostics",sWebLanguage,"EditEncounterCategoriesI")%>&nbsp;
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesJ' name='EditEncounterCategories' value='J' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("J")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.consultation",sWebLanguage,"EditEncounterCategoriesJ")%>&nbsp;
                <input type='radio' class="hand" onClick='setcategoryfields()' id='EditEncounterCategoriesK' name='EditEncounterCategories' value='K' ondblclick='this.checked=!this.checked' <%=sEditEncounterCategories.indexOf("K")>=0?"checked":"" %>/><%=getLabel(request,"web","cat.surgery.campaign",sWebLanguage,"EditEncounterCategoriesK")%>&nbsp;
            </td>
        </tr>
...
<script>  
  ...
  function doSave(){
    ...
    else if(EditEncounterForm.EditEncounterNewcase.value==""){
        alertDialog("web","encounter_invalid_patienttype");
        EditEncounterForm.EditEncounterNewcase.focus();
      }
  ...
  function checkorigin(){
	  if(document.getElementById('EditEncounterOrigin').value=='2'){
