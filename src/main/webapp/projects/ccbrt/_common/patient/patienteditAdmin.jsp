      ...
      out.print(
          ...
          +(
  	      		MedwanQuery.getInstance().getConfigInt("showAdminComment4",1)==0?"<input type='hidden' name='Comment4' value='"+checkString(activePatient.comment4)+"'/>":
           	      	"<tr><td class='admin'>"+getTran(request,"web","comment4",sWebLanguage)+"</td><td class='admin2'><input type='text' class='text' name='Comment4' id='Comment4' value='"+checkString(activePatient.comment4)+"' size='"+sTextWidth+"' onblur='if(this.value!=\""+MedwanQuery.getInstance().getConfigString("editMaskTelephone","___ ___ ____")+"\" && this.value.indexOf(\"_\")>-1){alert(\""+getTranNoLink("web","wrongphonenumber",sWebLanguage)+"\");document.getElementById(\"Comment4\").focus();}'/></td></tr>"
           )
          +(
	      ...
	      +(
	    		...
	    		inputRow(request,"Web","admincomment","Comment","Admin",activePatient.comment,"N",true, false,sWebLanguage)
	       )
...
function checkSubmitAdmin(){
  ...
  document.getElementById('Comment4').value=document.getElementById('Comment4').value.replace(/\_/g,'');
...
{
		MaskedInput({
			elm: document.getElementById('PTelephone'),
			format: '<%=MedwanQuery.getInstance().getConfigString("editMaskTelephone","___ ___ ____")%>', separator: '()- ', typeon: '_',
			onbadkey: function(){
				FlashBang({ elm: document.getElementById('PTelephone'),
				prop: 'color', delta: 120, color: '#f00' });
			}, badkeywait: 130
		});
		MaskedInput({
			elm: document.getElementById('PMobile'),
			format: '<%=MedwanQuery.getInstance().getConfigString("editMaskTelephone","___ ___ ____")%>', separator: '()- ', typeon: '_',
			onbadkey: function(){
				FlashBang({ elm: document.getElementById('PMobile'),
				prop: 'color', delta: 120, color: '#f00' });
			}, badkeywait: 130
		});
		MaskedInput({
			elm: document.getElementById('Comment4'),
			format: '<%=MedwanQuery.getInstance().getConfigString("editMaskTelephone","___ ___ ____")%>', separator: '()- ', typeon: '_',
			onbadkey: function(){
				FlashBang({ elm: document.getElementById('Comment4'),
				prop: 'color', delta: 120, color: '#f00' });
			}, badkeywait: 130
		});
}
