            ...
            %>
                <table width="100%" cellspacing="1" class="list">
                    <%=		setRow("Web.admin","addresschangesince",apc.begin,sWebLanguage)+
        		            ...
        		            setRow(request,"Web","province",apc.province,sWebLanguage)+
        		            setRow(request,"Web","district",apc.district,sWebLanguage)+
    ...
    if(!sShowButton.equals("false")){
        %>
            <%=ScreenHelper.alignButtonsStart()%>
                <input type="button" class="button" value="<%=getTran(request,"Web","history",sWebLanguage)%>" name="ButtonHistoryPrivate" onclick="parent.location='patienthistory.do?ts=<%=getTs()%>&contacttype=private'">&nbsp;
                <%
                    if (activeUser.getAccessRight("patient.administration.edit")){
                        %>
                            <input type="button" class="button" onclick="window.location.href='<c:url value="/patientedit.do"/>?Tab=AdminPrivate&ts=<%=getTs()%>'" value="<%=getTran(request,"Web","edit",sWebLanguage)%>">
                        <%
                    }
                %>
