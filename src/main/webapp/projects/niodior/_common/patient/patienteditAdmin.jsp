...
<%
    ...
    if((activePatient!=null)&&(request.getParameter("SavePatientEditForm")==null)) {
      ...
      out.print(normalRow("Web","gender","Gender","Admin",sWebLanguage)+sGender+"</select></td></tr>"
          ...
          %>
          <tr>
          	<td class='admin'><%=getTran(request,"web","comment4",sWebLanguage) %></td>
          	<td class='admin2'>
                ...
                <input type='hidden' name='Comment4' id='Comment4' value='<%=checkString(activePatient.comment4)%>'/>
                <input type='text' size='40' class='text' name='destinationid' id='destinationid'/>
                <input type='button' name='adddestination' onclick='addDestination()' value='<%=getTranNoLink("web","add",sWebLanguage) %>'/>
                <div id='destinations'></div>
                <script>
                	function addDestination(){
                		if(document.getElementById("destinationid").value.length>0 && document.getElementById("Comment4").value.indexOf(document.getElementById("destination").value+":")<0){
                    		var refid = document.getElementById("destination").value+":"+document.getElementById("destinationid").value;
                			if(document.getElementById('Comment4').value.length>0){
                				document.getElementById('Comment4').value+=";";
                			}
                			document.getElementById('Comment4').value+=refid;
                    		document.getElementById('Comment4').value=document.getElementById('Comment4').value.replace(';;',';');
                		}	
                		getDestinations();
                		document.getElementById("destinationid").value="";
                	}
                	
                	function deleteDestination(refid){
                		document.getElementById('Comment4').value=document.getElementById('Comment4').value.replace(refid+';','');
                		document.getElementById('Comment4').value=document.getElementById('Comment4').value.replace(refid,'');
                		getDestinations();
                	}
                	...
                	function getDestinations(){
	            	    var today = new Date();
	            	    var url= '<c:url value="/util/getReceiverDestinations.jsp"/>?destinationids='+document.getElementById('Comment4').value+'&ts='+today;
	            	    new Ajax.Request(url,{
	            	        method: "POST",
	            	        postBody: "",
	            	        onSuccess: function(resp){
	            	            var label = eval('('+resp.responseText+')');
	            	  		  	$('destinations').innerHTML=label.destinations;
	            	        },
	            	        onFailure: function(){
	            	            $('destinations').innerHTML = "Error in function getDestinations() => AJAX";
	            	        }
	            	    }
	            	 	);
	            	}
          ...
          <%
          out.print((activePatient.isDead()!=null?inputRow(request,"Web","deathcertificateon","DeathCertificateOn","Admin",checkString((String)activePatient.adminextends.get("deathcertificateon")),"T",true,false,sWebLanguage)
