...
<%
    ...
    if(lResults.size() > 0){
        ...
        if((sName.length()>0 || sFirstname.length() > 0 || sDateOfBirth.length()>0 || simmatnew.length()>0) && 
	        ...
	        %>
	            <br><br>
	            <img src="<%=sCONTEXTPATH%>/_img/themes/default/pijl.gif"/>&nbsp;<a href="<c:url value='/patientnew.do'/>?PatientNew=true&pLastname=<%=sName%>&pFirstname=<%=sFirstname%>&pImmatnew=<%=simmatnew%>&pNatreg=<%=snatreg%>&pDateofbirth=<%=sDateOfBirth%>&pDistrict=<%=sDistrict%>"><%=getTran(request,"web","new_patient",sWebLanguage)%></a>
	        <%
        }
    }
%>
