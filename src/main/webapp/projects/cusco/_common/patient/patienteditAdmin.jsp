...
<%
    ...
    if((activePatient!=null)&&(request.getParameter("SavePatientEditForm")==null)) {
        ...
        out.print(
            inputRow(request,"Web","Lastnamefather","Lastname","Admin",activePatient.getLastnameFather(),"T",true, true,sWebLanguage)
            +inputRow(request,"Web","Lastnamemother","Middlename","Admin",activePatient.getLastnameMother(),"T",true, true,sWebLanguage)
        ...
        String sNatRegHtml = normalRow("Web","natreg","NatReg","Admin",sWebLanguage)+
                             ...
                             " size='20' onblur='validateText(this);' maxLength='250'>&nbsp;&nbsp;&nbsp;"+
                             getTran(request,"web","documenttype",sWebLanguage)+": "+
                             "<select class='text' name='PersonType' id='PersonType'>"+
                             ScreenHelper.writeSelect(request, "documenttype", checkString(activePatient.personType).length()==0?MedwanQuery.getInstance().getConfigString("defaultDocumentType","1"):activePatient.personType, sWebLanguage)+
                             "</select>";
      ...
      out.print(normalRow("Web","gender","Gender","Admin",sWebLanguage)+sGender+"</select>"+sBioGender+"</td></tr>"
          +inputRow(request,"Web","fathersname","TreatingPhysician","Admin",activePatient.comment1,"T",true,false,sWebLanguage)
          +inputRow(request,"Web","grandfathersname","Comment3","Admin",activePatient.comment3,"T",true,false,sWebLanguage)
          +inputRow(request,"Web","childorder","TracnetID","Admin",checkString((String)activePatient.adminextends.get("tracnetid")),"S:childorder",true,false,sWebLanguage)
	      ...
	      +inputRow(request,"Web","populationgroup","Comment4","Admin",activePatient.comment4,"S:sis.populationgroup",true, false,sWebLanguage)
	      +inputRow(request,"Web","ethnicgroup","Comment5","Admin",activePatient.comment5,"S:sis.etnia",true, false,sWebLanguage)
	      +inputRow(request,"Web","comment","Comment","Admin",activePatient.comment,"TA",true, false,sWebLanguage)
