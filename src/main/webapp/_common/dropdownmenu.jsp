...
<script>    
  ...
  function newEncounter(){
	...
	function deleteServer(id){
		window.location.href='<%=sCONTEXTPATH%>/main.do?Page=statistics/offlineSyncStats.jsp&delete='+id;
	}
  <%-- NEW FAST ENCOUNTER --%>
  ...
  function openRemoteGMAO(){
	  window.open('<%=MedwanQuery.getInstance().getConfigString("remoteAssetURL","http://localhost/openclinic/assets/enterAssets.jsp")+"?autologin="+SH.cs("remotelogingmao."+activeUser.userid,"")+";"+SH.cs("remotepasswordgmao."+activeUser.userid,"")%>');
  }
  ...
  function showNeonatologyMonitor(){
	    openPopup("/util/showNeonatologyMonitor.jsp&ts=<%=getTs()%>",screen.width,screen.height).focus();
	  }
  ...
  function readBarcode2(barcode){
    ...
    if(barcode.substring(0,1)=="0"){
        window.location.href = "<c:url value='/patientslist.do'/>?findPersonID="+barcode.substring(1);
    }
  ...
  function showPatientTracking(){
	  window.location.href='<c:url value="/main.do?Page=patienttracking/readerMonitor.jsp"/>';
  }
  
  function linkBLETagToActivePatient(){
		var alias=window.prompt('<%=getTranNoLink("web","alias",sWebLanguage)%>');
		var url = '<c:url value="/patienttracking/ajax/linkBLETag.jsp"/>';
		new Ajax.Request(url,{
		  method: "GET",
  ...
  function unlinkBLETag(id){
	  	var alias="";
	  	if(id){
	  		alias=id;
	  	}
	  	else{
	  		alias=window.prompt('<%=getTranNoLink("web","alias",sWebLanguage)%>');
	  	}
		var url = '<c:url value="/patienttracking/ajax/unlinkBLETag.jsp"/>';
		new Ajax.Request(url,{
		  method: "GET",
		  parameters: "alias="+alias,
		  onSuccess: function(resp){
  ...
  function makeMalariyaPiBarcode(){
	  	var url = '<c:url value="/util/makeMalariyaPiBarcode2.jsp"/>';
	  	new Ajax.Request(url,{
		      parameters: "",
		      onSuccess: function(resp){
	              var barcode = eval("("+resp.responseText+")");
	              <% if(activePatient!=null && SH.c(activePatient.personid).length()>0){%>
		    	  	Modalbox.show("<div><table width='100%'><tr><td><img src='"+barcode.data+"'/></td><td><img height='50px' src='<%=sCONTEXTPATH%>/_img/themes/default/malariyapi.png'/><br/><br/><font style='font-size:14px'>[<%=activePatient.personid%>] <%=activePatient.getFullName()%><br/><%=activePatient.gender+" "+activePatient.dateOfBirth%>&nbsp;&nbsp;-&nbsp;&nbsp;<%=SH.cs("malariyapi.serverid","")%>.<%=Encounter.getActiveEncounter(activePatient.personid)!=null?Encounter.getActiveEncounter(activePatient.personid).getUid():Encounter.getLastEncounter(activePatient.personid).getUid()%></font></td></tr></table></div>",{title:"<%=getTranNoLink("web","scanbarcodewithmalariyapi",sWebLanguage)%>",width:400,height:170});
		    	  <%}%>
		      }
		});
  }
  ...
  function showdrugsoutbarcode(){
    openPopup("pharmacy/drugsOutBarcode.jsp&ts=<%=getTs()%>",900,500);
  }
  ...
  function showmanual(){
    <%
		if(MedwanQuery.getInstance().getConfigString("forceManual","").length()>0){
	        %>window.open("<%=MedwanQuery.getInstance().getConfigString("forceManual","")%>");<%
		}
		else if(MedwanQuery.getInstance().getConfigString("documentationLanguages","en,fr").toLowerCase().indexOf(sWebLanguage.toLowerCase())>-1){
  ...
  function openCNTSDashboard(){
	  openPopup('dashboards/<%=SH.cs("cntsDashbord","cntsDashboard.jsp")%>&language=<%=sWebLanguage%>',1024,800,'Dashboards');
  }
