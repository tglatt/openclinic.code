...
<%
	...
	PreparedStatement ps = conn.prepareStatement("select distinct * from ghb_servers order by ghb_server_domain,ghb_server_name");
...
<script>
	function selectServer(id,name){
		if(window.opener.document.getElementById('destinationid')){
			window.opener.document.getElementById('destinationid').value=id;
		}
		if(window.opener.document.getElementById('ITEM_TYPE_DESTINATIONID')){
			window.opener.document.getElementById('ITEM_TYPE_DESTINATIONID').value=id;
		}
		if(window.opener.document.getElementById('destination')){
			window.opener.document.getElementById('destination').value=name;
		}
		if(window.opener.document.getElementById('ITEM_TYPE_DESTINATIONNAME')){
			window.opener.document.getElementById('ITEM_TYPE_DESTINATIONNAME').value=name;
		}
		window.close();
	}
</script>