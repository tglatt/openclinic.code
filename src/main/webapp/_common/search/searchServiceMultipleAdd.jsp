...
<%!
    ...
    private String writeMyRow(String sID, String sLabel, String sWebLanguage, int rowIdx) {
        ...
        sReturn.append("<tr>")
                ...
                .append("&nbsp;<img src='" + sCONTEXTPATH + "/_img/icons/icon_view.png' onmouseover='this.style.cursor=\"hand\"' onmouseout='this.style.cursor=\"default\"' alt='" + getTran(null,"Web", "view", sWebLanguage) + "'" + " onclick='viewService(\"" + sID + "\")'>");

        /*
        // add edit icon if user has the right permissions
        if(activeUser.getAccessRight("system.manageservices.edit")){
            sReturn.append("&nbsp;<img src='"+sCONTEXTPATH+"/_img/icons/icon_edit.png' onmouseover='this.style.cursor=\"hand\"' onmouseout='this.style.cursor=\"default\"' alt='"+getTranNoLink("Web","edit",sWebLanguage)+"'"+" onclick='editService(\""+sID+"\")'>");
        }
        */
                            ...
                            <%
                                if ((sViewCode.length()>0)||(iTotal==0)){
                                    ...
                                    if (sService!=null) {
                                        ...
                                        out.print(setRow(request,"Web","Address", checkString(sService.address),sWebLanguage));
                                        out.print(setRow(request,"Web","zipcode", checkString(sService.zipcode),sWebLanguage));
                                        out.print(setRow(request,"Web","city", checkString(sService.city),sWebLanguage));
                                        out.print(setRow(request,"Web","country", sCountry,sWebLanguage));
                                        out.print(setRow(request,"Web","telephone", checkString(sService.telephone),sWebLanguage));
                                        out.print(setRow(request,"Web","fax", checkString(sService.fax),sWebLanguage));
                                        out.print(setRow(request,"Web","contract", checkString(sService.contract),sWebLanguage));
                                        out.print(setRow(request,"Web","contracttype", checkString(sService.contracttype),sWebLanguage));
                                        out.print(setRow(request,"Web","contactperson", checkString(sService.contactperson),sWebLanguage));
                                        out.print(setRow(request,"Web","comment", checkString(sService.comment),sWebLanguage));
                                        out.print(setRow(request,"Web","medicalcentre", checkString(sService.code5),sWebLanguage));
