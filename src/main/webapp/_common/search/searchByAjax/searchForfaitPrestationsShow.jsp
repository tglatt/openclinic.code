...
<table width='100%'>
<%
    String sForfaitUid = checkString(request.getParameter("EditForfaitUID"));
	if(sForfaitUid.length()>0){
		Prestation forfait = Prestation.get(sForfaitUid);
		if(forfait!=null){
			//First print all linked prestations
			HashSet prestations = forfait.getIncludedPrestations();
			if(prestations.size()>0){
				out.println("<tr class='admin'><td colspan='6'>"+getTran(request,"web","linkedprestations",sWebLanguage)+" "+forfait.getDescription()+"</td></tr></table><table id='searchresults' class='sortable' width='100%'>");
