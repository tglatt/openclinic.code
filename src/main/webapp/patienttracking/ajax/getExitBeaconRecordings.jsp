...
<%
	...
	while(rs.next()){
		...
		if(rs2.next()){
			...
			if(beaconAlias.trim().length()>0){
				if(comment.trim().length()==0){
					comment=getTranNoLink("web","unknown",sWebLanguage);
				}
				mustBeep=true;
				out.println("<div style='font-size: 44px;text-align: center;font-weight: bolder;color: red'>"+beaconAlias+" : "+comment+"</div>");
			}
		}
		rs2.close();
		ps2.close();
