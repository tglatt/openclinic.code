...
<!-- 
 ...
 -->
<%
	String accessright="mspls.malaria.followup";
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
        <%-- DATE --%>
        <tr>
            <td class="admin" width="<%=sTDAdminWidth%>" colspan="3">
            ...
            <td class="admin">
            	...
            	%>
            		<%=getTran(request,"web","admissiondays",sWebLanguage) %>: <span style='font-size: 14px;border: 2px solid;padding: 1px; background-color: white'><%=1+(new java.util.Date().getTime()-activeEncounter.getBegin().getTime())/SH.getTimeDay() %></span>
            	<%	} %>
            </td>
        </tr>
	    ...
	    <tr>
			...
			<td id='severetreatment3' class='admin2' colspan='2'>
				<%=getTran(request,"gfmalaria","quinine",sWebLanguage) %>:
				<%=SH.writeDefaultRadioButtons(tran, request, "yesno", "ITEM_TYPE_TREATMENT_QUININE", sWebLanguage, false, "", "") %>
				&nbsp;|&nbsp;<%=getTran(request,"web","dose",sWebLanguage) %>:
				<%=SH.writeDefaultTextInput(session, tran, "ITEM_TYPE_TREATMENT_QUININE_DOSE", 20) %>
			</td>
		</tr>
	    <tr>
		...
		<tr>
			...
			<td class='admin2' colspan='3'>
				<%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_EVOLUTION", "gfmalaria.evolution", sWebLanguage, "") %>
			</td>
		</tr>
