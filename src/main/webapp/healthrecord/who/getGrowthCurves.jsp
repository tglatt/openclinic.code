...
<script>
	...
	function showWeightforageGraph(){
		...
		if(document.getElementById('trandate') && document.getElementById('transactionId') && document.getElementById('transactionId').value*1<0 && document.getElementById('ITEM_TYPE_WEIGHT') && document.getElementById('ITEM_TYPE_WEIGHT').value*1>0){
			var year=document.getElementById('trandate').value.substring(6,10)*1;
			var month=document.getElementById('trandate').value.substring(3,5)*1-1;
			var day=document.getElementById('trandate').value.substring(0,2)*1;
			ds[5].data.length=ds[5].data.length+1;
			ds[5].data[ds[5].data.length-1]={x: new Date(year,month,day,12,00),y: document.getElementById('ITEM_TYPE_WEIGHT').value*1};
		}
	...
	function showHeightforageGraph(){
		...
		if(document.getElementById('trandate') && document.getElementById('transactionId') && document.getElementById('transactionId').value*1<0 && document.getElementById('ITEM_TYPE_HEIGHT') && document.getElementById('ITEM_TYPE_HEIGHT').value*1>0){
			var year=document.getElementById('trandate').value.substring(6,10)*1;
			var month=document.getElementById('trandate').value.substring(3,5)*1-1;
			var day=document.getElementById('trandate').value.substring(0,2)*1;
			ds[5].data.length=ds[5].data.length+1;
			ds[5].data[ds[5].data.length-1]={x: new Date(year,month,day,12,00),y: document.getElementById('ITEM_TYPE_HEIGHT').value*1};
		}
	...
	function showWeightforheightGraph(){
		...
		%>
		if(document.getElementById('transactionId') && document.getElementById('transactionId').value*1<0 && document.getElementById('ITEM_TYPE_HEIGHT') && document.getElementById('ITEM_TYPE_HEIGHT').value*1>0 && document.getElementById('ITEM_TYPE_WEIGHT') && document.getElementById('ITEM_TYPE_WEIGHT').value*1>0){
			if(document.getElementById('ITEM_TYPE_HEIGHT').value*1<minval){
				minval=document.getElementById('ITEM_TYPE_HEIGHT').value*1-10;
			}
			if(document.getElementById('ITEM_TYPE_HEIGHT').value*1>maxval){
				maxval=document.getElementById('ITEM_TYPE_HEIGHT').value*1+10;
			}
		}
		var minval=<%=Math.floor(minval)%>;
		var maxval=<%=Math.ceil(maxval)%>;
		var spread=<%=Math.floor((Math.ceil(maxval)-Math.floor(minval))/5)%>;
		var cannotations = [];
		var ds= [
		   	//Now add the measurement data
		   	{
		...
		if(document.getElementById('trandate') && document.getElementById('transactionId') && document.getElementById('transactionId').value*1<0 && document.getElementById('ITEM_TYPE_HEIGHT') && document.getElementById('ITEM_TYPE_HEIGHT').value*1>0 && document.getElementById('ITEM_TYPE_WEIGHT') && document.getElementById('ITEM_TYPE_WEIGHT').value*1>0){
			var year=document.getElementById('trandate').value.substring(6,10)*1;
			var month=document.getElementById('trandate').value.substring(3,5)*1-1;
			var day=document.getElementById('trandate').value.substring(0,2)*1;
			ds[0].data.length=ds[0].data.length+1;
			ds[0].data[ds[0].data.length-1]={x: document.getElementById('ITEM_TYPE_HEIGHT').value*1,y: document.getElementById('ITEM_TYPE_WEIGHT').value*1};
		}
		weightforheightGraph=drawGraph2(weightforheightGraph,'weightforheight',40,120,spread,cannotations,ds);
		<%
			conn.close();
		%>
	}
