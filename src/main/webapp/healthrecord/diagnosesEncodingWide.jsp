...
<table class="list" width="100%" cellspacing="1" cellpadding="1">
    <tr class="admin2">
		...
		// Armel Check if the icd11 is disable then display the icpc and icd10
		if (MedwanQuery.getInstance().getConfigInt("enableICD11",0) == 0) { %>
			...
			<%-- Armel Check if the icd11 is enabled then display the icd11 and icd11 link --%>
	   		<% if (MedwanQuery.getInstance().getConfigInt("enableICD11",0) == 1) { %>
			<table class="list" width="100%" cellspacing="1">
			    <tr class="admin">
			        <td align="center"><a href="javascript:openPopup('healthrecord/findICD11.jsp&AuthorUID='+(document.getElementById('diagnosisUser')?document.getElementById('diagnosisUser').value:'')+'&ts=<%=getTs()%>&patientuid=<%=activePatient.personid %>',700,400);void(0);"><%=getTran(request,"openclinic.chuk","diagnostic.document",sWebLanguage)%> <%=getTran(request,"Web.Occup","ICD-10",sWebLanguage)%>/<%=getTran(request,"Web.Occup","ICD-11",sWebLanguage)%></a></td>
			    </tr>
			    
			    <tr>
			    <td id="icd11Code">
			     <table width='100%' id="icd11codesTable">
			     <%
			     	items = curTran.getItems().iterator();
			     	while(items.hasNext()){
			             item = (ItemVO)items.next();
			             if(item.getType().indexOf("ICD11Code")==0){
			                 sCode = item.getType().substring("ICD11Code".length(),item.getType().length());
			                 ...
			                 %>
			                 <tr id='ICD11Code<%=item.getItemId()%>' style="list<%=sClass%>">
			                   <td width="1%" nowrap><img src='<c:url value="/_img/icons/icon_delete.png"/>' class="link" onclick="deleteDiagnosis(ICD11Code<%=item.getItemId()%>);"/></td>
			                   <td width="1%">ICD11</td>
			                   <td width="1%"><b><%=item.getType().replaceAll("ICD11Code","")%></b></td>
			                   <td><b><%=MedwanQuery.getInstance().getCodeTran(item.getType().trim(),sWebLanguage)%> <font color='red'><i><%=item.getValue().trim()%></i></font>&nbsp;<i>G:<%=sGravity%>/C:<%=sCertainty%><%=POA.length()>0?" POA":""%><%=NC.length()>0?" N":""%><%=flags.length()==0?"":" ("+flags+flagsuffix+")" %><%=userwarning %></i></b>
			                     <input type='hidden' name='ICD11Code<%=item.getType().replaceAll("ICD11Code","")%>' value='<%=item.getValue().trim()%>'/>
			                     <input type='hidden' name='GravityICD11Code<%=item.getType().replaceAll("ICD11Code","")%>' value="<%=sGravity%>"/>
			                     <input type='hidden' name='CertaintyICD11Code<%=item.getType().replaceAll("ICD11Code","")%>' value="<%=sCertainty%>"/>
			                     <input type='hidden' name='POAICD11Code<%=item.getType().replaceAll("ICD11Code","")%>' value="<%=POA%>"/>
			                     <input type='hidden' name='NCICD11Code<%=item.getType().replaceAll("ICD11Code","")%>' value="<%=NC%>"/>
			                     <input type='hidden' name='ServiceICD11Code<%=item.getType().replaceAll("ICD11Code","")%>' value="<%=serviceUid%>"/>
			                     <input type='hidden' name='FlagsICD11Code<%=item.getType().replaceAll("ICD11Code","")%>' value="<%=flags%>"/>			                   
			                     <input type='hidden' name='UserICD11Code<%=item.getType().replaceAll("ICD11Code","")%>' value="<%=userid%>"/>			                   
			                   </td>
			                 </tr>
			                 <%
			             }
			    ...
			    <%
			        if(activeEncounterUid.length()>0){
						...
						for(int n=0; n<d.size(); n++){
							...
							else if(diag.getCodeType().equalsIgnoreCase("icd11")){
							    %><tr class="list<%=sClass%>"><td width="1%">ICD10</td><td width="1%"><b><%=diag.getCode()%></b></td><td><b><%=MedwanQuery.getInstance().getCodeTran("icd11code"+diag.getCode(),sWebLanguage)%> <%=diag.getLateralisation()%>&nbsp;<i>G:<%=sGravity%>/C:<%=sCertainty%><%=POA.length()>0?" POA":""%><%=NC.length()>0?" N":""%><%=flags.length()==0?"":" ("+flags+flagsuffix+")" %></i></b></td></tr><%
							}
						}
				    }
				%>
