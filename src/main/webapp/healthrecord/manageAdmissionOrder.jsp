...
<%=checkPermission(out,"pediatric.admissionorder","select",activeUser)%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    	...
    	<tr>
	        <td style="vertical-align:top;height: 200px" width="70%">
	        	<table width="100%">
        			<tr>
						<td class='admin'>
							<div id="keywords_title_ITEM_TYPE_ADMISSIONORDER_STATUS"><%=getTran(request,"admissionorder","statusfield",sWebLanguage)%></div>
						</td>
						<td class='admin2'>
							<%=ScreenHelper.writeDefaultKeywordField(session,(TransactionVO)transaction, "ITEM_TYPE_ADMISSIONORDER_STATUS", "admissionorder.status", "keywords", sCONTEXTPATH, sWebLanguage,request) %>
				      	</td>
        			</tr>
        			<tr>
						<td class='admin'>
							<div id="keywords_title_ITEM_TYPE_ADMISSIONORDER_SPECIALTY"><%=getTran(request,"admissionorder","specialtyfield",sWebLanguage)%></div>
						</td>
						<td class='admin2'>
							<%=ScreenHelper.writeDefaultKeywordField(session,(TransactionVO)transaction, "ITEM_TYPE_ADMISSIONORDER_SPECIALTY", "admissionorder.specialty", "keywords", sCONTEXTPATH, sWebLanguage,request) %>
				      	</td>
        			</tr>
        			<tr>
        				<td class='admin'><%=getTran(request,"admissionorder","estimatedadmissiondays",sWebLanguage) %></td>
        				<td class='admin2'><%=ScreenHelper.writeDefaultNumericInput(session, (TransactionVO)transaction, "ITEM_TYPE_ADMISSIONORDER_ESTIMATEDDAYS", 5, 1, 100, sWebLanguage) %></td>
        			</tr>
        			<tr>
        				<td class='admin'><%=getTran(request,"admissionorder","diseasetype",sWebLanguage) %></td>
        				<td class='admin2'><%=ScreenHelper.writeDefaultRadioButtons((TransactionVO)transaction, request, "admissionorder.diseasetype", "ITEM_TYPE_ADMISSIONORDER_DISEASETYPE", sWebLanguage, true, "", "")%></td>
        			</tr>
	        	</table>
	        </td>
	        <%-- KEYWORDS --%>
	...
	<%=ScreenHelper.alignButtonsStart()%>
	    <%=getButtonsHtml(request,activeUser,activePatient,"pediatric.admissionorder",sWebLanguage)%>
