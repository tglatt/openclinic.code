    ...
    <table class='menu' width='100%' cellspacing="0">
        <tr>
            ...
            <td colspan='2' class='admin2'>
                ...
                <% if(SH.cs("openai-api-key","").length()>0){ %>
                	<span id='openaiwait'><input class='button' type='button' name='openaibutton' id='openaibutton' onclick='searchOpenAI()' value='<%=getTranNoLink("Web","openai-icd10",sWebLanguage)%>'/></span>
                <% } %>
...
<script>
  ...
  function searchOpenAI(){
	  document.getElementById("openaiwait").innerHTML="&nbsp;<img height='14px' src='<c:url value="/_img/icons/icon-search.gif"/>'/>";        
	  	var params = "keywords="+document.getElementById("keywords").value;
	  	var url = '<c:url value="/ikirezi/checkICD10.jsp"/>';
	    new Ajax.Request(url,{
		      parameters: params,
		      onSuccess: function(resp){
		    	  document.getElementById("openaiwait").innerHTML="<input class='button' type='button' name='openaibutton' id='openaibutton' onclick='searchOpenAI()' value='<%=getTranNoLink("Web","openai-icd10",sWebLanguage)%>'/>";
		    	  openPopup("<c:url value="ikirezi/showOpenAIResult.jsp"/>&attribute=checkICD10code",600,400);
		      }
		});
  }
