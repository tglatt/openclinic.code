	            ...
	            <select class="text" id="cntsobjectid" name="currentTransactionVO.items.<ItemVO[hashCode=<mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_LAB_OBJECTID" property="itemId"/>]>.value">
	                ...
	                <%
	                	...
	                	for(int n=0;n<bloodgifts.size();n++){
	                		...
	                		String actualvalue= tran.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_LAB_OBJECTID");
	                		if(expirydate.length()==10 || actualvalue.equalsIgnoreCase(bloodgift.getTransactionId()+"")){
	                			try{
	                				java.util.Date expdate = ScreenHelper.parseDate(expirydate);
	                				if(expdate.after(new java.util.Date()) || actualvalue.equalsIgnoreCase(bloodgift.getTransactionId()+"")){
	                					if(actualvalue.equalsIgnoreCase(bloodgift.getTransactionId()+"")){
	                						bActive=true;
	                					...
	                					else if(!bloodgift.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNTSBLOODGIFT_FIT").equalsIgnoreCase("medwan.common.true") && (bloodgift.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNTSBLOODGIFT_FIT").equalsIgnoreCase("medwan.common.false") || bloodgift.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNTSBLOODGIFT_PERMANENTREJECTION").equalsIgnoreCase("medwan.common.true") || bloodgift.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNTSBLOODGIFT_TEMPORARYREJECTION").equalsIgnoreCase("medwan.common.true")) ){
	                	...
	                	if(!bActive){
	                		out.println("<option value='-1' selected>N/A</option>");
	                		out.println("<option value='-2' >"+getTranNoLink("web","technicalcontrol",sWebLanguage)+"</option>");
	                		out.println("<option value='-3' >"+getTranNoLink("web","qualitycontrol",sWebLanguage)+"</option>");
	                	}
	                %>
	            </select>
	            <% if(!bActive){%>
	                 &nbsp;<%=getTran(request,"bloodgift","samplingunit",sWebLanguage)%>: <%=SH.writeDefaultSelect(request, (TransactionVO)transaction, "ITEM_TYPE_CNTSBLOODGIFT_COLLECTIONUNIT","cnts.collectionunit", sWebLanguage, "",SH.c((String)session.getAttribute("defaultCNTSCollectionUnit"))) %>
	            <%} %>
	        </td>
	    </tr>
	<%
...
<script>
  ...
  function doSubmit(){
	...
	%>
	    ...
	    else if(document.getElementById("cntsobjectid").value=="-1" && document.getElementById("ITEM_TYPE_CNTSBLOODGIFT_COLLECTIONUNIT").value.length==0){
			alert('<%=getTranNoLink("bloodgift","samplingunit",sWebLanguage)%> <%=getTranNoLink("web","ismandatory",sWebLanguage)%>');
