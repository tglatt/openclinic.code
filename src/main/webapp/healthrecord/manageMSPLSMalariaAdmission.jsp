...
<!-- 
 ...
 -->
<%
	String accessright="mspls.malaria.admission";
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
        ...
        -->
        <tr>
        	<td class='admin'><%=getTran(request,"web","datetimeofarrival",sWebLanguage) %></td>
        	<td class='admin2'>
        		<%
        			if(tran.isNew()){
        				Encounter activeEncounter= Encounter.getActiveEncounter(activePatient.personid);
        				if(activeEncounter!=null){
        					tran.getItem("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_ARRIVALDATETIME").setValue(SH.formatDate(activeEncounter.getBegin(), "dd/MM/yyyy"));
        					tran.getItem("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_ARRIVALDATETIME_TIME").setValue(SH.formatDate(activeEncounter.getBegin(), "HH:mm"));
        				}
        			}
        		%>
        		<%=SH.writeDefaultDateTimeInput(session, tran, "ITEM_TYPE_ARRIVALDATETIME", sWebLanguage, sCONTEXTPATH) %>
        	</td>
        	<td class='admin'><%=getTran(request,"web","declaredwaitingtime",sWebLanguage) %></td>
        	<td class='admin2'>
        		<%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_DECLAREDWAITINGTIME", 5, 0, 999, sWebLanguage) %> min
        	</td>
        </tr>
        <%if(activePatient.gender.equalsIgnoreCase("f") && activePatient.getAge()>=12 && activePatient.getAge()<=60){ %>
	        <tr>
	        	...
	        	<td class='admin'><%=getTran(request,"web","pregancyduration",sWebLanguage) %></td>
	        	<td class='admin2'>
	        		<%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_PREGANCYDURATION", 5, 0, 999, sWebLanguage) %> <%=getTran(request,"web","weeks",sWebLanguage) %>
	        	</td>
	        </tr>
	     <%} %>
	    ...
	    <tr class='admin'><td colspan='4'><%=getTran(request,"web","vitalsigns",sWebLanguage) %></td></tr>
		<tr>
	    ...
	    <tr>
			...
			<td class='admin2'>
				<%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_FAMILYHISTORY", 40, 1)%>
			</td>
		</tr>
	    <tr>
	    ...
	    <tr>
			...
			<td class='admin2'>
				<%=SH.writeDefaultRadioButtons(tran, request, "gfmalaria.presumeddiagnosislimited", "ITEM_TYPE_PRESUMEDDIAGNOSIS", sWebLanguage, false, "onchange='checkFields()'", "") %>
	    ...
	    <tr class='admin'><td colspan='4'><%=getTran(request,"web","decisions",sWebLanguage) %></td></tr>
		<tr>
			<td class='admin'><%=getTran(request,"web","examinations",sWebLanguage) %></td>
			<td class='admin2' colspan='3'>
				<%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_EXAMINATIONS", 40, 1) %>
			</td>
		</tr>
		<tr>
			<td class='admin'><%=getTran(request,"malaria","treatments",sWebLanguage) %></td>
			<td class='admin2' colspan='3'>
				<%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_TREATMENTS", 40, 1) %>
			</td>
		</tr>
		<tr>
			<td class='admin'><%=getTran(request,"web","recommendations",sWebLanguage) %></td>
			<td class='admin2' colspan='3'>
				<%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_RECOMMENDATIONS", 40, 1) %>
			</td>
		</tr>
    </table>
...
<script>  
  ...
  function checkFields(){
	  ...
	  if(document.getElementById("ITEM_TYPE_PREGNANTWOMEN.1") && document.getElementById("ITEM_TYPE_PREGNANTWOMEN.1").checked){
