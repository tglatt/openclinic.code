...
<table width="100%">
	<tr>
		...
		<td width='20%' nowrap><%=SH.writeDefaultLastTodayNumericInput(request, tran, "[GENERAL.ANAMNESE]ITEM_TYPE_TEMPERATURE", 5,25,50,sWebLanguage,"if(triage) triage();") %> °C</td>
		<td width='1%' nowrap><b><%=getTran(request,"Web.Occup","medwan.healthrecord.biometry.length",sWebLanguage)%>:</b></td>
		<td width='20%' nowrap><%=SH.writeDefaultLastNumericInput(request, tran, "ITEM_TYPE_BIOMETRY_HEIGHT", 5,0,250,sWebLanguage,"calculateBMI();if(triage) triage();") %> cm</td>
		<td width='1%' nowrap><b><%=getTran(request,"Web.Occup","medwan.healthrecord.biometry.weight",sWebLanguage)%>:</b></td>
		<td width='20%' nowrap><%=SH.writeDefaultLastTodayNumericInput(request, tran, "ITEM_TYPE_BIOMETRY_WEIGHT", 5,0,400,sWebLanguage,"calculateBMI();if(triage) triage();") %> kg</td>
	...
	<tr>
		...
		<td nowrap><%=SH.writeDefaultLastTodayNumericInput(request, tran, "[GENERAL.ANAMNESE]ITEM_TYPE_SATURATION", 5,0,100,sWebLanguage,"if(triage) triage();") %> %</td>
		<td width='1%' nowrap><b><%=getTran(request,"web","abdomencircumference",sWebLanguage)%>:</b></td>
		<td nowrap><%=SH.writeDefaultLastNumericInput(request, tran, "ITEM_TYPE_ABDOMENCIRCUMFERENCE", 5,0,200,sWebLanguage,"if(triage) triage();") %> cm</td>
		<% if(activePatient.gender.toLowerCase().startsWith("f") && activePatient.getAge()>12){ %>
			...
			<td nowrap><%=SH.writeDefaultLastTodayNumericInput(request, tran, "ITEM_TYPE_FOETAL_HEARTRATE", 5,0,300,sWebLanguage,"if(triage) triage();") %> /min</td>
			<td width='1%' nowrap><b><%=getTran(request,"web","armcircumferenceshort",sWebLanguage)%>:</b></td>
			<td nowrap><%=SH.writeDefaultLastNumericInput(request, tran, "ITEM_TYPE_ARM_CIRCUMFERENCE", 5,0,50,sWebLanguage,"if(triage) triage();") %> cm</td>
		<% }else{ %>
			<td width='1%' nowrap><b><%=getTran(request,"web","armcircumferenceshort",sWebLanguage)%>:</b></td>
			<td nowrap>
				<%=SH.writeDefaultLastNumericInput(request, tran, "ITEM_TYPE_ARM_CIRCUMFERENCE", 5,0,50,sWebLanguage,"if(triage) triage();") %> cm
	...
	<tr>
		...
		<td colspan='7' nowrap><%=ScreenHelper.writeDefaultLastTodayTextInput(request, (TransactionVO) transaction, "ITEM_TYPE_BIOMETRY_GLYCEMIA", 5,"onchange='if(triage) triage();'") %> g/dl</td>
	</tr>
	<tr>
	...
	<tr>
		<td nowrap colspan='2'><%=SH.writeDefaultLastTodayNumericInput(request, tran, "ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_SYSTOLIC_PRESSURE_RIGHT", 3,0,300,sWebLanguage, "setBP(this,\"ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_SYSTOLIC_PRESSURE_RIGHT\",\"ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_DIASTOLIC_PRESSURE_RIGHT\");if(triage) triage();") %> / <%=SH.writeDefaultLastTodayNumericInput(request, tran, "ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_DIASTOLIC_PRESSURE_RIGHT", 3,0,300,sWebLanguage, "setBP(this,\"ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_SYSTOLIC_PRESSURE_RIGHT\",\"ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_DIASTOLIC_PRESSURE_RIGHT\");if(triage) triage();") %> mmHg</td>
		<td nowrap colspan='2'><%=SH.writeDefaultLastTodayNumericInput(request, tran, "[GENERAL.ANAMNESE]ITEM_TYPE_RESPIRATORY_FRENQUENCY", 5, 0, 60, sWebLanguage,"if(triage) triage();") %> /min</td>
		<td nowrap colspan='2'><%=SH.writeDefaultLastTodayNumericInput(request, tran, "ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_HEARTH_FREQUENCY", 5, 0, 300, sWebLanguage,"setHF(this);if(triage) triage();") %> /min</td>
		<td nowrap colspan='2'>
		<% if(activePatient.getAgeInDaysOnDate(tran.getUpdateTime())<1857){ %>
			<table width='100%'>
				<tr>
