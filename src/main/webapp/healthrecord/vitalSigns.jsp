...
<table width="100%">
	<tr>
		...
		<td width='20%' nowrap><%=SH.writeDefaultLastTodayNumericInput(request, tran, "[GENERAL.ANAMNESE]ITEM_TYPE_TEMPERATURE", 5,25,50,sWebLanguage,"if(triage) triage();if(checkFields) checkFields();") %> °C</td>
		<td width='1%' nowrap><b><%=getTran(request,"Web.Occup","medwan.healthrecord.biometry.length",sWebLanguage)%>:</b></td>
		<td width='20%' nowrap><%=SH.writeDefaultLastTodayNumericInput(request, tran, "ITEM_TYPE_BIOMETRY_HEIGHT", 5,0,250,sWebLanguage,"calculateBMI();if(triage) triage();") %> cm</td>
	...
	<tr>
		...
		<td nowrap><%=SH.writeDefaultNumericInput(session, tran, "[GENERAL.ANAMNESE]ITEM_TYPE_SATURATION", 5,0,100,sWebLanguage,"if(triage) triage();") %> %</td>
		<td width='1%' nowrap><b><%=getTran(request,"web","abdomencircumference",sWebLanguage)%>:</b></td>
		<td nowrap><%=SH.writeDefaultLastTodayNumericInput(request, tran, "ITEM_TYPE_ABDOMENCIRCUMFERENCE", 5,0,200,sWebLanguage,"if(triage) triage();") %> cm</td>
		<% if(activePatient.gender.toLowerCase().startsWith("f") && activePatient.getAge()>12){ %>
			...
			<td nowrap><%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_FOETAL_HEARTRATE", 5,0,300,sWebLanguage,"if(triage) triage();") %> /min</td>
			<td width='1%' nowrap><b><%=getTran(request,"web","armcircumferenceshort",sWebLanguage)%>:</b></td>
			<td nowrap><%=SH.writeDefaultLastTodayNumericInput(request, tran, "ITEM_TYPE_ARM_CIRCUMFERENCE", 5,0,50,sWebLanguage,"if(triage) triage();") %> cm</td>
		<% }else{ %>
			<td width='1%' nowrap><b><%=getTran(request,"web","armcircumferenceshort",sWebLanguage)%>:</b></td>
			<td nowrap>
				<%=SH.writeDefaultLastTodayNumericInput(request, tran, "ITEM_TYPE_ARM_CIRCUMFERENCE", 5,0,50,sWebLanguage,"if(triage) triage();") %> cm
	...
	<tr>
		...
		<td colspan='7' nowrap><%=ScreenHelper.writeDefaultTextInput(session, (TransactionVO) transaction, "ITEM_TYPE_BIOMETRY_GLYCEMIA", 5,"onchange='if(triage) triage();'") %> g/dl</td>
	</tr>
	<tr>
