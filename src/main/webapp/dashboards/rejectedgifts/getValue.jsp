...
<%
	...
	if(sTimeUnit.equalsIgnoreCase("day")){
		String sSql = 	"select count(distinct t.transactionid) total,date_format(t.updatetime,'%Y-%m-%d') day,i.value from transactions t,items i, items j where"+
						...
						" j.type in ('be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNTSBLOODGIFT_PERMANENTREJECTION','be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNTSBLOODGIFT_TEMPORARYREJECTION') and"+
						" j.value='medwan.common.true' and"+
						" t.updatetime>? and"+
						" i.value in (--ivalues--)"+
		...
		String sSql = 	"select count(distinct t.transactionid) total,date_format(t.updatetime,'%Y-%m-01') month,i.value from transactions t,items i,items j where"+
						" t.serverid=j.serverid and"+
						" t.transactionid=j.transactionid and"+
						" t.serverid=i.serverid and"+
						" t.transactionid=i.transactionid and"+
		...
		String sSql = 	"select count(distinct t.transactionid) total,date_format(t.updatetime,'%Y-01-01') year,i.value from transactions t,items i where"+
						" t.serverid=j.serverid and"+
						" t.transactionid=j.transactionid and"+
						" t.serverid=i.serverid and"+
						" t.transactionid=i.transactionid and"+
