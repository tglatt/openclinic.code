	...
	try{
		...
		if(sDates.length()>0){
			...
			for(int n=0;n<dates.length;n++){
				ps=conn.prepareStatement("select count(*) total from OC_PLANNING where OC_PLANNING_PLANNEDDATE >= ? and OC_PLANNING_PLANNEDDATE <? and OC_PLANNING_CANCELATIONDATE is NULL and OC_PLANNING_USERUID=?");
