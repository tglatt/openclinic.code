...
public class DHIS2Server
{
    ...
    public Document getXml(String query) throws DocumentException, IOException, NoSuchAlgorithmException {
    	String sUrl = query.replaceAll("\\[", "%5B").replaceAll("\\]", "%5D");
    	String response = sendToServer(sUrl);
    	Document document = DocumentHelper.parseText(XML.toString(new org.json.JSONObject(response)));
    	return document;
    }
    
    public org.json.JSONObject getJson(String query) throws DocumentException, IOException, NoSuchAlgorithmException {
    	String sUrl = query.replaceAll("\\[", "%5B").replaceAll("\\]", "%5D");
    	String response = sendToServer(sUrl);
    ...
    public String sendToServer(DataValueSet dataValueSet) throws NoSuchAlgorithmException, JAXBException, ClientProtocolException, IOException
    {
    	...
    	//Generate JSON from dataValueSet
    	JSONObject root = new JSONObject();
    	root.put("dataSet", dataValueSet.getDataSet());
    	root.put("completeDate", dataValueSet.getCompleteDate());
    	root.put("period", dataValueSet.getPeriod());
    	root.put("orgUnit", dataValueSet.getOrgUnit());
    	root.put("attributeOptionCombo", dataValueSet.getAttributeOptionCombo());
    	JSONArray dataValues = new JSONArray();
    	Iterator<DataValue> iDataValues = dataValueSet.getDataValues().iterator();
    	while(iDataValues.hasNext()) {
    		DataValue dataValue = iDataValues.next();
    		JSONObject dv = new JSONObject();
    		dv.put("dataElement", dataValue.getDataElementID());
    		dv.put("categoryOptionCombo", dataValue.getCategoryOptionComboID());
    		dv.put("value", dataValue.getValue());
    		dv.put("comment", dataValue.getComment());
    		dataValues.put(dv);
    	}
    	root.put("dataValues",dataValues);
