...
public class DHIS2Helper {
	...
	public static DataValueSet cleanDataValueSet(DataValueSet dataValueSet) {
		dataValueSet.setDataSet(dataValueSet.getDataSet().split(";")[0]);
		if(dataValueSet.getAttributeOptionCombo().contains("-getorguid")) {
	...
	public static void sendToServer(DataValueSet dataValueSet,Element dataset, JspWriter jspWriter){
		dataset.addAttribute("uid", dataset.attributeValue("uid").split(";")[0]);
        ...
        try {
            SH.syslog("Sending dataValueSet to DHIS2 server " + DHIS2_SERVER_URI);
            String postResponse = server.sendToServer(cleanDataValueSet(dataValueSet));
            SH.syslog("POST response = "+postResponse);
			...
			try {
				...
				if(!activeDataSet.equals(title) || !activeAttribute.equalsIgnoreCase(attribute)){
					...
					if(!activeDataSet.startsWith(getDataSetTitle(root.attributeValue("dataSet"),dataSetLabel))){
						...
						html.append("<table data-tag='dhis2' width='100%'><tr><td style='height: 20px;background-color: "+getDataSetColor(root.attributeValue("dataSet"),dataSetLabel)+";color:white;font-weight: bolder' colspan='"+(1+columns)+"'>"+getDataSetTitle(root.attributeValue("dataSet"),dataSetLabel).toUpperCase()+"</td></tr>");
						bColumnsDrawn=false;
					}
					...
					while(dataElements.hasNext()){
						...
						if(!ScreenHelper.checkString(dataset.attributeValue("missing")).equalsIgnoreCase("1") && dataset.attributeValue("type").equalsIgnoreCase("pharmacy")){
							...
							if(dataElement.attributeValue("uid").equals(uid)){
								String name = ScreenHelper.checkString(dataElement.attributeValue("label"));
								String flags = ScreenHelper.checkString(dataElement.attributeValue("flags"));
								String sFlagLabels ="";
								if(flags.length()>0) {
									sFlagLabels="{";
									for(int n=0;n<flags.split(",").length;n++) {
										if(sFlagLabels.length()>1) {
											sFlagLabels+=", ";
										}
										if(flags.split(",")[n].equals(flags.split(",")[n].toLowerCase())) {
											sFlagLabels+=SH.getTranNoLink("lowercasediagnosisflags", flags.split(",")[n], language);
										}
										else {
											sFlagLabels+=SH.getTranNoLink("uppercasediagnosisflags", flags.split(",")[n], language);
										}
									}
									sFlagLabels+="} ";
								}
								if(name.length()==0){
									...
									if(tokens.split(",").length>1){
										if(dataset.attributeValue("type").equalsIgnoreCase("diagnosis")){
											name = tokens.split(",")[0]+"<img src='"+MedwanQuery.getInstance().getConfigString("imageSource","http://localhost/openclinic")+"/_img/icons/icon_info.gif'/> ["+tokens.replaceAll(tokens.split(",")[0]+",", "")+"] - "+MedwanQuery.getInstance().getCodeTran("icd10code"+tokens.split(",")[0], language).replaceAll("'", "´")+" "+sFlagLabels;
										}
										else{
										...
										if(dataset.attributeValue("type").equalsIgnoreCase("diagnosis")){
											name = tokens+"<img src='"+MedwanQuery.getInstance().getConfigString("imageSource","http://localhost/openclinic")+"/_img/icons/icon_info.gif'/> "+MedwanQuery.getInstance().getCodeTran("icd10code"+tokens.split(",")[0], language).replaceAll("'", "´")+" "+sFlagLabels;
										}
										else{
											name=tokens;
										}
