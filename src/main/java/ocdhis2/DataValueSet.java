...
public class DataValueSet
{
    ...
    public void setOrgUnit(Element ds)
    {
        if(SH.c(attributeOptionCombo).length()>0 && attributeOptionCombo.endsWith("-getorguid")) {
        	//Set organisationuid which is specific for a department
        	String[] orgUnitMappings = SH.cs("dhis2_department_orgunits", "").split(";");
        	for(int n=0;n<orgUnitMappings.length;n++) {
        		if(orgUnitMappings[n].split("=").length>1 && orgUnitMappings[n].split("=")[0].equalsIgnoreCase(attributeOptionCombo.replaceAll("-getorguid", ""))){
        			this.orgUnit=orgUnitMappings[n].split("=")[1];
        			return;
        		}
        	}
        }
        if(SH.cs("activeservice", "").length()>0) {
        	if(Pointer.getPointer("dhis2.orgunitid."+SH.cs("activeservice", "")).length()>0) {
        		orgUnit=Pointer.getPointer("dhis2.orgunitid."+SH.cs("activeservice", ""));
        		return;
        	}
        }
    	//Set generic organisationuid
    	orgUnit=SH.cs(SH.c(ds.attributeValue("dhis2serverprefix",""))+"dhis2_orgunit","");
    }
