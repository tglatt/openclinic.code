...
public class ActiveARVGuinea extends DHIS2Plugin {
	...
	public Vector getItems(Date begin, Date end, Element dataset, Hashtable pluginParameters) {
		...
		try {
			...
			ps.setDate(2, SH.toSQLDate(begin));
				...
				if(!uniquepatients.contains(personid)) {
					...
					if(pluginParameters!=null) {
						...
						while(keys.hasMoreElements()){
							...
							if(key.startsWith("be.mxs.common.model.vo.healthrecord.IConstants")) {
					...
					if(pluginParameters.get("dead")!=null) {
						AdminPerson person = AdminPerson.get(personid);
						SH.syslog("dead="+person.isDead());
						if(person.isDead()==null || person.isDead().before(begin) || person.isDead().after(end)) {
							continue;
						}
					}
		...
		SH.syslog("ARVActiveGuinea plugin returned "+items.size()+" items");
