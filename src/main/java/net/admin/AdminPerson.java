...
public class AdminPerson extends OC_Object{
    ...
    public java.util.Date isDead(){
        ...
        if(death==null) {
        	ItemVO item = MedwanQuery.getInstance().getLastItemVO(getPersonId(), "be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_MSHP_CONSULTATION", "be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_RESULTS");
        	if(item!=null && item.getValue().equalsIgnoreCase("5")) {
        		death=item.getDate();
        	}
        }
        if(death==null) {
        	ItemVO item = MedwanQuery.getInstance().getLastItemVO(getPersonId(), "be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_MSHP_CSI_CONSULTATION", "be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_RESULTS");
        	if(item!=null && item.getValue().equalsIgnoreCase("4")) {
        		death=item.getDate();
        	}
        }
        if(death==null) {
        	ItemVO item = MedwanQuery.getInstance().getLastItemVO(getPersonId(), "be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_MSHP_DELIVERY", "be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_MSAS_DELIVERIES_MOTHERSTATUS");
        	if(item!=null && (item.getValue().equalsIgnoreCase("3")||item.getValue().equalsIgnoreCase("4"))) {
        		death=item.getDate();
        	}
        }
        return death;
