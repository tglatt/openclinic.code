...
public class Invoice extends OC_Object {
    ...
    public Vector getDebetsForInsurer(String sInsurerUid) {
        Vector d = new Vector();
        for(int n=0;n<getDebets().size();n++){
            Debet debet = (Debet)getDebets().elementAt(n);
            if(debet!=null && debet.getInsurance()!=null && sInsurerUid.equalsIgnoreCase(debet.getInsurance().getInsurarUid())){
                d.add(debet);
            }
        }
        return d;
