...
public class UpdateStats4 extends UpdateStatsBase{
	...
	public void execute(){
		...
		try{
			...
			while(rs.next()){
				try{
					...
					counter++;
					if(counter%100==0) Debug.println("U4 processing encounter UID "+encounteruid+" (#"+counter+") "+new SimpleDateFormat("yyyyMMddHHmmssSSS").format(lastupdatetime));
					...
					try{
						...
						Debug.println("debetobjectid="+debetobjectid);
						Debug.println("patientamount="+patientamount);
						e2.printStackTrace();
					}
					finally{
						stats_conn.close();
			...
			rs.close();
			ps.close();
			Debug.println("closing "+this.modulename);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
		...
		Debug.println("closed "+this.modulename);
	}
}
