...
public class ArchiveDocument extends OC_Object implements Comparable {
	...
	*/
	...
	public static ArchiveDocument getById(String sUid){
        return getById(sUid,false);		
	}
	
	public static ArchiveDocument getById(String sUid, boolean mustHaveStorageName){
		if(sUid.split("\\.").length<2) {
			return null;
		}
		ArchiveDocument archDoc = null;
		...
		try{
			ps = conn.prepareStatement(sSql);
			ps.setString(1,sUid.split("\\.")[0]);
			ps.setString(2,sUid.split("\\.")[1]);
			rs = ps.executeQuery();
			...
			if(recsUpdated > 0 && bUpdateTransaction){
				...
				if(serverId > -1 && tranId > -1){
					...
					if(updatedRecs==0){
	                    ...
	                    sSql = "INSERT INTO Items(itemId,type,"+MedwanQuery.getInstance().getConfigString("valueColumn","value")+","+
	                    		MedwanQuery.getInstance().getConfigString("dateColumn","date")+","+
