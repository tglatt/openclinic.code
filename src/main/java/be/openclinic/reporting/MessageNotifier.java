...
public class MessageNotifier {
	...
	public void sendSpooledMessages(){
		...
		try{
			PreparedStatement ps = conn.prepareStatement("SELECT * from OC_MESSAGES where OC_MESSAGE_TRANSPORT<>'none' and OC_MESSAGE_SENTDATETIME IS NULL and OC_MESSAGE_CREATEDATETIME>? AND OC_MESSAGE_SENDAFTER<?");
			...
			while(rs.next()){
				...
				if(transport.equalsIgnoreCase("sms")){
					...
					else if(messageType.equalsIgnoreCase("erpnext")) {
						try {
							if(ERPNext.sendMessage(messageId,data)) {
								setSpoolMessageSent(messageId,transport);
							}
						}
						catch(Exception f) {
