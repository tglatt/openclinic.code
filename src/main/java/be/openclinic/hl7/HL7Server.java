...
public class HL7Server {
	...
	public String start(int port) {
		try {
			...
			HL7Context context = new HL7Context();
			...
			server.startAndWait();
			String[] clients = SH.cs("s5.hl7.clients", "").split(";");
			if(clients[0].split(":").length>1) {
				for(int n=0;n<clients.length;n++) {
					HL7Client client = new HL7Client();
					client.start(clients[n].split(":")[0], Integer.parseInt(clients[n].split(":")[1]));
				}
			}
	...
	public void stop() {
		if(server.isRunning()) {
			server.stop();
			SH.syslog("S5 server stopped");
		}
		SH.syslog("Starting S5 server");
		start(port);
	}
    ...
    public static class HL7ConnectionListener implements ConnectionListener {
        public void connectionReceived(Connection theC) {
        	SH.syslog("New connection received from: " + theC.getRemoteAddress().toString()+":"+theC.getRemotePort());
        }
        public void connectionDiscarded(Connection theC) {
        	SH.syslog("Lost connection from: " + theC.getRemoteAddress().toString());
        }
    }
    ...
    public static class HL7ExceptionHandler implements ReceivingApplicationExceptionHandler {
		...
		*/
		public String processException(String theIncomingMessage, Map<String, Object> theIncomingMetadata, String theOutgoingMessage, Exception theE) throws HL7Exception {
			...
			*/
			SH.syslog("ERRONEOUS Message: ");
			SH.syslog(theIncomingMessage.replaceAll("\r","\r\n"));
			SH.syslog("");
			SH.syslog("ERROR: "+theE.getMessage());
    ...
    public static void storeReceivedMessage(String messageType, Message message) {
		...
		try {
	    	...
	    	try {
	    		...
	    		ps = conn.prepareStatement("update OC_HL7IN set OC_HL7IN_ID=? where OC_HL7IN_ID=?");
	    		ps.setString(1, id+"_"+new java.util.Date().getTime());
	    		ps.setString(2,id);
	    		ps.execute();
	    		ps.close();
    ...
    public static void setReceivedMessageProcessed(Message message) {
		...
		try {
    		...
    		Parser p = context.getPipeParser();
    		String msg = p.encode(message);
    		ps = conn.prepareStatement("update OC_HL7IN set OC_HL7IN_PROCESSED=? where OC_HL7IN_PROCESSED is null && OC_HL7IN_ID = ?");
            Terser terser = new Terser(message);
            String id=terser.get("/.MSH-10");
    ...
    public static void sendTransactions() throws SQLException {
    	...
    	PreparedStatement ps;
		try {
			ps = conn.prepareStatement("select * from OC_HL7OUT where OC_HL7OUT_SENT IS NULL and OC_HL7OUT_ERROR IS NULL and exists (select * from RequestedLabanalyses where serverid=OC_HL7OUT_SERVERID and transactionid=OC_HL7OUT_TRANSACTIONID and worklisteddatetime is null and samplereceptiondatetime is not null and (resultvalue is null or resultvalue=''))");
	    	//ps.setTimestamp(1, SH.getSQLTimestamp(SH.dateAdd(new java.util.Date(), -SH.getTimeDay()*7)));
			ResultSet rs =ps.executeQuery();
	    	while(rs.next()) {
					...
					for(int n=0;n<v_analyses.size();n++) {
						...
						if(analysercode.length()>0) {
							...
							if(version==1) {
								...
								orc.getOrderControl().setValue("AT");
							...
							if(userid.length()>0) {
								...
								String last = HL7Server.getUserLastname(userid);
								String first = HL7Server.getUserLastname(userid);
								if(last.length()>20) {
								...
								orc.getOrderingProvider(0).getFamilyName().getSurname().setValue(last);
								orc.getOrderingProvider(0).getGivenName().setValue(first);
							}
