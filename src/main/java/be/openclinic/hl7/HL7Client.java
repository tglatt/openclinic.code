...
import be.openclinic.hl7.HL7Server.HL7ConnectionListener;
import be.openclinic.hl7.HL7Server.HL7ExceptionHandler;
...
public class HL7Client implements ReceivingApplication {
	...
	public String start(String ipaddress, int port) {
		try {
			
			boolean useTls = SH.ci("s5.hl7.ssl", 0)==1; // Should we use TLS/SSL?
			HapiContext context = new DefaultHapiContext();
	        CanonicalModelClassFactory mcf = new CanonicalModelClassFactory("2.5.1");
	        context.setModelClassFactory(mcf);			
			client = context.newClient(ipaddress, port, useTls);
			Initiator initiator = client.getInitiator();
			if(client.isOpen()){
				return "Connection open, listening for incoming messages";
			}
			else {
				SH.syslog("Failed: not connected");
				System.exit(0);
			}
		}
		catch(Exception e) {
			SH.syslog("Failed: "+e.getMessage());
			System.exit(0);
		}
		return "Error";
	}
 	...
 	public Message processMessage(Message message, Map<String, Object> theMetadata) throws ReceivingApplicationException, HL7Exception {
        ...
        if(messageType.equalsIgnoreCase("OML") && messageSubType.equalsIgnoreCase("O21")) {
        	...
        	while(iPatientResults.hasNext()) {
        		...
        		SH.syslog("specimen ID: "+specimenid);
            	...
            	while(iOrderObservations.hasNext()) {
            		...
            		if(observations.size()>0) {
	                	...
	                	while(iObservations.hasNext()) {
	            			...
	            			String labcode="";
	            			try {
								if(HL7Server.getConfigString("s5.hl7.codemapping",HL7Server.getConfigString("labanalysercodemapping","loinc")).equalsIgnoreCase("loinc")) {
