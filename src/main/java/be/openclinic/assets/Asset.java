...
public class Asset extends OC_Object {
    ...
    public static void updateAsset(Element eAsset) throws Exception{
		...
		if(!rs.next()){
			...
			asset.store(eAsset.element("updateid")==null?"":eAsset.element("updateid").getText(),updatetime);
    ...
    public static boolean storeXml(StringBuffer xml) throws DocumentException{
    	boolean bSuccess = true;
    	//For each of these assets, maintenanceplans and maintenanceoperations, 
    ...
    public boolean store(String userUid, java.util.Date updateTime){
        boolean errorOccurred = false;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String sSql;
        ...
        try{            
            if(getUid().equals("-1")){                  
                ...
                // update-info
                ps.setTimestamp(psIdx++,new Timestamp(updateTime.getTime()));
                ps.setString(psIdx++,userUid);
                ps.setString(psIdx++,gmdncode);
                ps.setString(psIdx++,nomenclature);
                ps.setString(psIdx++,comment1);
   ...
   //===============================================================
    //check if the serial number given already exists
    public static boolean countSerialNumber(
    		Integer objectid,
    		String serialnumber
    		){
    	
    	if(SH.c(serialnumber).length()==0) {
    		return false;
    	}
    	
		boolean serialnumberexist = false;
		Integer foundserialnumber = 0;
		
    	Connection conn = MedwanQuery.getInstance().getOpenclinicConnection();
    	try{
    		PreparedStatement ps = conn.prepareStatement(
    				" select count(*) as total from oc_assets "
    				+ " where oc_asset_objectid <> ? "
    				+ " and OC_ASSET_SERIAL = ?");
    		...
    		ResultSet rs = ps.executeQuery();
    		if(rs.next()){
    			foundserialnumber = rs.getInt("total");
    		}
    		if(foundserialnumber > 0) {
