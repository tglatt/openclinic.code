...
public class RequestedLabAnalysis {
   ...
   //--- RESULT USER ID --------------------------------------------------------------------------
    ...
    public static RequestedLabAnalysis getByLOINC(int serverId, int transactionId, String analysisCode){
        // create LabRequest and initialize
        RequestedLabAnalysis labAnalysis = new RequestedLabAnalysis();
        labAnalysis.setServerId(serverId+"");
        labAnalysis.setTransactionId(transactionId+"");
        ...
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try{
            String sSelect = "SELECT a.*,b.userId,b.updateTime FROM RequestedLabAnalyses a,Transactions b, Labanalysis c where a.serverid=b.serverid and a.transactionid=b.transactionId "+
                             "  AND a.serverid = ?"+
                             "  AND a.transactionid = ?"+
                             "  AND a.analysiscode = c.labcode"+
                             "  AND c.medidoccode = ?";
                             ;
            ps = oc_conn.prepareStatement(sSelect);
            ps.setInt(1,serverId);
    ...
    public static Vector find(String serverId, String transactionId, String patientId, String analysisCode,
        ...
        try{
            ...
            while(rs.next()){
                    ...
                    labAnalysis.technicalvalidation   = rs.getInt("technicalvalidator");
                    labAnalysis.technicalvalidationdatetime   = rs.getTimestamp("technicalvalidationdatetime");
                    labAnalysis.samplereceptiondatetime   = rs.getTimestamp("samplereceptiondatetime");
                    labAnalysis.sampletakendatetime   = rs.getTimestamp("sampletakendatetime");
                    labAnalysis.sampler   = rs.getInt("sampler");
                    labAnalysis.finalvalidation   = rs.getInt("finalvalidator");
                    labAnalysis.finalvalidationdatetime   = rs.getTimestamp("finalvalidationdatetime");
                    labAnalysis.worklisteddatetime   = rs.getTimestamp("worklisteddatetime");
                    labAnalysis.resultProvisional   = ScreenHelper.checkString(rs.getString("resultprovisional"));
