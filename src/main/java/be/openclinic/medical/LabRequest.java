    ...
    public static LabRequest getUnsampledRequest(int serverid,String transactionid,String language){
        ...
        try{
            ...
            while (rs.next()){
                ...
                Encounter encounter = Encounter.getActiveEncounterOnDate(SH.getSQLTimestamp(labRequest.requestdate),labRequest.personid+"");
                if(encounter!=null){
    ...
    public LabRequest(int serverid,int transactionid){
        ...
        try{
            ...
            while (rs.next()){
                ...
                if(language==null || language.trim().length()==0 || !"*fr*nl*pt*es*".contains(language)){
                    language="fr";
                }
                //For every transaction ID, let's create a Labrequest
                ...
                Encounter encounter = Encounter.getActiveEncounterOnDate(rs.getTimestamp("creationdate"),rs.getInt("patientid")+"");
                if(encounter!=null){
                    if(encounter.getService()!=null){
    ...
    public static Vector findOpenRequests(String worklistAnalyses,String language){
        ...
        try{
            ...
            while (rs.next()){
                ...
                labRequest.setUserid(rs.getInt("userid"));
                labRequest.setPatientdateofbirth(rs.getDate("dateofbirth"));
                Encounter encounter = Encounter.getActiveEncounterOnDate(new Timestamp(labRequest.getRequestdate().getTime()), labRequest.getPersonid()+"");;
                if(encounter!=null){
                    if(encounter.getService()!=null){
                        labRequest.setServicename(encounter.getService().getLabel(language));
                    }
