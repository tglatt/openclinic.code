...
import be.mxs.common.util.io.MessageReader.Transaction;
...
public class ClinicalAssistant {
	...
	public static boolean isFever(TransactionVO transaction, String itemTypeId) {
		String val = transaction.getItemValue(itemTypeId);
		try {
			if(val.length()>0 && Double.parseDouble(val)>=38) {
				return true;
			}
		}
		catch(Exception e) {
			SH.syslog(val+" is not a valid temperature");
		}
		return false;
	}
	
	public static boolean hasMissedMalariaSignsInEncounter(String encounterUid) {
		boolean bMissed=false;
		Encounter encounter = Encounter.get(encounterUid);
		LabAnalysis analysis = LabAnalysis.getLabAnalysisByMedidocCode("70569-9");
		if(analysis!=null) {
			if(RequestedLabAnalysis.find("", "", encounter.getPatientUID(), analysis.getLabcode(), "", "", "", "", "", "", "", "", SH.formatDate(encounter.getBegin()), SH.formatDate(SH.getTomorrow()), "", "", false, "").size()>0) {
				return false;
			}
		}
		analysis = LabAnalysis.getLabAnalysisByMedidocCode("32700-7");
		if(analysis!=null) {
			if(RequestedLabAnalysis.find("", "", encounter.getPatientUID(), analysis.getLabcode(), "", "", "", "", "", "", "", "", SH.formatDate(encounter.getBegin()), SH.formatDate(SH.getTomorrow()), "", "", false, "").size()>0) {
				return false;
			}
		}
		Vector<TransactionVO> transactions = MedwanQuery.getInstance().getTransactionsByEncounter(Integer.parseInt(encounter.getPatientUID()), encounterUid);
		for(int n=0;n<transactions.size();n++) {
			SH.syslog(n);
			TransactionVO transaction = transactions.elementAt(n);
			if(transaction.getTransactionType().contains("GF_MALARIA")) {
				return false;
			}
		...
		for(int n=0;n<transactions.size();n++) {
			TransactionVO transaction = transactions.elementAt(n);
			if(isFever(transaction,"be.mxs.common.model.vo.healthrecord.IConstants.[GENERAL.ANAMNESE]ITEM_TYPE_TEMPERATURE")) return true;
			if(isFever(transaction,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_BIOMETRY_TEMPERATURE")) return true;
			if(isFever(transaction,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNRKR_BIOMETRY_TEMPERATURE")) return true;
			if(isFever(transaction,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_MALARIA_TEMPERATURE")) return true;
			if(isFever(transaction,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_TEMPERATURE")) return true;
			if(isFever(transaction,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CPN_TEMPERATURE")) return true;
			if(isFever(transaction,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CPON_TEMPERATURE")) return true;
			if(isFever(transaction,"be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_ADMISSION_FORM_EXAMINATION_TEMPERATURE")) return true;
		}
		return bMissed;
	}
	
	public static boolean hasMissedMalariaSignsInEncounter(Encounter encounter) {
		return hasMissedMalariaSignsInEncounter(encounter.getUid());
	}
