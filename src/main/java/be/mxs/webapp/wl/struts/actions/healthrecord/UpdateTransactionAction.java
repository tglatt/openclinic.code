...
public class UpdateTransactionAction extends org.apache.struts.action.Action {
            ...
            while (iterator.hasNext()) {
                ...
                if (request.getParameter("be.mxs.healthrecord.updateTransaction.actionForwardKey").indexOf("be.mxs.healthrecord.transaction_id=currentTransaction")<=0){
                    ...
                    while (ICPCItems.hasNext()){
                        ...
                        if (item.getType().indexOf("ICPCCode")==-1 && item.getType().indexOf("ICD10Code")==-1 && item.getType().indexOf("ICD11Code") == -1 && item.getType().indexOf("DSM4Code")==-1){
                            newItems.add(item);
                        }
                    }
                    ...
                    Hashtable ICD11Codes = RequestParameterParser.getInstance().parseRequestParameters(request, "ICD11Code");
                    enumeration = ICD11Codes.keys();
                    while (enumeration.hasMoreElements()) {
                    	code = (String)enumeration.nextElement();
                        if (code.indexOf("ICD11Code") == 0 || code.indexOf("GravityICD11Code") == 0 || code.indexOf("CertaintyICD11Code") == 0 || code.indexOf("POAICD11Code") == 0 || code.indexOf("NCICD11Code") == 0 || code.indexOf("ServiceICD11Code") == 0 || code.indexOf("FlagsICD11Code") == 0) {
                            oldTransaction.getItems().add(new ItemVO(new Integer(IdentifierFactory.getInstance().getTemporaryNewIdentifier()), code, (String)ICD11Codes.get(code), new Date(), itemContextVO)); 
                        }
                    }
    ...
    public static void saveDiagnosesToTable(Hashtable ICPCCodes,Hashtable ICD10Codes, Hashtable ICD11Codes,Hashtable DSM4Codes,java.util.Date updatetime, String sTransactionUID,SessionContainerWO sessionContainerWO,Encounter encounter){
        ...
        while (enumeration.hasMoreElements()) {
        	code = (String)enumeration.nextElement();
            if (code.indexOf("ICD11Code") == 0) {
            	String gravity = SH.c((String)ICD11Codes.get("Gravity" + code), "500");
            	String certainty = SH.c((String)ICD11Codes.get("Certainty" + code), "500");
            	Diagnosis.saveTransactionDiagnosisWithServiceAndFlags(code, 
            	ScreenHelper.checkString((String)ICD11Codes.get(code)), 
            	gravity, 
            	certainty, 
            	(sessionContainerWO.getPersonVO()).personId.toString(), 
            	"ICD11Code", 
            	...
            	sTransactionUID, 
            	sessionContainerWO.getUserVO().getUserId().intValue(), 
            	(ScreenHelper.checkString((String)ICD11Codes.get("User" + code)).length() > 0) ? Integer.parseInt(ScreenHelper.checkString((String)ICD11Codes.get("User" + code))) : sessionContainerWO.getUserVO().getUserId().intValue(), 
            	encounter, 
            	ScreenHelper.checkString((String)ICD11Codes.get("POA" + code)), 
            	ScreenHelper.checkString((String)ICD11Codes.get("NC" + code)), 
            	ScreenHelper.checkString((String)ICD11Codes.get("Service" + code)), 
            	ScreenHelper.checkString((String)ICD11Codes.get("Flags" + code)));
        	} 
        }
    }
}