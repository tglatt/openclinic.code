...
public class PDFSummaryInvoiceGenerator extends PDFInvoiceGenerator {
        	...
        	Vector debets = invoice.getAllDebets();
        	 ...
        	 for(int n=0;n<debets.size();n++){
        		...
        		if(debet!=null & debet.getServiceUid()!=null){
        			...
        			servicename = debet.getService().getLabel(sPrintLanguage);
        		}
        		else {
	    	...
	    	while(iEncounters.hasNext()){
	    		...
	    		if(encounter!=null){
	    			...
	    			if(encounter.getType()!=null && 
	    					encounter.getType().equalsIgnoreCase("admission")){
	    				if(encounter.getService()!=null){
		    				cell= createValueCell(getTran("web","service")+":",15);
			    			cell.setBorder(PdfPCell.NO_BORDER);
			    			table.addCell(cell);
			    			cell= createValueCell(getServicesAsStringBySignature(debets
			            			,sPrintLanguage),85);
			    			cell.setBorder(PdfPCell.NO_BORDER);
			    			table.addCell(cell);
		    			}
	    				...
	    				if(encounter.getService()!=null){
			    			...
			    			cell= createValueCell(
			    					getServicesAsStringBySignature(
			            			debets
			            			,sPrintLanguage),30);
			    			cell.setBorder(PdfPCell.NO_BORDER);
			    			table.addCell(cell);
	    			...
	    			cell = createGrayCell(getServicesAsStringBySignature(debets
	            			,sPrintLanguage), 1);
	    			wrappertable.addCell(cell);
    ...
    public static Integer CountPointers(String pinv) throws SQLException{
		...
		PreparedStatement ps = conn.prepareStatement("SELECT count(*) FROM  oc_pointers WHERE oc_pointer_key = ? ");
		ps.setString(1, "INV.SVC."+pinv);
    ...
    public String getServicesAsStringBySignature(
    		Vector debets, 
    		String language){
  	   ...
  	   if(debets.size() > 0 ) {
  		  ...
  		  Debet debet = (Debet)debets.elementAt(0);
  		  String patientInvoiceUid = debet.getPatientInvoice().getUid();
  		 
  		  try {
			if(CountPointers(patientInvoiceUid) > 0) {
			  
			  String pointer = Pointer.getPointer("INV.SVC."+patientInvoiceUid);
					  
			  if(pointer.length() > 0){

				 ptr = Service.getService(pointer).getLabel(language);
