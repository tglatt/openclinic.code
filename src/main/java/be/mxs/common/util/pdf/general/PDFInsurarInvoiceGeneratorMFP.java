                	...
                	if(invoiceUid.contains("*")){
	                	...
	                	if(iv!=null){
	                		if(iv.getPatient()!=null){
	                			...
	                			beneficiary=iv.getPatient().lastname.toUpperCase()+", "+iv.getPatient().firstname+" ("+ScreenHelper.getTranNoLink("insurance.status", status, sPrintLanguage)+")";
	                			if(ScreenHelper.checkString(iv.getAcceptationUid()).length()>0) {
	                				validated+=ScreenHelper.getTranNoLink("web", "validatedby", sPrintLanguage)+": "+User.getFullUserName(iv.getAcceptationUid());
	                			}
	                			if(ScreenHelper.checkString(iv.getComment()).length()>0) {
	                				if(validated.length()>0) {
	                					validated+=", ";
	                				}
                					validated+=iv.getComment();
	                			}
	                			if(validated.length()>0) {
	                				beneficiary+="\n["+validated+"]";
	                			}
