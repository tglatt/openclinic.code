    ...
    private void addPatientDataVisit(PatientInvoice invoice,Service service,int subinvoices,Vector activedebets,Insurance insurance){
        ...
        try{
        	...
        	if(person!=null){
                ...
                String s=Pointer.getPointer("INV.SVC."+invoice.getUid());
            	for(int n=0;n<activedebets.size();n++){
            		Debet debet = (Debet)activedebets.elementAt(n);
            		if(debet!=null && debet.getServiceUid()!=null){
            			if(s.length()>0 || debet.getServiceUid().equalsIgnoreCase(service.code)){
            				debets.add(debet);
            				if(encounter==null){
            		...
            		else if(s.length()>0 || debet.getEncounter().getServiceUID().equalsIgnoreCase(service.code)){
        				debets.add(debet);
        				if(encounter==null){
        					encounter=debet.getEncounter();
        				}
    ...
    private void addPatientDataAdmission(PatientInvoice invoice,Service service,int subinvoice,Vector activedebets,Insurance insurance){
        ...
        try{
        	...
        	if(person!=null){
            	...
            	for(int n=0;n<activedebets.size();n++){
            		Debet debet = (Debet)activedebets.elementAt(n);
            		if(debet!=null && (s.length()>0 || debet.getServiceUid()!=null)){
            			if(s.length()>0 || debet.getServiceUid().equalsIgnoreCase(service.code)){
            				debets.add(debet);
            				if(encounter==null){
            					encounter=debet.getEncounter();
            		...
            		else if(debet!=null && (s.length()>0 || debet.getEncounter().getServiceUID().equalsIgnoreCase(service.code))){
        				debets.add(debet);
        				if(encounter==null){
        					encounter=debet.getEncounter();
        				}
