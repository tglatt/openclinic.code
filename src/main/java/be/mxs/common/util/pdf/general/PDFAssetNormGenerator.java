...
public class PDFAssetNormGenerator extends PDFOfficialBasic {
    ...
    private void printNorms() throws DocumentException{
		...
		while(services.hasNext()){
			...
			cell = createHeaderCell(ScreenHelper.getTranNoLink("asset", "norm", sPrintLanguage), 50);
			...
			table.addCell(cell);
			cell = createHeaderCell(ScreenHelper.getTranNoLink("asset", "needed", sPrintLanguage), 10);
			table.addCell(cell);
			...
			while(iservicenorms.hasNext()){
				...
				if(norms.length()==0 || (norms+";").contains(sn_nomenclature)){
					...
					if(sn_result.split(";").length>1){
						...
						table.addCell(cell);
						cell = createValueCell(ScreenHelper.getTranNoLink("admin.nomenclature.asset", sn_nomenclature, sPrintLanguage), 35);
						table.addCell(cell);
						cell = createValueCell(new Double(sn_minimumquantity).intValue()+(sn_nomenclature.toUpperCase().startsWith("I")?" m2":""), 10);
						cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
						table.addCell(cell);
						cell = createValueCell(sn_foundquantity==0?"-":new Double(sn_foundquantity).intValue()+(sn_nomenclature.toUpperCase().startsWith("I")?" m2":""), 10);
						cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
						table.addCell(cell);
						cell = createValueCell(sn_nonfunctional==0?"-":new Double(sn_nonfunctional).intValue()+(sn_nomenclature.toUpperCase().startsWith("I")?" m2":""), 10);
						cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
						table.addCell(cell);
						cell = createValueCell(sn_minimumquantity<=sn_foundquantity?"-":new Double(sn_minimumquantity-sn_foundquantity).intValue()+(sn_nomenclature.toUpperCase().startsWith("I")?" m2":""), 10);
						cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
						table.addCell(cell);
						if(sn_foundquantity<sn_minimumquantity){
							cell = createWhiteBoldValueCell(sn_foundquantity>=sn_minimumquantity?"100%":SH.formatDouble(sn_compliance*100)+"%", 10);
							cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
					        cell.setBackgroundColor(new BaseColor(230,50,50)); // light red
						}
						else{
							cell = createBoldValueCell(sn_foundquantity>=sn_minimumquantity?"100%":SH.formatDouble(sn_compliance*100)+"%", 10);
							cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
			...
			table.addCell(cell);
			cell = createValueCell(ScreenHelper.getTranNoLink("web", "conformityscore", sPrintLanguage)+": ", 40);
			cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
			table.addCell(cell);
			cell = createBoldValueCell(numberofnorms==0?"-":SH.formatDouble(compliance*100/numberofnorms)+"%", 10);
			cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
			table.addCell(cell);
