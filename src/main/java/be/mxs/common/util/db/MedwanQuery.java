...
import be.openclinic.malariyapi.MalariyaPiClient;
import be.openclinic.mpi.MPICheck;
...
public class MedwanQuery {
    ...
    private static MPICheck mpicheck;
    private static MalariyaPiClient malariaPiClient;
    ...
    public MedwanQuery(String uri, boolean bStartThreads){
        ...
        if(bStartThreads) {
	        ...
	        if(mpicheck==null && SH.ci("enableMPI", 0)==1){
	        	mpicheck=new MPICheck();
	        	mpicheck.start();
	        }
	        if(malariaPiClient==null && SH.ci("enableMalariyaPi", 0)==1){
	        	malariaPiClient=new MalariyaPiClient(uri);
	        	malariaPiClient.start();
	        }
    ...
    // ICD11 
    public Vector getAlternativeDiagnosisCodesICD11(String sCodeType, String sCode){
        Vector result = new Vector();
        try{
            Connection oc_conn=getOpenclinicConnection();
           // String sQuery = "select distinct "+(sCodeType.equalsIgnoreCase("icpc") ? "icd10" : "icpc")+" as code from Classifications where "+sCodeType.toLowerCase()+" like '"+sCode+"%'";
            String sQuery = "select distinct icd10 as code from classification_icd11 where "+sCodeType.toLowerCase()+" like '"+sCode+"%'";
            PreparedStatement ps = oc_conn.prepareStatement(sQuery);
            ResultSet rs = ps.executeQuery();
            while (rs.next()){
                result.add(rs.getString("code"));
    ...
    public String getDiagnosisLabelICD11(String sCodeType, String sCode, String sLanguage){
        String result = "";
        try{
            String sQuery = "select * from ICPC2 where code=?";
            if(sCodeType.equalsIgnoreCase("icd11")){
                sQuery = "select * from ICD11 where code=?";
            }
    ...
    public static String translate(String term,String sourcelanguage,String targetlanguage){
    	String s = GoogleTranslate.translate(MedwanQuery.getInstance().getConfigString("googleTranslateKey","-"), sourcelanguage, targetlanguage, term);
     	return s;
    }
    ...
    public String getConfigString(String key){
    	if(config==null || config.size()<500) {
    		reloadConfigValues();
    	}
    ...
    public boolean storeActivity(int personId, int userId, java.util.Date activityDateTime, String activityCode,
        ...
        try{
            ...
            if(!rs.next()){
            	rs.close();
            	ps.close();
                ps = OccupdbConnection.prepareStatement("insert exportActivities(personId,userId,activityDateTime,activityCode,activityId,medicalCenter,MD,para,ts,provider,"+getConfigString("valueColumn","value")+") values(?,?,?,?,?,?,?,?,?,?,?)");
                ps.setInt(1, personId);
                ps.setInt(2, userId);
    ...
    public boolean updateActivity(int personId, int userId, java.util.Date activityDateTime, String activityCode, String activityId, String medicalCenter, String md, String para, String provider, String value){
        ...
        try{
            ...
            String query = "UPDATE exportActivities SET personId=?,userId=?,activityDateTime=?,activityCode=?," +
                    " medicalCenter=?,MD=?,para=?,ts=?,provider=?,"+getConfigString("valueColumn","value")+"=?" +
                    " WHERE activityId=?";
    ...
    public void exportTransaction(int personid, TransactionVO transactionVO, String userid, boolean nightShift, String activeMedicalCenter, String activeMD, String activePara){
        try{
            ...
            //We zijn vooreerst geÎnteresseerd in de formulieren voor gezondheidsbeoordeling
    ...
    public void reloadConfigValues(){
        ...
        try{
            ...
            // set Debug
            Debug.enabled = getConfigString("Debug").equalsIgnoreCase("on");
            Debug.println("Loaded "+config.size()+" config values");
        }
        catch(Exception e){
        	Debug.println(e.getMessage());
        }
    ...
    public Vector findICD10Codes(String keywords, String language){
        ...
        int mandatorykeys=0;
        for (int n = 0; n < keys.length; n++){
        	if(keys[n].endsWith(".0")) {
        		keys[n]=keys[n].substring(n,keys[n].length()-2);
        	}
            if(keys[n].length() > 2){
            	if(keys[n].length() >2){
            		mandatorykeys++;
    ...
    //--- FIND ICD11 CODES ------------------------------------------------------------------------
    public Vector findICD11Codes(String keywords, String language){
        Vector codes = new Vector();
        Hashtable counters = new Hashtable();
        String label = "labelen";
        if(language.equalsIgnoreCase("FR")){
        ...
        for (int n = 0; n < keys.length; n++){
            if(keys[n].length() > 2){
                ...
                sQuery = " select distinct code as code from ICD11 where "+label+" like '%"+ScreenHelper.normalizeSpecialCharacters(keys[n].toLowerCase()).toUpperCase()+"%' or code like '%"+ScreenHelper.normalizeSpecialCharacters(keys[n].toLowerCase()).toUpperCase()+"%'";
//                sQuery += " union select distinct b.icd10 as code from Concepts b,Keywords c where c.language='"+language+"' and b.icd10 is not null and b.concept=c.concept and c.keyword like '"+ScreenHelper.normalizeSpecialCharacters(keys[n].toLowerCase()).toUpperCase()+"%'";
                sQuery += " order by code";
                PreparedStatement ps;
                Connection OccupdbConnection;
        ...
        for (int n = 0; n < codes.size(); n++){
            String code = (String) codes.elementAt(n);
            if(((Integer) counters.get(code)).intValue() >= mandatorykeys){
                ICPCCode icpc = new ICPCCode(code, label, "icd11");
                if(icpc.label.length() > 0 && existing.get(icpc.code) == null){
                	existing.put(icpc.code, "1");
                    returncodes.add(icpc);
                }
    ...
    public String getCodeTran(String code, String language){
        ...
        try{
            ...
            else if(code.toLowerCase().indexOf("icd11code") == 0){
                ps = OccupdbConnection.prepareStatement("select "+label+" from ICD11 where code=?");
                ps.setString(1, code.toLowerCase().trim().replaceAll("icd11code", "").toUpperCase());
                ResultSet rs = ps.executeQuery();
                if(rs.next()){
                    result = rs.getString(label);
                }
    ...
    public HealthRecordVO loadHealthRecord(PersonVO personVO, String sortOrder, SessionContainerWO sessionContainerWO){
        ...
        try{
            ...
            for (Occuprs = Occupstatement.executeQuery(); Occuprs.next();){
                ...
                if(transactionVO.getTransactionType().equalsIgnoreCase("be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_IMPORT")){
                    ItemVO documentType = getItem(serverId, transactionId, "be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_TITLE");
    ...
    public void deleteTransaction(String transactionId, String serverId){
        ...
        try{
            ...
            Occupstatement.execute();
            Occupstatement.close();
            Occupstatement = OccupdbConnection.prepareStatement("insert into ItemsHistory(itemId,type,"+getConfigString("valueColumn","value")+","+getConfigString("dateColumn","date")+",transactionId,serverid,versionserverid,version) select itemId,type,"+getConfigString("valueColumn","value")+","+getConfigString("dateColumn","date")+",transactionId,serverid,versionserverid,version from Items where serverid=? and transactionId=?");
            Occupstatement.setInt(1, Integer.parseInt(serverId));
            Occupstatement.setInt(2, Integer.parseInt(transactionId));
            Occupstatement.execute();
            Occupstatement.close();
  ...
  //--- GET AGE ---------------------------------------------------------------------------------
        ...
        try{
        	...
        	for(int n=0;n<types.length;n++){
	        	...
	        	if(values.length>n && values[n].length()>0){
	        		...
	        		else if(values[n].startsWith("{like}") || values[n].startsWith("{notlike}")){
	        	...
	        	if(values.length>n && values[n].length()>0){
	        		ps.setString(4, values[n].replaceAll("\\{lessthan\\}","").replaceAll("\\{morethan\\}","").replaceAll("\\{morethanorequal\\}","").replaceAll("\\{like\\}","").replaceAll("\\{notlike\\}","").replaceAll("\\{sem\\}",";"));
	        	}
	        	ResultSet rs = ps.executeQuery();
	        	if(rs.next()){
	        		bHasItem=values[n].startsWith("{notlike}")?false:true;
	        	}
	        	else{
	        		bHasItem=values[n].startsWith("{notlike}")?true:false;
	        	}
	        	rs.close();
	        	ps.close();
	        	if(!bHasItem){
    ...
    public boolean hasItemPeriod(String encounterUid,String itemType,String itemValue,java.util.Date begin,java.util.Date end){
        ...
        try{
        	...
        	for(int n=0;n<types.length;n++){
	        	...
	        	if(values.length>n && values[n].length()>0){
	        		ps.setString(6, values[n].replaceAll("\\{lessthan\\}","").replaceAll("\\{morethan\\}","").replaceAll("\\{morethanorequal\\}","").replaceAll("\\{like\\}","").replaceAll("\\{notlike\\}","").replaceAll("\\{sem\\}",";"));
	        	}
	        	ResultSet rs = ps.executeQuery();
	        	if(rs.next()){
	        		bHasItem=values[n].startsWith("{notlike}")?false:true;
        	...
        	for(int i=0;i<transactionTypes.length;i++){
	            for(int n=0;n<types.length;n++){
		        	...
		        	if(values.length>n && values[n].length()>0){
		        		...
		        		else if(values[n].startsWith("{notlike}")){
		        			sSql+=" and i.value not like '%'||?||'%'";
		        		}
		        		else{
		        			sSql+=" and i.value=?";
		        		}
    ...
    public boolean hasItem(String itemType,String itemValue, int transactionid){
        ...
        try{
            ...
            for(int n=0;n<types.length;n++){
	        	...
	        	if(values.length>n && values[n].length()>0){
	        		ps.setString(3, values[n].replaceAll("\\{lessthan\\}","").replaceAll("\\{morethan\\}","").replaceAll("\\{morethanorequal\\}","").replaceAll("\\{like\\}","").replaceAll("\\{notlike\\}","").replaceAll("\\{sem\\}",";").replaceAll("\\{semicolon\\}",";"));
	        	}
	        	ResultSet rs = ps.executeQuery();
	        	if(rs.next()){
	        		bHasItem=values[n].startsWith("{notlike}")?false:true;
        ...
        try{
        	...
        	for(int i=0;i<transactionTypes.length;i++){
	            for(int n=0;n<types.length;n++){
		        	...
		        	ps.setString(3, types[n]);
		        	if(theValue.length()>0){
		        		ps.setString(4, theValue.replaceAll("\\{lessthan\\}","").replaceAll("\\{morethan\\}","").replaceAll("\\{morethanorequal\\}","").replaceAll("\\{like\\}","").replaceAll("\\{notlike\\}","").replaceAll("\\{sem\\}",";"));
		        	}
		        	ResultSet rs = ps.executeQuery();
		        	if(rs.next()){
		        		bHasItem=values[n].startsWith("{notlike}")?false:true;
    ...
    public ItemVO getLastItemVO(String healthRecordId, String transactionType, String itemType){
        String sSelect;
        ItemVO itemVO = null;
        Connection OccupdbConnection;
        PreparedStatement Occupstatement;
        try{
            OccupdbConnection = getOpenclinicConnection();
            if(itemType.indexOf("%") > -1){
                sSelect = "SELECT * from Transactions a,Items b where a.healthRecordId=? and a.transactionId=b.transactionId and a.serverid=b.serverid and b.type like ? and a.transactionType=? order by a.updateTime DESC,itemId DESC";
            } 
            else{
                sSelect = "SELECT * from Transactions a,Items b where a.healthRecordId=? and a.transactionId=b.transactionId and a.serverid=b.serverid and b.type = ? and a.transactionType=? order by a.updateTime DESC,itemId DESC";
            }
            Occupstatement = OccupdbConnection.prepareStatement(sSelect);
            Occupstatement.setInt(1, Integer.parseInt(healthRecordId));
            Occupstatement.setString(2, itemType);
    ...
    public ItemVO getLastItemVO(int personId, String transactionType, String itemType){
        ItemVO itemVO = null;
        Connection OccupdbConnection;
        try{
            OccupdbConnection = getOpenclinicConnection();
            PreparedStatement Occupstatement = OccupdbConnection.prepareStatement("SELECT * from Transactions a,Items b, Healthrecord c where a.healthRecordId=c.healthRecordId and c.personId=? and a.transactionId=b.transactionId and a.serverid=b.serverid and b.type = ? and a.transactionType=? order by a.creationDate DESC,itemId DESC");
            Occupstatement.setInt(1, personId);
            Occupstatement.setString(2, itemType);
            Occupstatement.setString(3, transactionType);
            ResultSet Occuprs = Occupstatement.executeQuery();
    ...
    //--- LOAD TRANSACTION ------------------------------------------------------------------------
    public TransactionVO loadTransactionNoCacheNoStore(int serverId, int transactionId){
        TransactionVO transactionVO = null;
        Connection OccupdbConnection;
        try{
            OccupdbConnection = getOpenclinicConnection();
...
//                var = "";
    ...
    public TransactionVO updateTransaction(int personid, TransactionVO transactionVO){
        try{
            ...
            if(rs.next()){
                ...
                ps.execute();
                ps.close();
                sSelect = "insert into ItemsHistory(itemId,type,"+getConfigString("valueColumn","value")+","
                       +getConfigString("dateColumn","date")+",transactionId,serverid,versionserverid,version)" +
                        " select itemId,type,"+getConfigString("valueColumn","value")+","+getConfigString("dateColumn","date")+"," +
            ...
            for(Iterator iterator = transactionVO.getItems().iterator(); iterator.hasNext();){
                ...
                if(itemVO.getValue().trim().length() > 0){
                    ...
                    sSelect = "insert into Items(itemId,type,"+getConfigString("valueColumn","value")+","+
                              getConfigString("dateColumn","date")+",transactionId,serverid,version,versionserverid,valuehash)"+
                              " values(?,?,?,"+getConfigString("timeStamp")+",?,?,?,?,?)";
                    ps = oc_conn.prepareStatement(sSelect);
                    ...
                    ps.setString(2, itemVO.getType());
                    ps.setString(3, itemVO.getValue());
                    ps.setInt(4, transactionVO.getTransactionId().intValue());
                    ps.setInt(5, transactionVO.getServerId());
                    ps.setInt(6, transactionVO.getVersion());
                    ps.setInt(7, transactionVO.getVersionserverId());
                    ps.setInt(8, (itemVO.getType()+itemVO.getValue()).hashCode());
                    ps.execute();
                    ps.close();
                    
                    //Handle special items here
                    if(SH.ci("enableHemovigilance",0)==1) {
                    	if(transactionVO.getTransactionType().equalsIgnoreCase("be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_CNTS_BLOODREQUEST") &&
                    		...
                    		ps=oc_conn.prepareStatement("select * from OC_HEMOVIGILANCE where OC_HEMOVIGILANCE_TRANSACTIONUID=? and OC_HEMOVIGILANCE_POCKETNUMBER=?");
                    		ps.setString(1, transactionVO.getUid());
                    		ps.setString(2, transactionVO.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_BLOODPOCKET_GIFTNUMBER")+"."+itemVO.getValue());
                    		rs=ps.executeQuery();
                    		if(!rs.next()) {
                    			rs.close();
	                    		ps.close();
	                    		...
	                    		ps.setString(3, transactionVO.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_BLOODPOCKET_GIFTNUMBER")+"."+itemVO.getValue());
	                    		ps.execute();
	                    		ps.close();
                    		}
                    		else {
    ...
    public Vector getItemHistory(int healthRecordId, String itemType){
        ...
        try{
            ...
            PreparedStatement ps = OccupdbConnection.prepareStatement("select b.transactionid,b.itemId,b.type,a.updateTime,b."+getConfigString("valueColumn","value")+",b.serverid from Transactions a,Items b where a.healthRecordId=? and a.serverid=b.serverid and a.transactionId=b.transactionId and b.type in ("+types+") order by updateTime DESC,transactionid,type ");
            ps.setInt(1, healthRecordId);
            ResultSet rs = ps.executeQuery();
    ...
    public Vector<TransactionVO> getTransactionsBetween(int personId, java.util.Date begin, java.util.Date end){
        Vector<TransactionVO> transactionList = new Vector<TransactionVO>();
        Connection OccupdbConnection;
        String sSql = "SELECT a.* FROM Transactions a, Healthrecord b"+
                      " WHERE a.healthRecordId = b.healthRecordId"+
