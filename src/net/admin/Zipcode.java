...
public class Zipcode {
    ...
    public static Vector getProvinces(String table) {
        PreparedStatement ps = null;
        ResultSet rs = null;

        Vector vResults = new Vector();
        String sSelect = " SELECT distinct province FROM "+table+" ";
        ...
        try {
            ...
            while (rs.next()) {
                vResults.addElement(ScreenHelper.checkString(rs.getString("province")));
            }
            rs.close();
            ps.close();
        } catch (Exception e) {
    ...
    public static Vector getSectors(String sProvince, String table) {

        Vector vResults = new Vector();

        if (ScreenHelper.checkString(sProvince).length()>0){
            PreparedStatement ps = null;
            ResultSet rs = null;

            String sSelect = " SELECT distinct sector FROM "+table+" WHERE province = ?";

        	Connection ad_conn = MedwanQuery.getInstance().getAdminConnection();
            try {
                ps = ad_conn.prepareStatement(sSelect);
                ps.setString(1,sProvince);
                rs = ps.executeQuery();
    ...
    public static Vector getDistricts(String region, String sector, String table) {

        Vector vResults = new Vector();

        if (ScreenHelper.checkString(region).length()>0 && ScreenHelper.checkString(sector).length()>0){
            PreparedStatement ps = null;
            ResultSet rs = null;

            String sSelect = " SELECT distinct district FROM "+table+" WHERE sector = ? and province=?";
    ...
    public static Vector getCities(String region, String sector, String district, String table) {

        Vector vResults = new Vector();

        if (ScreenHelper.checkString(region).length()>0 && ScreenHelper.checkString(sector).length()>0 && ScreenHelper.checkString(district).length()>0){
            PreparedStatement ps = null;
            ResultSet rs = null;

            String sSelect = " SELECT distinct city FROM "+table+" WHERE sector = ? and province=? and district=?";
    ...
    public static String getZipcode2(String sRegion, String sSector, String sDistrict, String sCity,String table) {
        ...
        if (ScreenHelper.checkString(sDistrict).length()>0 && ScreenHelper.checkString(sCity).length()>0){
            ...
            String sSelect = " SELECT zipcode FROM "+table+" WHERE province = ? AND sector = ? and district=? and city=?";
            
        	Connection ad_conn = MedwanQuery.getInstance().getAdminConnection();
            try {
                ps = ad_conn.prepareStatement(sSelect);
                ps.setString(1,sRegion);
                ps.setString(2,sSector);
                ps.setString(3,sDistrict);
