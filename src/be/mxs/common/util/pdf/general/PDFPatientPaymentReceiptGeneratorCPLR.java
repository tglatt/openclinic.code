...
public class PDFPatientPaymentReceiptGeneratorCPLR extends PDFInvoiceGenerator {
    ...
    private void printCredit(PatientCredit credit){
    	try{
            ...
            cell = createTotalPriceCell(credit.getAmount(), 50-50*MedwanQuery.getInstance().getConfigInt("patientInvoiceReceiptCareDeliveryColumnWidthPercent",60)/100,credit.getDate(),credit.getCurrency(),new Double(7*scaleFactor).intValue());
            cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
            table.addCell(cell);
