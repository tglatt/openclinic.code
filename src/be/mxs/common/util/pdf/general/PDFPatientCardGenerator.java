...
import be.mxs.common.util.pdf.official.*;
...
public class PDFPatientCardGenerator extends PDFOfficialBasic {
    ...
    public ByteArrayOutputStream generatePDFDocumentBytes(final HttpServletRequest req, AdminPerson person) throws Exception {
		...
		try{
            ...
            if(SH.ci("enableMPI", 0)==1 && person.getExtendedValue("mpiid").length()>0) {
                docWriter.setPageEvent(new EndPagePatientIDCard());
            	printMPIPatientCard(person);
            }
            else {
    ...
    protected void printMPIPatientCard(AdminPerson person){
        try {
            ...
            if(SH.cs("countrycode", "be").equalsIgnoreCase("bi")) {
            	image = Miscelaneous.getImage("burundi.jpg",sProject);
            }
            else {
            	image = Miscelaneous.getImage(SH.cs("opencarenetmpiimage","opencarenet_small.png"),sProject);
            }
            ...
            image = PdfBarcode.getBarcode("0"+person.getExtendedValue("mpiid"),"", docWriter);            
            cell = new PdfPCell(image);
            cell.setBorder(PdfPCell.NO_BORDER);
            ...
            cell=createLabel(person.getExtendedValue("mpiid"),SH.ci("IPUTextSize", 14),3,Font.BOLD);
            cell.setBorder(PdfPCell.NO_BORDER);
            cell.setVerticalAlignment(PdfPCell.ALIGN_MIDDLE);
            cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
            cell.setColspan(3);
            ...
            cell=createLabel(MedwanQuery.getInstance().getLabel("web","mpicardhospitalref",user.person.language),6,4,Font.NORMAL);
