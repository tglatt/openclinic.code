...
public abstract class PDFInvoiceGenerator extends PDFBasic {
    ...
    protected String sCurrency = MedwanQuery.getInstance().getConfigParam("currency","EUR");
    ...
    protected PdfPCell createTotalPriceCell(double price, boolean indicateNegative, int colspan,java.util.Date date, String sPaymentCurrency){
    	return createTotalPriceCell(price, indicateNegative, colspan, date, sPaymentCurrency, 7);
    }
    ...
    protected PdfPCell createTotalPriceCell(double price, boolean indicateNegative, int colspan,java.util.Date date, String sPaymentCurrency, int fontSize){
        ...
        String sValue="";
    	if(SH.cs("preferAlternateCurrency","").length()>0 && !SH.cs("currency","").equalsIgnoreCase(sAlternateCurrency)) {
    		double alternatePrice = price/ExportSAP_AR_INV.getExchangeRate(sAlternateCurrency, date);
	    	sValue = (indicateNegative?"- ":"")+new DecimalFormat(SH.cs("AlternateCurrencyPriceFormat","# ##0.00")).format(alternatePrice)+" "+sAlternateCurrency;
	    	if(MedwanQuery.getInstance().getConfigInt("totalPriceCurrencyBeforeAmount",0)==1){
	    		sValue = sAlternateCurrency+" "+(indicateNegative?"- ":"")+new DecimalFormat(SH.cs("AlternateCurrencyPriceFormat","# ##0.00")).format(alternatePrice);
	    	}
	    	if(MedwanQuery.getInstance().getConfigInt("printPaymentCurrencyOnly",0)==0) {
	        	if(MedwanQuery.getInstance().getConfigInt("totalPriceCurrencyBeforeAmount",0)==0){
	        		sValue+="\n("+priceFormat.format(price)+" "+SH.cs("currency","")+")";
	        	}
	        	else{
	        		sValue+="\n("+SH.cs("currency","")+" "+(indicateNegative?"- ":"")+priceFormat.format(price)+")";
	        	}
	    	}
    	}
    	else {
	    	sValue = (indicateNegative?"- ":"")+priceFormat.format(price)+" "+sCurrency;
	    	if(MedwanQuery.getInstance().getConfigInt("totalPriceCurrencyBeforeAmount",0)==1){
	    		sValue = sCurrency+" "+(indicateNegative?"- ":"")+priceFormat.format(price);
	    	}
	    	sAlternateCurrency = SH.cs("AlternateCurrency","").toUpperCase();
    ...
    protected String getTotalPriceString(double price, boolean indicateNegative,java.util.Date date, String sPaymentCurrency){
        String sAlternateCurrency = SH.cs("preferAlternateCurrency","").toUpperCase();
        String sValue="";
    	if(sAlternateCurrency.length()>0 && !SH.cs("currency","").equalsIgnoreCase(sAlternateCurrency)) {
    		double alternatePrice = price/ExportSAP_AR_INV.getExchangeRate(sAlternateCurrency, date);
	    	sValue = (indicateNegative?"- ":"")+new DecimalFormat(SH.cs("AlternateCurrencyPriceFormat","# ##0.00")).format(alternatePrice)+" "+sAlternateCurrency;
	    	if(MedwanQuery.getInstance().getConfigInt("totalPriceCurrencyBeforeAmount",0)==1){
	    		sValue = sAlternateCurrency+" "+(indicateNegative?"- ":"")+new DecimalFormat(SH.cs("AlternateCurrencyPriceFormat","# ##0.00")).format(alternatePrice);
    ...
    protected String getTotalPriceString(double price){
        String sAlternateCurrency = SH.cs("preferAlternateCurrency","").toUpperCase();
        String sValue="";
    	if(SH.cs("preferAlternateCurrency","").length()>0 && !SH.cs("currency","").toUpperCase().equalsIgnoreCase(sAlternateCurrency)) {
	    	...
	    	sValue = priceFormat.format(price)+" "+SH.cs("currency","").toUpperCase();
    	}
        return sValue;
    }
