    ...
    private void printInvoice(PatientInvoice invoice, Vector debets){
    	try{
	        ...
	        for(int n=0;n<invoice.getCredits().size();n++){
	            ...
	            if(SH.c(credit.getComment()).length()>0) {
	            	creditComment+="\n"+credit.getComment();
	            }
	        ...
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
	        table.addCell(cell);
	        cell = createBoldLabelCell("°"+invoice.getPatient().dateOfBirth, 35,new Double(7*scaleFactor).intValue());
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
	        table.addCell(cell);
	        cell = createValueCell("", 15,new Double(7*scaleFactor).intValue(),Font.NORMAL);
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
	        table.addCell(cell);
	        cell = createBoldLabelCell(invoice.getPatientUid(), 35,new Double(12*scaleFactor).intValue());
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
	        table.addCell(cell);
	        ...
	        cell = createValueCell(ScreenHelper.getTran(null,"web","payments",sPrintLanguage)+" ["+credits+"]: "+getTotalPriceString(totalCredit, false, new java.util.Date(), sPaymentCurrency)+creditComment, 50*MedwanQuery.getInstance().getConfigInt("patientInvoiceReceiptCareDeliveryColumnWidthPercent",60)/100,new Double(7*scaleFactor).intValue(),Font.NORMAL);
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
	        table.addCell(cell);
	        ...
	        if(SH.ci("printBankInstructionOnReceipt", 0)==1) {
		        ...
		        cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
		        table.addCell(cell);
		        cell = createValueCell(SH.ci("printBankInstructionBalance", 1)==1?getTotalPriceString(invoice.getBalance()):getTotalPriceString(invoice.getPatientAmount()), 50,new Double(15*scaleFactor).intValue(),Font.BOLD);
		        cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
		        table.addCell(cell);
	        }
