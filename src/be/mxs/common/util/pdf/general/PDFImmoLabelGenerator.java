    ...
    protected void printImmoLabels(Vector articles){
        try{
            ...
            for(int n=0; n<articles.size(); n++){
                ...
                // part 2 : code/buyer
                cell = new PdfPCell(new Paragraph(id+": "+article.name.split("_")[1],FontFactory.getFont(FontFactory.COURIER,MedwanQuery.getInstance().getConfigInt("immofontsize",8),Font.BOLD)));
                cell.setColspan(1);
                cell.setBorder(PdfPCell.NO_BORDER);
                cell.setVerticalAlignment(PdfPCell.ALIGN_MIDDLE);
                cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
