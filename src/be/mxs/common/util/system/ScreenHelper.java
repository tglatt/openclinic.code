...
public class ScreenHelper {
    ...
    public static String getSync(String name, String id) {
    	String sId="-1";
    	Connection conn = MedwanQuery.getInstance().getOpenclinicConnection();
    	try {
    		PreparedStatement ps = conn.prepareStatement("select * from oc_sync where type=? and oldid=?");
    		ps.setString(1, name);
    		ps.setString(2, id);
    		ResultSet rs = ps.executeQuery();
    		if(rs.next()) {
    			sId=SH.c(rs.getString("newid"),"-1");
    		}
    		rs.close();
    		ps.close();
    		conn.close();
    ...
    public static long nightsBetween(java.util.Date begin,java.util.Date end){
    	...
    	if(nights==0) {
    		nights=SH.ci("minimumNumberOfAdmissionNights",0);
    	}
    	return nights;
    }
    ...
    public static java.util.Date beginOfDay(java.util.Date date){
    	if(date!=null){
    		try {
				return new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").parse(formatDate(date)+" 00:00:00");
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
    ...
    public static boolean updateEncounterUid(String oldUid, String newUid) {
    	...
    	try {
    		...
    		ps.execute();
    		ps.close();
    		//Update Encounterservicess
    		sSql="update oc_encounter_services set oc_encounter_objectid=? where oc_encounter_serverid=? and oc_encounter_objectid=?";
    		...
    		//Update Transactions
    		Vector<TransactionVO> transactions = MedwanQuery.getInstance().getTransactionsByEncounter(Integer.parseInt(encounter.getPatientUID()), oldUid);
    ...
    public static boolean updateTransactionUid(String oldUid, String newUid) {
    	boolean bSuccess = false;
    	try {
    		updateTableColumn("Transactions", "transactionid", oldUid.split("\\.")[1], newUid.split("\\.")[1]);
    		updateTableColumn("Items", "transactionid", oldUid.split("\\.")[1], newUid.split("\\.")[1]);
    		updateTableColumn("OC_PLANNING", "OC_PLANNING_TRANSACTIONUID", oldUid, newUid);
    		updateTableColumn("Requestedlabanalyses", "transactionid", oldUid.split("\\.")[1], newUid.split("\\.")[1]);
    ...
    public static boolean updateItemUid(String oldUid, String newUid) {
    	boolean bSuccess = false;
    	try {
    		updateTableColumn("Items", "itemid", oldUid, newUid);
    		bSuccess=true;
    	}
    	catch(Exception e) {
    		e.printStackTrace();
    ...
    public static String convertCharToHtmlCode(String text){
...
public static String removeAccents(String sTest){
    ...
    public static String writeDefaultLastCheckBox(TransactionVO transaction,HttpServletRequest request,String sLabelType, String sName, String sWebLanguage, String sEvent){
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
    		return("Unknown item: <a href='javascript:createTransactionItem(\""+transaction.getTransactionType()+"\",\"be.mxs.common.model.vo.healthrecord.IConstants."+sName+"\");'>"+sName+"</a>");
    	}
    	else {
    		...
    		s.append("<input "+setRightClickMini(request.getSession(), sName, sLabelType)+" "+sEvent+" type='checkbox' id='"+sName+"' value='1'");
	    	s.append(" name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value'");
	    	if(transaction.isNew() && SH.getLastItem(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue().equalsIgnoreCase("1")){
                s.append(" checked");
            }
	    	else if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue().equalsIgnoreCase("1")){
                s.append(" checked");
            }
            s.append(">");
            return s.toString();
    ...
    public static String writeDefaultLastCheckBoxes(TransactionVO transaction,HttpServletRequest request,String sLabelType, String sName, String sWebLanguage, boolean sorted){
    	return writeDefaultLastCheckBoxes(transaction, request, sLabelType, sName, sWebLanguage, sorted, "");
    }
    ...
    public static String writeDefaultLastCheckBoxes(TransactionVO transaction,HttpServletRequest request,String sLabelType, String sName, String sWebLanguage, boolean sorted, String sEvent){
    	return writeDefaultLastCheckBoxes(transaction, request, sLabelType, sName, sWebLanguage, sorted, sEvent,"");
    }
    ...
    public static String writeDefaultLastCheckBoxes(TransactionVO transaction,HttpServletRequest request,String sLabelType, String sName, String sWebLanguage, boolean sorted, String sEvent,String separator){
		boolean bMandatory=false;
		if(sName.startsWith("!")) {
			bMandatory=true;
			sName=sName.substring(1);
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
        	...
        	checkItemLabelTranslation(request.getSession(), sName, sLabelType);
	    	String sSelected = !transaction.isNew()?transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItem(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue();
    		StringBuffer s = new StringBuffer();
    		s.append(writeDefaultLastHiddenInput(transaction, sName,request));
	        if(request!=null && checkString((String)request.getSession().getAttribute("editmode")).equalsIgnoreCase("1")){
	    		saveTransactionItemTranslation(transaction.getTransactionType(), "be.mxs.common.model.vo.healthrecord.IConstants."+sName, sLabelType);
	        	s.append("<img width='16' src='"+request.getRequestURI().replaceAll(request.getServletPath(),"")+"/_img/icons/icon_plus.png' onclick='window.open(\""+request.getRequestURI().replaceAll(request.getServletPath(),"")+"/popup.jsp?Page=system/manageTranslations.jsp&FindLabelType="+sLabelType+"&find=1\",\"popup\",\"toolbar=no,status=yes,scrollbars=yes,resizable=yes,width=800,height=500,menubar=no\")'/>");
	        }
    ...
    public static String writeDefaultLastTodayCheckBoxes(TransactionVO transaction,HttpServletRequest request,String sLabelType, String sName, String sWebLanguage, boolean sorted, String sEvent,String separator){
		boolean bMandatory=false;
		if(sName.startsWith("!")) {
			bMandatory=true;
			sName=sName.substring(1);
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
        	...
        	checkItemLabelTranslation(request.getSession(), sName, sLabelType);
	    	String sSelected = !transaction.isNew()?transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItemToday(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue();
    		StringBuffer s = new StringBuffer();
    		s.append(writeDefaultLastTodayHiddenInput(transaction, sName,request));
	        if(request!=null && checkString((String)request.getSession().getAttribute("editmode")).equalsIgnoreCase("1")){
	    		saveTransactionItemTranslation(transaction.getTransactionType(), "be.mxs.common.model.vo.healthrecord.IConstants."+sName, sLabelType);
	        	s.append("<img width='16' src='"+request.getRequestURI().replaceAll(request.getServletPath(),"")+"/_img/icons/icon_plus.png' onclick='window.open(\""+request.getRequestURI().replaceAll(request.getServletPath(),"")+"/popup.jsp?Page=system/manageTranslations.jsp&FindLabelType="+sLabelType+"&find=1\",\"popup\",\"toolbar=no,status=yes,scrollbars=yes,resizable=yes,width=800,height=500,menubar=no\")'/>");
	        }
	        ...
	        if(labelTypes!=null){
	            ...
	            if(labelIds!=null){
	                ...
	                while(it.hasNext()){
	                    ...
	                    s.append("<label style='display: inline-block;'><input "+setRightClickMini(request.getSession(), sName)+" "+sEvent+" ondblclick='document.getElementById(\""+sName+"\").value=\"\";uncheckRadio(this);this.onchange();' onclick='removeOutline(this);document.getElementById(\""+sName+"\").value=\""+sLabelID+";\";' type='radio' name='"+sName+".radio' id='"+sName+"."+sLabelID+"' value='"+sLabelID+"'");
	                    if(arrayContains(sSelected,sLabelID,";")){
	                        s.append(" checked");
	                    }
    ...
    public static String writeDefaultLastRadioButtons(TransactionVO transaction,HttpServletRequest request,String sLabelType, String sName, String sWebLanguage, boolean sorted, String sEvent, String separator){
		boolean bMandatory=false;
		if(sName.startsWith("!")) {
			bMandatory=true;
			sName=sName.substring(1);
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	if(transaction.isNew()) {
	    		sSelected=SH.getLastItem(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue();
	    	}
    		StringBuffer s = new StringBuffer();
    		s.append(writeDefaultHiddenInput(transaction, sName,bMandatory));
	        if(request!=null && checkString((String)request.getSession().getAttribute("editmode")).equalsIgnoreCase("1")){
    ...
    public static String writeDefaultLastTextArea(HttpServletRequest request, TransactionVO transaction, String sName,int cols, int rows){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	StringBuffer s = new StringBuffer();
	    	s.append("<textarea onkeyup='resizeTextarea(this,10);limitChars(this,5000);' ");
	    	s.append(setRightClick(request.getSession(),sName));
	    	if(bMandatory) {
	    		s.append(" "+SH.cdm()+" ");
	    	}
	    	s.append(" class='text' cols='"+cols+"' rows='"+rows+"' id='"+sName+"' name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value'>");
	    	s.append((!transaction.isNew()?transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItem(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue())+"</textarea>");
	    	return s.toString();
    	}
    }
    ...
    public static String writeDefaultLastTextInput(HttpServletRequest request, TransactionVO transaction, String sName,int cols){
    	return writeDefaultLastTextInput(request, transaction, sName, cols, "");
    }
    ...
    public static String writeDefaultLastTextInput(HttpServletRequest request, TransactionVO transaction, String sName,int cols,String event){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	s.append(" class='text' size='"+cols+"' name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value'");
	    	s.append(" value='"+(!transaction.isNew()?transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItem(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue())+"' "+event+"/>");
	    	return s.toString();
    	}
    }
    
    public static String writeDefaultLastTodayTextInput(HttpServletRequest request, TransactionVO transaction, String sName,int cols,String event){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	s.append(" class='text' size='"+cols+"' name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value'");
	    	s.append(" value='"+(!transaction.isNew()?transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItemToday(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue())+"' "+event+"/>");
	    	return s.toString();
    	}
    }
    ...
    public static java.util.Date getDateAdd(java.util.Date date,long millis){
    ...
    public static String writeDefaultTextInputCenter(HttpSession session, TransactionVO transaction, String sName,int cols, String title){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	s.append(" value='"+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue()+"' title='"+title+"'/>");
	    	return s.toString();
    	}
    }
    ...
    public static String writeDefaultTimeInput(HttpSession session, TransactionVO transaction, String sName, String language,String contextpath){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	StringBuffer s = new StringBuffer();
	    	s.append(" <input type='time' id='"+sName);
	    	s.append(setRightClick(session,sName));
	    	if(bMandatory) {
	    		s.append(" "+SH.cdm()+" ");
	    	}
    ...
    public static String writeDefaultDateTimeInput(HttpSession session, TransactionVO transaction, String sName, String language,String contextpath){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
    	}
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null || transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName+"_TIME")==null){
    		return("Unknown item: <a href='javascript:createTransactionItem(\""+transaction.getTransactionType()+"\",\"be.mxs.common.model.vo.healthrecord.IConstants."+sName+"\");'>"+sName+"</a> Unknown item: <a href='javascript:createTransactionItem(\""+transaction.getTransactionType()+"\",\"be.mxs.common.model.vo.healthrecord.IConstants."+sName+"_TIME\");'>"+sName+"_TIME</a>");
    	}
    	else{
	    	StringBuffer s = new StringBuffer();
	    	s.append("<input type='text' id='"+sName+"' ");
	    	...
	    	s.append(" onblur='checkDate(this);' value='"+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue()+"'/>");
	    	s.append("<script>writeMyDate('"+sName+"', '"+contextpath+"/_img/icons/icon_agenda.png', '"+ScreenHelper.getTranNoLink("web","PutToday",language)+"');</script>");
	    	s.append(" <input type='time' id='"+sName+"_TIME' ");
	    	s.append(setRightClick(session,sName+"_TIME"));
	    	if(bMandatory) {
	    		s.append(" "+SH.cdm()+" ");
	    	}
	    	s.append(" class='text' size='12' maxlength='10' name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName+"_TIME").getItemId()+"]>.value'");
	    	s.append(" value='"+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName+"_TIME").getValue()+"'/>");
	    	return s.toString();
    	}
    }
    ...
    public static String writeDefaultLastHiddenInput(TransactionVO tran, String sName, HttpServletRequest request){
    	if(tran.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
    		return("Unknown item: <a href='javascript:createTransactionItem(\""+tran.getTransactionType()+"\",\"be.mxs.common.model.vo.healthrecord.IConstants."+sName+"\");'>"+sName+"</a>");
    	}
    	else{
	    	...
	    	s.append("<input type='hidden' id='"+sName+"' ");
	    	s.append(" name='currentTransactionVO.items.<ItemVO[hashCode="+tran.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value'");
	    	s.append(" value='"+(!tran.isNew()?tran.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItem(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue()).replaceAll("'", "´")+"'/>");
	    	return s.toString();
    	}
    }
    
    public static String writeDefaultLastTodayHiddenInput(TransactionVO tran, String sName, HttpServletRequest request){
    	if(tran.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
    		return("Unknown item: <a href='javascript:createTransactionItem(\""+tran.getTransactionType()+"\",\"be.mxs.common.model.vo.healthrecord.IConstants."+sName+"\");'>"+sName+"</a>");
    	}
    	else{
	    	...
	    	s.append("<input type='hidden' id='"+sName+"' ");
	    	s.append(" name='currentTransactionVO.items.<ItemVO[hashCode="+tran.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value'");
	    	s.append(" value='"+(!tran.isNew()?tran.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItemToday(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue()).replaceAll("'", "´")+"'/>");
	    	return s.toString();
    	}
    }
    ...
    public static String writeDefaultLastSelect(HttpServletRequest request,TransactionVO transaction, String sName,String type, String language, String sEvent){
    	return writeDefaultLastSelect(request, transaction, sName, type, language, sEvent, "");
    }
    ...
    public static String writeDefaultLastSelect(HttpServletRequest request,TransactionVO transaction, String sName,String type, String language, String sEvent,String sDefault){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
        	...
        	checkItemLabelTranslation(request.getSession(), sName, type);
	    	StringBuffer s = new StringBuffer();
	    	String itemValue=checkString(!transaction.isNew()?transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItem(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue());
	        if(request!=null && checkString((String)request.getSession().getAttribute("editmode")).equalsIgnoreCase("1")){
	    		saveTransactionItemTranslation(transaction.getTransactionType(), "be.mxs.common.model.vo.healthrecord.IConstants."+sName, type);
		    	s.append("<select style='vertical-align: top' "+(bMandatory?"data-mandatory='1'":"")+" title='"+sName+"' "+setRightClickMini(request.getSession(),sName)+" name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value' style='border:2px solid black; border-style: dotted' id='"+sName+"' onclick='window.open(\""+request.getRequestURI().replaceAll(request.getServletPath(),"")+"/popup.jsp?Page=system/manageTranslations.jsp&FindLabelType="+type+"&find=1\",\"popup\",\"toolbar=no,status=yes,scrollbars=yes,resizable=yes,width=800,height=500,menubar=no\")'>");
		    	s.append(" <option/>");
    ...
    public static String writeDefaultLastToggle(HttpServletRequest request,TransactionVO transaction, String sName,String type, String language, String sEvent){
    	return writeDefaultLastToggle(request, transaction, sName, type, language, sEvent, "");
    }
    ...
    public static String writeDefaultLastToggle(HttpServletRequest request,TransactionVO transaction, String sName,String type, String language, String sEvent,String sDefault){
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
    		return("Unknown item: <a href='javascript:createTransactionItem(\""+transaction.getTransactionType()+"\",\"be.mxs.common.model.vo.healthrecord.IConstants."+sName+"\");'>"+sName+"</a>");
    	}
    	else{
	    	...
	    	String itemValue=checkString(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue());
	    	if(transaction.isNew()) {
	    		itemValue=SH.getLastItem(request,"be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue();
	    	}
	        if(request!=null && checkString((String)request.getSession().getAttribute("editmode")).equalsIgnoreCase("1")){
	    		saveTransactionItemTranslation(transaction.getTransactionType(), "be.mxs.common.model.vo.healthrecord.IConstants."+sName, type);
		    	s.append("<div class='switch-field'>");
    ...
    public static String writeDefaultSelect(HttpServletRequest request,TransactionVO transaction, String sName, String language, String sEvent,int min,int max){
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	        ...
	        if(request!=null && checkString((String)request.getSession().getAttribute("editmode")).equalsIgnoreCase("1")){
		    	...
		    	s.append("<select style='vertical-align: top' "+(bMandatory?SH.cdm():"")+" "+sEvent+" class='text' id='"+sName+"' title='"+setRightClick(request.getSession(),sName)+"' ");
		    	s.append(" name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value'>");
		    	s.append(" <option/>");
		    	for(int n=min;n<=max;n++){
		    		s.append("<option value='"+n+"' "+(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue().equalsIgnoreCase(n+"")?"selected":"")+">"+n+"</option>");
    ...
    public static String writeDefaultLastTodayNumericInput(HttpServletRequest request, TransactionVO transaction, String sName,int cols,double min,double max,String language, String sOnBlur){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	s.append(" class='text' size='"+cols+"' name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value' ");
	    	s.append(" value='"+(!transaction.isNew()?transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItemToday(request, "be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue())+"' onblur='if(!this.value==\"\" && !isNumberLimited(this,"+min+","+max+")){this.value=\"\";alert(\""+getTranNoLink("web","valueoutofrange",language)+": "+min+" - "+max+"\");this.focus();}else{"+sOnBlur+";}'/>");
	    	return s.toString();
    	}
    }
    
    public static String writeDefaultLastNumericInput(HttpServletRequest request, TransactionVO transaction, String sName,int cols,double min,double max,String language, String sOnBlur){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	s.append(" class='text' size='"+cols+"' name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value' ");
	    	s.append(" value='"+(!transaction.isNew()?transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItem(request, "be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue())+"' onblur='if(!this.value==\"\" && !isNumberLimited(this,"+min+","+max+")){this.value=\"\";alert(\""+getTranNoLink("web","valueoutofrange",language)+": "+min+" - "+max+"\");this.focus();}else{"+sOnBlur+";}'/>");
	    	return s.toString();
    	}
    }
    ...
    public static String writeDefaultLastTodayNumericInput(HttpServletRequest request, TransactionVO transaction, String sName,int cols,double min,double max,String language, String sOnBlur, String sOnEvent){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
    	...
    	if(transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName)==null){
	    	...
	    	s.append(" class='text' size='"+cols+"' name='currentTransactionVO.items.<ItemVO[hashCode="+transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getItemId()+"]>.value' ");
	    	s.append(" value='"+(!transaction.isNew()?transaction.getItem("be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue():SH.getLastItemToday(request, "be.mxs.common.model.vo.healthrecord.IConstants."+sName).getValue())+"' onblur='if(!this.value==\"\" && !isNumberLimited(this,"+min+","+max+")){this.value=\"\";alert(\""+getTranNoLink("web","valueoutofrange",language)+": "+min+" - "+max+"\");this.focus();}else{"+sOnBlur+";}' "+sOnEvent+"/>");
	    	return s.toString();
    	}
    }
    
    public static String writeDefaultLastNumericInput(HttpServletRequest request, TransactionVO transaction, String sName,int cols,double min,double max,String language, String sOnBlur, String sOnEvent){
    	boolean bMandatory=false;
    	if(sName.startsWith("!")) {
    		sName=sName.substring(1);
    		bMandatory=true;
  ...
  public static String writeSelectWithStyle(HttpServletRequest request,String sLabelType, String sSelected,String sWebLanguage, String sStyle){
        ...
        if(labelTypes!=null){
            ...
            if(labelIds!=null){
                ...
                while(it.hasNext()){
                    ...
                    sOptions+= "/><label "+setEditTitle(request.getSession(), sBaseName, sLabelType)+" id='label-"+sBaseName+"-"+sLabelID+"' for='"+sBaseName+"-"+sLabelID+"'>"+sLabelValue+"</label>";
                }
            }
        }
        if(request!=null && checkString((String)request.getSession().getAttribute("editmode")).equalsIgnoreCase("1")){
    ...
    public static String getTotalPriceString(double price){
        String sAlternateCurrency = SH.cs("preferAlternateCurrency","").toUpperCase();
        String sValue="";
    	if(SH.cs("preferAlternateCurrency","").length()>0 && !SH.cs("currency","").toUpperCase().equalsIgnoreCase(sAlternateCurrency)) {
    		double alternatePrice = price/ExportSAP_AR_INV.getExchangeRate(sAlternateCurrency, new java.util.Date());
       		sValue =new DecimalFormat(SH.cs("AlternateCurrencyPriceFormat","# ##0.00")).format(alternatePrice)+" "+SH.cs("preferAlternateCurrency","");
    	}
    	else {
	    	sValue = new DecimalFormat(MedwanQuery.getInstance().getConfigString("priceFormat","#,##0.00")).format(price)+" "+SH.cs("currency","").toUpperCase();
    	}
        return sValue;
    }

    public static String getTotalPriceStringNoCurrency(double price){
        String sAlternateCurrency = SH.cs("preferAlternateCurrency","").toUpperCase();
        String sValue="";
    	if(SH.cs("preferAlternateCurrency","").length()>0 && !SH.cs("currency","").toUpperCase().equalsIgnoreCase(sAlternateCurrency)) {
    		double alternatePrice = price/ExportSAP_AR_INV.getExchangeRate(sAlternateCurrency, new java.util.Date());
       		sValue =new DecimalFormat(SH.cs("AlternateCurrencyPriceFormat","# ##0.00")).format(alternatePrice);
    	}
    	else {
	    	sValue = new DecimalFormat(MedwanQuery.getInstance().getConfigString("priceFormat","#,##0.00")).format(price);
    	}
        return sValue;
    }
    ...
    public static Vector getAllServices(){
        Vector serviceCodes = new Vector();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String sSelect;
        ...
        try{
            ...
            rs = ps.executeQuery();
            while(rs.next()){
                serviceCodes.add(Service.getService(rs.getString("serviceid")));
            }
        }
        catch(Exception e){
            e.printStackTrace();
    ...
    public static String convertToUUID(String n) {
    	...
    	if(SH.ci("enableOpenCarenetIPU", 0)==1) {
    		return MayeleAPI.convertToUUID(Integer.parseInt(n));
    	}
    	try {
    ...
    public static int convertFromUUID(String s) {
    	if(SH.ci("enableOpenCarenetIPU", 0)==1) {
    		return MayeleAPI.convertFromUUID(s);
    	}
    	int number=-1;
    	try {
        	if(s.split("-").length==2) {
    ...
    public static ItemVO getLastItemToday(HttpServletRequest request, String sType){
        try{
            SessionContainerWO sessionContainerWO = (SessionContainerWO) SessionContainerFactory.getInstance().getSessionContainerWO(request, SessionContainerWO.class.getName());
            if(sessionContainerWO.getHealthRecordVO()!=null && sessionContainerWO.getCurrentTransactionVO()!=null){
                ItemVO actualItem = sessionContainerWO.getCurrentTransactionVO().getItem(sType);
                if(actualItem==null || actualItem.getItemId().intValue() < 0){
                    ItemVO lastItem = MedwanQuery.getInstance().getLastItemVOAfterWithHealthrecordId(Integer.parseInt(sessionContainerWO.getHealthRecordVO().getHealthRecordId().toString()), sType,new java.util.Date(new java.util.Date().getTime()-SH.getTimeDay()));
                    if(lastItem==null){
                        if(sessionContainerWO.getCurrentTransactionVO().getItem(sType)!=null){
                            return sessionContainerWO.getCurrentTransactionVO().getItem(sType);
                        }
