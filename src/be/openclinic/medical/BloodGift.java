...
public class BloodGift {
	...
	public static Vector<BloodGift> find(Date begin,Date end, String site){
		...
		try {
			...
			while(rs.next()) {
				...
				if(gift.getTransaction()==null) {
					continue;
				}
				gift.setCollectionUnit();
				if(site.length()>0 && SH.c(gift.getCollectionUnit()).length()==0) { //unspecified collection site
					continue;
				}
	...
	private void setRefusalReasons() {
		...
		if(getAge()>65) reasonsForRefusal.add("bloodgift;olderthan65");
	...
	private void setNewDonor(Date begin) {
		setNewDonor(true);
		if(Pointer.getPointer("oldblooddonations."+getPersonid(),"0").equals("0")) {
			Connection conn = MedwanQuery.getInstance().getOpenclinicConnection();
			try {
