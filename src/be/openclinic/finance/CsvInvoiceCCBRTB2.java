...
public class CsvInvoiceCCBRTB2 {
	...
	public static String getOutput(javax.servlet.http.HttpServletRequest request){
		...
		String sOutput="#;DATE;INVOICE_ID;PATIENTID;BENEFICIARY_AGE;BENEFICIARY_SEX;BENEFICIARY_NAME;DEPARTMENTE;USER;CONS;LAB;IMA;HOS;ACT;MAT;OTH;MED;TOT 100%;TOT "+coverage+"%\r\n";
		if(invoiceuid!=null){
	        Vector debets = ExtraInsurarInvoice.getDebetsForInvoiceSortByDate(invoiceuid);
	        if(debets.size() > 0){
	            // print debets
	            Debet debet;
	            String sPatientName="", sPrevPatientName = "",sPatientId="",sPreviousInvoiceUID="",sUser="",sService="",beneficiarynr="",beneficiaryage="",beneficiarysex="",affiliatecompany="";
	            ...
	            for(int i=0; i<debets.size(); i++){
	                ...
	                sService=debet.getServiceUid();
	                if(sService!=null && sService.length()>0){
	                	Service service = Service.getService(sService);
	                	if(service !=null){
	                ...
	                if(i>0 && (displayDate || displayPatientName)){
	                    sOutput+=printDebet2(categories,displayDate,prevdate!=null?prevdate:date,invoiceid,sPatientId,sService,sUser,adherent,sPrevPatientName.split(";")[0],total100pct,total85pct,recordnumber,linecounter++,insurarreference,beneficiarynr,beneficiaryage,beneficiarysex,affiliatecompany);
    ...
    private static String printDebet2(SortedMap categories, boolean displayDate, Date date, String invoiceid,String patientid,String sService,String sUser,String adherent,String beneficiary,double total100pct,double total85pct,String recordnumber,int linecounter,String insurarreference,String beneficiarynr,String beneficiaryage,String beneficiarysex,String affiliatecompany){
        ...
        sOutput+=amount==null?"0;":amount+";";
        String otherprice=MedwanQuery.getInstance().getConfigString("ExcelFormulaPrefix","+")+"0";
