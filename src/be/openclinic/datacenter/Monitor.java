...
import be.openclinic.reporting.MalariaReport;
...
public class Monitor implements Runnable{
	...
	public static void runScheduler(){
        ...
        try {
    		...
    		Date lastNeonatalMonitorReport = new SimpleDateFormat("yyyyMMdd").parse(MedwanQuery.getInstance().getConfigString("lastNeonatalMonitorReport","19000101"));
    		if(new java.util.Date().getTime()-lastNeonatalMonitorReport.getTime()>SH.getTimeDay()+SH.getTimeHour()*2){
    			be.openclinic.util.NeonatalMonitoringData.exportReports();
	    		MedwanQuery.getInstance().setConfigString("lastNeonatalMonitorReport",new SimpleDateFormat("yyyyMMdd").format(SH.getToday()));
    		}
    		Date lastNeonatalMonitorCleaning = new SimpleDateFormat("yyyyMMddHHmmss").parse(MedwanQuery.getInstance().getConfigString("lastNeonatalMonitorCleaning","19000101010000"));
    		if(new java.util.Date().getTime()-lastNeonatalMonitorCleaning.getTime()>=hour){
    			be.openclinic.util.NeonatalMonitoringData.clean();
	    		MedwanQuery.getInstance().setConfigString("lastNeonatalMonitorCleaning",new SimpleDateFormat("yyyyMMddHHmmss").format(new java.util.Date()));
    		}
    		Date lastHemovigilance = new SimpleDateFormat("yyyyMMddHHmmss").parse(MedwanQuery.getInstance().getConfigString("lastHemovigilance","19000101010000"));
    		if(new java.util.Date().getTime()-lastHemovigilance.getTime()>=minute){
    			be.openclinic.util.Hemovigilance.send();
	    		MedwanQuery.getInstance().setConfigString("lastHemovigilance",new SimpleDateFormat("yyyyMMddHHmmss").format(new java.util.Date()));
    		}
