...
<%=sJSPROTOTYPE %>
<%
	String server=(request.getProtocol().toLowerCase().startsWith("https")?"https":SH.cs("weasisProtocol","http"))+"://"+ request.getServerName()+":"+SH.cs("weasisPort",""+request.getServerPort());
...
<script>
	var url='weasis://'+encodeURIComponent('$dicom:get -w "<%=server %><%=request.getRequestURI().replaceAll(request.getServletPath(),"")%>/pacs/wadoQuery.jsp?wadouid=<%=wadoid %>"');
	<%	
