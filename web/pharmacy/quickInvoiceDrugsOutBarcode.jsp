...
<%
	...
	String  sFinancialTransactionId = SH.p(request,"financialtransactionid");
	String  sInvoiceUid = SH.p(request,"invoiceuid");
	if(sWicketUid.length()==0){
		...
		if(insurance==null){
			...
			if(invoice.getDebets().size()>0){
				...
				String s = getTranNoLink("web","automaticdrugdeliverypayment",sWebLanguage);
				...
				sMessage="invoice;"+invoice.getUid();
		   		...
		   		MobileMoney.updateCreditOperationIds(sFinancialTransactionId, credit.getUid(), wicketCredit.getUid());
			}
			else{
