...
<%
	String sIncludePatients = checkString(request.getParameter("includePatients"));
	String sIncludeStocks = checkString(request.getParameter("includeStocks"));
	String sIncludeOther = checkString(request.getParameter("includeOther"));
    ...
    try {
        ...
        if(sIncludePatients.length()>0){
            parameters.put("includePatients","1");
        }
        if(sIncludeStocks.length()>0){
            parameters.put("includeStocks","1");
        }
        if(sIncludeOther.length()>0){
            parameters.put("includeOther","1");
        }
    	baosPDF = pdfGenerator.generatePDFDocumentBytes(request,"monthlyConsumption",parameters);
