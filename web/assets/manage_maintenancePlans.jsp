		...
		<form name="EditForm" id="EditForm" method="POST">
		    ...
		    <table class="list" border="0" width="100%" cellspacing="1">
		        ...
		        <tr>
		            ...
		            <td class="admin"><%=getTran(request,"web.assets","list.mail",sWebLanguage)%></td>
		            <td class="admin2" colspan="1">

		            <textarea maxlength="255" class="text" name="comment9" id="comment9" cols="40" rows="4" ><%= plan.getComment9() %></textarea>
		        ...
		        <tr>
		          ...
		          <tr>
			        <%-- DOCUMENTS (multi-add) --%>   
		            <td class="admin" nowrap>
		            	<%=getTran(request,"web.assets","expetiseexige",sWebLanguage)%>&nbsp;
		            </td>
		           
		           
		            <td class="admin2" colspan="3">
		            	<select class='text' name='comment6' id='comment6'>
		            		<option/>
		            		<%=ScreenHelper.writeSelect(request, "maintenanceplan.expertise", checkString(plan.getComment6() ), sWebLanguage) %>
		            	</select>
		            </td>
        ...
        new Ajax.Request(url,{
          ...
          onSuccess: function(resp){
           ...
           //alert(data.status);
            ...
            if(data.status=="error.samefrequencyplan.exist"){
             	alert('<%=getTranNoLink("web","error_samefrequencyplan_exist",sWebLanguage)%>');
           	 enableButtons();
  ...
  function loadDefaultMaintenancePlan(uid){
	    ...
	    new Ajax.Request(url,{
	      ...
	      onSuccess: function(resp){
              ...
              EditForm.comment9.value=data.comment9;
              EditForm.comment6.value=data.comment6;
              /*
              "&comment7="+EditForm.comment7.value+
              "&comment8="+EditForm.comment8.value+ 
              "&comment10="+EditForm.comment10.value+
