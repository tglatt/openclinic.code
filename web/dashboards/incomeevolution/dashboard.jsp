...
<script>
	...
	function <%=prefix%>showDetails(){
		openPopup('dashboards/<%=prefix%>/getDetails.jsp&language=<%=sWebLanguage%>&timeunit='+document.getElementById('<%=prefix%>timeunit').value,500,300,'<%=getTranNoLink("web","incomeevolution",sWebLanguage)%>');
	}
