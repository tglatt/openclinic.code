...
<%
	...
	String title2 = getTranNoLink("web","maternalmortality",sWebLanguage);
	String period="";

	String dates="",values="",values2="";
	Connection conn = SH.getOpenClinicConnection();
	if(sTimeUnit.equalsIgnoreCase("day")){
		SortedMap mortality = new TreeMap(), maternalmortality = new TreeMap();
		...
		while(rs.next()){
			...
			if(rs2.next()){
				if(maternalmortality.get(date)==null){
					maternalmortality.put(date,1);
				}
				else{
					maternalmortality.put(date,(Integer)maternalmortality.get(date)+1);
				}
			}
			rs2.close();
			ps2.close();
		...
		while(iMortality.hasNext()){
			...
			values2+= maternalmortality.get(date)==null?"0":maternalmortality.get(date)+"";
		}
		rs.close();
		ps.close();
