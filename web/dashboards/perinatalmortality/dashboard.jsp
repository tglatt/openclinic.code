...
<%=sJSCHARTJS %>
<%
	String prefix="perinatalmortality";
%>
<table width='100%'>
	<tr>
		<td style='text-align: center'><%=writeDashboardTimeGraph(prefix+"graph", 200, 150,"day") %></td>
	</tr>
	<tr>
...
<script>
	...
	function <%=prefix%>showDetails(){
		openPopup('dashboards/<%=prefix%>/getDetails.jsp&language=<%=sWebLanguage%>&timeunit='+document.getElementById('<%=prefix%>timeunit').value,500,300,'<%=getTranNoLink("web","maternalmortality",sWebLanguage)%>');
	}
