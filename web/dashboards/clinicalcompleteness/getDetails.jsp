...
<%
	String prefix="clinicalcompleteness";
%>
<table width='100%'>
	<tr class='admin'>
		<td colspan='3'>Complétude clinique par service</td>
	</tr>
	<%
		...
		Hashtable<String,Integer> hEncounters = new Hashtable<String,Integer>();
		HashSet hServices=new HashSet();
		...
		rs = ps.executeQuery();
		while(rs.next()){
			if(hEncounters.get(rs.getString("value"))!=null){
				hEncounters.put(rs.getString("value"),hEncounters.get(rs.getString("value"))+1);
			}
			else{
				hEncounters.put(rs.getString("value"),1);
			}
		}
		rs.close();
		ps.close();	
			...
			for(int n=0;n<vContacts.size();n++){
				if(vContacts.elementAt(n).split(";").length>=2){
					...
					if(serviceid.equalsIgnoreCase(service)){
						total++;
						if(hEncounters.get(uid)!=null){
							coverage+=hEncounters.get(uid);
						}
					}
					if(hEncounters.get(uid)!=null){
						allcoverage+=hEncounters.get(uid);
					}
				}
			}
