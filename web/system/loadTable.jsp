		...
		if (mrequest.getParameter("ButtonReadfile")!=null){
	        try{
	            ...
	            if (files != null && !files.isEmpty()){
					...
					if(mrequest.getParameter("filetype").equalsIgnoreCase("prestationscsv")){
					...
					else if(mrequest.getParameter("filetype").equalsIgnoreCase("assetscsvdryrun")){
						...
						while(reader.ready()){
							...
							String[] line = s.split(";");
							if(line.length==1){
								result+=("Line "+lines+": empty line, skipped<br/>");
								continue;
							}
							if(line[0].equalsIgnoreCase("code") || line.length<14){
								result+=("Line "+lines+": contains column names, skipped<br/>");
								continue;
							}
							...
							if(name.length()==0){
								result+=("Line "+lines+": name column cannot be empty, error<br/>");
								errors++;
								continue;
							}
							if(service.length()==0){
								result+=("Line "+lines+": service column cannot be empty, error<br/>");
								errors++;
								continue;
							}
							if(service.length()==0){
							...
							if(nomenclature.length()==0){
								result+=("Line "+lines+": nomenclature column cannot be empty, error<br/>");
								errors++;
								continue;
							}
							if(Service.getService(service)==null){
								result+=("Line "+lines+": service code <b>"+service+"</b> does not exist, error<br/>");
								errors++;
								continue;	
							}
							try{
								if(startdate.trim().length()>0){
									String r = SH.formatDate(new SimpleDateFormat(sDateFormat).parse(startdate));
								}
							}
							catch(Exception e){
								result+=("Line "+lines+": begin date <b>"+startdate+"</b> is invalid, error<br/>");
								errors++;
								continue;	
							}
							try{
								...
								result+=("Line "+lines+": price <b>"+price+"</b> is invalid, error<br/>");
								errors++;
								continue;	
							}
							result+=("Line "+lines+": is OK<br/>");
							loaded++;
						}
						message=result+
								"<h3>"+loaded+" " +getTran(request,"web","records.loaded",sWebLanguage)+"</h3><br/>"+
								"<h3>"+errors+" " +getTran(request,"web","errors",sWebLanguage)+"</h3><br/>";
					}
			...
			<td class='admin'>
				...
				<select name="filetype" id="filetype" class="text" onchange="showstructure();">
					<%if(!checkString(request.getParameter("pharmacyloadonly")).equalsIgnoreCase("1")){ %>
						<option value="prestationscsv" <%=filetype.equals("prestationscsv")?"selected":"" %>><%=getTran(request,"web","prestations.csv",sWebLanguage)%></option>
						<option value="servicescsv" <%=filetype.equals("servicescsv")?"selected":"" %>><%=getTran(request,"web","services.csv",sWebLanguage)%></option>
						<option value="labelscsv" <%=filetype.equals("labelscsv")?"selected":"" %>><%=getTran(request,"web","labels.csv",sWebLanguage)%></option>
						<option value="userscsv" <%=filetype.equals("userscsv")?"selected":"" %>><%=getTran(request,"web","userscsv",sWebLanguage)%></option>
						<option value="stockscsv" <%=filetype.equals("stockscsv")?"selected":"" %>><%=getTran(request,"web","stockscsv",sWebLanguage)%></option>
						<option value="assetscsvdryrun" <%=filetype.equals("assetscsvdryrun")?"selected":"" %>><%=getTran(request,"web","assetscsvdryrun",sWebLanguage)%></option>
						<option value="assetscsv" <%=filetype.equals("assetscsv")?"selected":"" %>><%=getTran(request,"web","assetscsv",sWebLanguage)%></option>
						<option value="labxml" <%=filetype.equals("labxml")?"selected":"" %>><%=getTran(request,"web","lab.xml",sWebLanguage)%></option>
						<option value="drugsxml" <%=filetype.equals("drugsxml")?"selected":"" %>><%=getTran(request,"web","drugs.xml",sWebLanguage)%></option>
						<option value="keywordsxml" <%=filetype.equals("keywordsxml")?"selected":"" %>><%=getTran(request,"web","keywords.xml",sWebLanguage)%></option>
					<%} %>
					<option value="pharmacyloadcsv" <%=filetype.equals("pharmacyloadcsv")?"selected":"" %>><%=getTran(request,"web","pharmacyload.csv",sWebLanguage)%></option>
				</select>
			</td>
