        	...
        	<%
                ...
                if(sEditWicketOperationUID.length()>0){
                    ...
                    <input class="button" type="button" name="buttonPrint" value='<%=getTranNoLink("Web","print",sWebLanguage)%>' onclick="doPrintPdf(document.getElementById('EditWicketOperationUID').value);">
                     <input class="button" type="button" name="buttonPrint" value='<%=getTranNoLink("Web","printreceipt",sWebLanguage)%>' onclick="doPrintReceiptPdf(document.getElementById('EditWicketOperationUID').value);">
                    <%
                }
                if(bShow){
...
<script>
  ...
  function doPrintReceiptPdf(creditUid){
	    var url = "<c:url value='/financial/createWicketPaymentReceiptPdfForPos.jsp'/>?CreditUid="+creditUid+"&ts=<%=getTs()%>&PrintLanguage=<%=sWebLanguage%>";
	    window.open(url,"WicketPaymentReceiptPdf<%=getTs()%>","height=600,width=900,toolbar=yes,status=no,scrollbars=yes,resizable=yes,menubar=yes");
	  }
