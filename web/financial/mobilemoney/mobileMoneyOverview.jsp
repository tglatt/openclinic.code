		...
		<tr>
			...
			<td class="admin2">
				<select name='operator' id='operator' class='text'>
					<%=SH.writeSelect(request, "momo.operator", "orange", sWebLanguage) %>
				</select>
			</td>
...
<script>
	...
	function checkPaymentStatus(transactionId,operator){
		if(operator=='mtn'){
			...
			document.getElementById('image_'+transactionId).height=8;
			document.getElementById('image_'+transactionId).src='<%=sCONTEXTPATH%>/_img/themes/default/ajax-loader.gif';
		    var params = "requestId="+transactionId;
			var today = new Date();
			...
			new Ajax.Request(url,{
		      	...
		      	onSuccess: function(resp){
	              	var paymentRequest = eval('('+resp.responseText+')');
	           	  	if(paymentRequest.status.length==0 || paymentRequest.status=="ok"){
	           	  		registerPaymentStatusMoov(transactionId,paymentRequest.status,paymentRequest.financialTransactionId);
	              	}
	           	  	else{
	            		document.getElementById('image_'+transactionId).height=16;
	              		document.getElementById('image_'+transactionId).src='<%=sCONTEXTPATH %>/_img/icons/icon_reload.png';
	          	...
	          	onError: function(resp){
	        	  	alert('Malitel Status validation error');
	        		document.getElementById('image_'+transactionId).height=16;
	          		document.getElementById('image_'+transactionId).src='<%=sCONTEXTPATH %>/_img/icons/icon_reload.png';
	          	}
			});
	...
	function registerPaymentStatusMoov(transactionId,status,financialTransactionId){
	    var params = "transactionId="+transactionId+"&status="+status+"&financialTransactionId="+financialTransactionId;
		var today = new Date();
	    var url= '<c:url value="/financial/mobilemoney/registerPaymentStatusMoov.jsp"/>?ts='+today;
		new Ajax.Request(url,{
		  	method: "POST",
	      	parameters: params,
	      	onSuccess: function(resp){
