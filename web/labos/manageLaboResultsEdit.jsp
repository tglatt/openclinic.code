                ...
                while (analysisEnumeration.hasNext()) {
                    ...
                    if(analysisCode.startsWith("00SCAN00")){
						...
						if(SH.ci("enableOutpatientMalariaExtension",0)==1){
							if(analysis.getMedidoccode().equalsIgnoreCase("70569-9")){
								sImage="&nbsp;&nbsp;<img style='vertical-align: middle' onclick='makeMalariyaPiBarcode()' height='16px' src='"+sCONTEXTPATH+"/_img/themes/default/malariyapi2.png'>";
							}
						}

	                    out.print("<td class='color color"+i+"'>"+analysisCode+"</td><td class='color color"+i+"' width='*'><b>"+MedwanQuery.getInstance().getLabel("labanalysis", c, sWebLanguage)+" "+u+sImage+"</b></td>");
	                    requestsIterator = requestList.keySet().iterator();
	                    while (requestsIterator.hasNext()) {
	                        LabRequest labRequest = (LabRequest) requestList.get(requestsIterator.next());
	                        ...
	                        if(requestedLabAnalysis != null){
		                        if (analysis.getEditor().equals("text")){
									if(bEditable){
										result="<input class='text' type='text' size='"+analysis.getEditorparametersParameter("SZ")+"' maxlength='"+analysis.getEditorparametersParameter("SZ")+"'  name='result."+labRequest.getServerid()+"."+labRequest.getTransactionid()+"."+requestedLabAnalysis.getAnalysisCode()+"' value='"+checkString(requestedLabAnalysis.getResultValue(),analysis.getEditorparametersParameter("DV"))+"'/>"+u;
									} else {
									...
									if(bEditable){
										...
										String[] options = analysis.getEditorparametersParameter("OP").split("\\|")[0].split(",");
										for(int n=0;n<options.length;n++){
											String key=options[n];
											String label=key;
											if(key.split("\\|").length>1){
									...
									if(analysis.getEditorparametersParameter("OP").split("\\|").length>1){
										result+="<font style='font-size: 9px;line-height: 1.4'><br/>"+analysis.getEditorparametersParameter("OP").split("\\|")[1]+":</font>";
									}
									result+="<br/><input type='text' name='resultcomment."+labRequest.getServerid()+"."+labRequest.getTransactionid()+"."+requestedLabAnalysis.getAnalysisCode()+"' value='"+requestedLabAnalysis.getResultComment()+"' class='text'/>";
		                        }
