...
<script>
  ...
  function doSave(){
	if(validateInsuranceNumber()){
	    if("<%=MedwanQuery.getInstance().getConfigString("InsuranceAuthorizationNumberNeededFor","$$").replaceAll("\\*","")%>"==document.getElementById('EditInsurarUID').value && document.getElementById('EditInsuranceNr').value==''){
	      alertDialog("web","insurancenr.mandatory");
	    }
	    else if("<%=MedwanQuery.getInstance().getConfigString("InsuranceAuthorizationNumberNeededFor","$$").replaceAll("\\*","")%>"==document.getElementById('EditInsurarUID').value && document.getElementById('EditInsuranceStatus').value==''){
...
%>
        ...
        <%-- employer --%>
        <tr>
            <td class="admin"><%=getTran(request,"insurance","iafa",sWebLanguage)%></td>
            <td class="admin2">
            	<select class="text" name="EditInsuranceMemberEmployer" id="EditInsuranceMemberEmployer" onchange='checkiafa();'>
            		<%=ScreenHelper.writeSelect(request, "sis.iafa", sEditInsuranceMemberEmployer.length()==0?"0":sEditInsuranceMemberEmployer, sWebLanguage) %>
            	</select>
            </td>
        </tr>
        <tr id='iafaimmat' style='display: none'>
            <td class="admin" width="<%=sTDAdminWidth%>"><%=getTran(request,"insurance","iafaimmat",sWebLanguage)%></td>
            <td class="admin2">
...
%>
 ...
 %>
        ...
        <%=ScreenHelper.setFormButtonsStart()%>
            ...
            <%if(activeUser.getAccessRight("financial.forceaccreditation.add")){ %>
            	<input class="button" type="button" name="ForceAccreditationbutton" id="ForceAccreditationbutton" value='<%=getTranNoLink("Web","forceaccreditation",sWebLanguage)%>' onclick="checkInsuranceAuthorization(true,'',true);">
            <%} %>
...
<script>
  ...
  function deleteAccreditation(accreditationId){
	  if(window.confirm('<%=getTranNoLink("web","areyousuretodelete",sWebLanguage)%>')){
		  var params = "accreditationid="+accreditationId;
		    var url= '<c:url value="/financial/deleteSISaccreditation.jsp"/>?ts='+new Date();
		    new Ajax.Request(url,{
			  method: "POST",
		      parameters: params,
		      onSuccess: function(resp){
		    	  checkInsuranceAuthorization();
	           	  $('ForceAccreditationbutton').style.display='';
		      },
			  onFailure: function(){
			    alert('error');
		      }
  ...
  function checkInsuranceAuthorization(force,ignorewarnings,manual){
	if(manual && EditInsuranceForm.EditInsuranceNr.value.length==0){
		alert('<%=getTranNoLink("web","insurancenumberismandatory",sWebLanguage)%>');
		return;
	}
    ...
    var params = "insuraruid="+EditInsuranceForm.EditInsurarUID.value+
                 ...
                 "&insurancenr="+EditInsuranceForm.EditInsuranceNr.value+
	...
	if(manual){
		params+="&manualautorization=true";
	}                 
	if(ignorewarnings){
    ...
    var url= '<c:url value="/financial/checkInsuranceAuthorizationSIS.jsp"/>?ts='+new Date();
    new Ajax.Request(url,{
	  method: "POST",
      parameters: params,
      onSuccess: function(resp){
        ...
        if($('authorized') && $('authorized').value=='1'){
        	...
        	$('ForceAccreditationbutton').style.display='none';
        }
        else if($('authorized') && $('authorized').value=='2'){
        	$('ForceAccreditationbutton').style.display='none';
        }
        if($('insid')){
        	$('EditInsuranceNr').value=$('insid').value;
        }
      },
	  onFailure: function(){
	    alert('error');
  ...
  function showdetails(accnr){
	  openPopup("/financial/showDetails.jsp&ts=<%=getTs()%>&PopupWidth=500&PopupHeight=420&accnr="+accnr);
  }
  
  function checkiafa(){
	  if(document.getElementById("EditInsuranceMemberEmployer") && document.getElementById('iafaimmat')){
		  if(document.getElementById("EditInsuranceMemberEmployer").value=='0' || document.getElementById("EditInsuranceMemberEmployer").value=='5' ){
			  document.getElementById('iafaimmat').style.display='none';
		  }
		  else{
			  document.getElementById('iafaimmat').style.display='';
		  }
	  }
  }
