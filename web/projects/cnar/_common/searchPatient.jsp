...
<table width="90%" onKeyDown='checkKeyDown(event);'>
    <form name="SF" method='post' action="<c:url value='/patientslist.do'/>?ts=<%=getTs()%>" id="SF">
        ...
        <%-- row 1 --%>
        <tr>
            <td align="right" nowrap width="55"><%=getTran(request,"Web", "Name", sWebLanguage)%>
            </td>
            <td>
            ...
            <td align="right" nowrap><%=getTran(request,"Web", "Firstname", sWebLanguage)%>&nbsp;
                <input id="ac1" autocomplete="off" class='<%=setFocus("Name",sDefaultFocus)%>' type='TEXT'
                       style='text-transform:uppercase' name='findFirstname' value="<%=sFirstname%>" size='20'
                       onblur='validateText(this);limitLength(this);'></td>
            <div id="ac1update" style="display:none; border: 1px solid black;background-color: white;"></div>

            <td align="right" nowrap><%=getTran(request,"Web", "DateOfBirth", sWebLanguage)%>&nbsp;<input
            ...
            <td width="1%" nowrap>
            	...
            	if(activePatient!=null){
	            	...
	            	if(death!=null){
						out.print("<img src='_img/warning.gif'/> <font style='{font-size: 12px; font-weight: bold; vertical-align: top}'>"+getTran(request,"web","died",sWebLanguage)+" "+new SimpleDateFormat("dd/MM/yyyy").format(death)+"</font>");
	            	}
	            	else {
	            		out.print(" ("+(activePatient.gender.equalsIgnoreCase("M")?getTran(request,"web.occup","male",sWebLanguage):getTran(request,"web.occup","female",sWebLanguage))+" - "+activePatient.getAge()+" "+getTran(request,"web","years",sWebLanguage).toLowerCase()+")");
	            	}
            	}
            	%>
            </td>
        ...
        <tr>
            ...
            <td>
                <input class='<%=setFocus("natreg",sDefaultFocus)%>' TYPE='TEXT' NAME='findnatreg' VALUE="<%=sNatreg%>"
                       size='25' onblur='validateText(this);limitLength(this);'>
            </td>
            <td align="right" nowrap><%=getTran(request,"Web", "immatnew", sWebLanguage)%>&nbsp;<input
                    class='<%=setFocus("immatnew",sDefaultFocus)%>' type='TEXT' style='text-transform:uppercase'
                    name='findimmatnew' value="<%=sNewimmat%>" size='20' onblur='validateText(this);limitLength(this);'>
            </td>
            <%
                if (activePatient != null && activePatient.getID("archiveFileCode").length() > 0) {
            %>
            <td align="right" nowrap>
                <a href="javascript:showArchiveCode();"><%=getTran(request,"Web", "archiveFileCode", sWebLanguage)%>
                </a>
                &nbsp;<input class='<%=setFocus("archiveFileCode",sDefaultFocus)%>' type='TEXT'
                             style='text-transform:uppercase' name='findArchiveFileCode' value="<%=sArchiveFileCode%>"
                             size='20' onblur='validateText(this);limitLength(this);'></td>
            <%
            } else {
            %>
            <td align="right" nowrap>
            ...
            %>
            <td align="right" nowrap><%=getTran(request,"Web", "personid", sWebLanguage)%>&nbsp;<input
        ...
        <%-- row 3 --%>
        <tr>
            <td align="right" nowrap><%=getTran(request,"Web", "service", sWebLanguage)%>
            </td>
            <td colspan='2' nowrap>
                <input class='text' TYPE="text" NAME="findUnitText" readonly size="49" TITLE="<%=sUnitText%>"
                       VALUE="<%=sUnitText%>" onkeydown="enterEvent(event,13)? window.event.keyCode='' : (window.which='');return true;">
                <%
                    if (sUnit.length() > 0) {
                %>
                <img src="<c:url value='/_img/icons/icon_info.gif'/>" class="link"
                     alt="<%=getTran(null,"Web","Information",sWebLanguage)%>" onclick='searchInfoService(SF.findUnit)'/>
                <%
                    }
                %>
            ...
            <td align='right' nowrap>
                <input class='button' type="button" name="findSearchPatient" id="findSearchPatient"
                       onclick='doSPatient();' value='<%=getTran(null,"Web","Find",sWebLanguage)%>'>&nbsp;
                <button accesskey="<%=ScreenHelper.getAccessKey(getTran(request,"AccessKey","Clear",sWebLanguage))%>"
                        class='button' name="clearButton"
                        onClick='clearPatient();return false;'><%=getTran(request,"AccessKey", "Clear", sWebLanguage)%>
                </button>
            </td>
        </tr>
    </form>
...
function clearPatient() {
    if (checkSaveButton("<%=sCONTEXTPATH%>", "<%=getTran(null,"Web.Occup","medwan.common.buttonquestion",sWebLanguage)%>")) {
        if (verifyPrestationCheck()) {
            if(document.getElementById("SF").findimmatnew) {
