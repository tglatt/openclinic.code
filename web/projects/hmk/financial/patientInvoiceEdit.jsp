                ...
                if(checkString(patientInvoice.getUid()).length() > 0){
                    ...
                    %>
                        ...
                        <%
                        	...
                        	if((!isInsuranceAgent 
                        			&& MedwanQuery.getInstance().getConfigInt("printPDFreceiptenabled",1)==1 
                        			&& patientInvoice.getBalance() == 0)
                        			|| 
                        			(!isInsuranceAgent 
                        			&& MedwanQuery.getInstance().getConfigInt("printPDFreceiptenabled",1)==1
                        			&& activeUser.getAccessRight("financial.modifyinvoice.select")
                        			)){
                        %>
                        ...
                        %>
                        <%
                        	if(MedwanQuery.getInstance().getConfigInt("enablePatientInvoiceRecreation",0)==1 
                        	&& activeUser.getAccessRight("financial.modifyinvoice.select") && 
                        	checkString(patientInvoice.getStatus()).equalsIgnoreCase("closed")){
                        %>
	                ...
	                %>
	                <% if(checkString(patientInvoice.getStatus()).equalsIgnoreCase("open") && 
	                		activeUser.getAccessRight("occup.transferinvoiceinsurer.select") && 
	                		MedwanQuery.getInstance().getConfigInt("enableEncounterTransfert",0)==1
	                		){%>
	                	
	                	 <input class="button" type="button" name="transferEncounter" value='<%=getTranNoLink("Web.finance","transferencounter",sWebLanguage)%>' onclick="dotransferEncounter('<%=patientInvoice.getUid()%>', 'encounterUid', 'encounterName');">	
	                
	                     <input class="text" type="hidden" id="encounterUid" name="encounterUid"/>
	                     <input class="text" type="hidden" id="encounterName" name="encounterName"/>
	  ...
	  <% 	}
	   ...
	   <script>
	    ...
	    function doModifyInvoice(invoiceuid){
		  ...
		  function dotransferEncounter(invoiceUid, encounterUid, encounterName ){
		
			  
			    	    openPopup("/_common/search/searchEncounter2.jsp&ts=<%=getTs()%>&VarCode="+encounterUid+"&VarText="+encounterName+"&VarFunction=executeTransferContact()&FindEncounterPatient=<%=activePatient.personid%>");
		  ...
		  function executeTransferContact(){
			  var invoiceuid = EditForm.EditPatientInvoiceUID.value;
			  var encounteruid = EditForm.encounterUid.value;
			  if(invoiceuid.length &&  encounteruid.length){
		    				 // alert(invoiceuid);
		    				  //alert(encounteruid);
		    				  
				    var params = '';
				    var today = new Date();
				    var url= '<c:url value="/financial/encountertransfer.jsp"/>?invoiceuid='+invoiceuid+'&encounteruid='+encounteruid+'&ts='+today;
				    document.getElementById('patientInvoiceDebets').innerHTML = "<img src='<c:url value="/_img/themes/default/ajax-loader.gif"/>'/><br/>Loading";
				    new Ajax.Request(url,{
					  method: "GET",
				      parameters: params,
