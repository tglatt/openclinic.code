...
function changeQuarter(){
    ...
    var url= path + '/_common/search/searchByAjax/getZipcodeByQuarterAndCityAndDistrictAndRegionMali.jsp?ts=' + today;
    new Ajax.Request(url,{
            method: "POST",
        ...
        ,
            onSuccess: function(resp){
                ...
                $("PZipcode").value = s.zipcode;
                $('PCity').value = "";
                var aSubQuarters = s.subquarters.split("$");
                $("PCity").options.length=0;
                for(var i=0; i<aSubQuarters.length; i++){
                    aSubQuarters[i] = aSubQuarters[i].replace(/^\s+/,'');
                    aSubQuarters[i] = aSubQuarters[i].replace(/\s+$/,'');
                    if ((aSubQuarters[i].length>0)&&(aSubQuarters[i]!=" ")){
                        $("PCity").options[i] = new Option(aSubQuarters[i], aSubQuarters[i]);
                    }
                }
            },
            onFailure: function(){
...
//            sBeginDate+= "&nbsp;<input type='button' name='buttonNewAPC' class='button' onclick='newAPC()' value='"+getTran(request,"Web","new",sWebLanguage)+"'>";
        ...
        out.print(sBeginDate
              +inputRow(request,"Web","address","PAddress","AdminPrivate",apc.address,"T",true,false,sWebLanguage)
              ...
              +normalRow("Web","subquarter","PCity","AdminPrivate",sWebLanguage)+"<select name='PCity' id='PCity'><option/>"
	       	  +ScreenHelper.writeSelect(request,"subquarter."+apc.quarter.split(" ")[0], checkString(apc.city), sWebLanguage)+"</select></td></tr>"
