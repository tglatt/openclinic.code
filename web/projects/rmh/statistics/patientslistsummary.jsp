...
<%
	...
	if(records.size()>0){
		out.println("<tr class='admin'><td>#</td><td>"+getTran(request,"web","date",sWebLanguage)+"</td><td>"+getTran(request,"web","insurance.number",sWebLanguage)+"</td><td>"+getTran(request,"web","personid",sWebLanguage)+"</td><td>"+getTran(request,"web","patient",sWebLanguage)+"</td><td>"+getTran(request,"web","administrator",sWebLanguage)+"</td>");
	...
	while(i.hasNext()){
		...
		out.println("<tr><td class='admin2'>"+counter+"</td><td class='admin2'>"+uid.split(";")[0]+"</td><td class='admin2'>"+(insurance==null?"":insurance.getInsuranceNr())+"</td><td class='admin2'>"+(patient==null?"":patient.personid)+"</td><td class='admin2'>"+(patient.lastname==null?"":patient.lastname.toUpperCase())+", "+patient.firstname+"</td><td class='admin2'>"+(uid.split(";").length<4?"?":uid.split(";")[3])+"</td>");
		Hashtable amounts = (Hashtable)records.get(uid);
	...
	out.println("<tr class='admin'><td colspan='6'>"+getTran(request,"web","total",sWebLanguage)+"</td>");
	Iterator m = invoicegroups.keySet().iterator();
	while(m.hasNext()){
		String invoicegroup=(String)m.next();
		out.println("<td></td>");
