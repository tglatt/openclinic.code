...
<%
	...
	tran.initializeLastItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_BIOMETRY_HEIGHT_EXIT");
%>
<table width="100%">
	<tr>
		<td width='1%' nowrap><b><%=getTran(request,"openclinic.chuk","temperature",sWebLanguage)%>:</b></td>
		<td width='20%' nowrap><%=SH.writeDefaultNumericInput(session, tran, "[GENERAL.ANAMNESE]ITEM_TYPE_TEMPERATURE_EXIT", 5,25,50,sWebLanguage) %> °C</td>
		<td width='1%' nowrap><b><%=getTran(request,"Web.Occup","medwan.healthrecord.biometry.length",sWebLanguage)%>:</b></td>
		<td width='20%' nowrap><%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_BIOMETRY_HEIGHT_EXIT", 5,0,250,sWebLanguage,"calculateBMI();") %> cm</td>
		<td width='1%' nowrap><b><%=getTran(request,"Web.Occup","medwan.healthrecord.biometry.weight",sWebLanguage)%>:</b></td>
		<td width='20%' nowrap><%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_BIOMETRY_WEIGHT_EXIT", 5,0,400,sWebLanguage,"calculateBMI();") %> kg</td>
	...
	<tr>
		...
		<td nowrap><%=SH.writeDefaultNumericInput(session, tran, "[GENERAL.ANAMNESE]ITEM_TYPE_SATURATION_EXIT", 5,0,100,sWebLanguage) %> %</td>
		<td width='1%' nowrap><b><%=getTran(request,"web","abdomencircumference",sWebLanguage)%>:</b></td>
		<td nowrap><%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_ABDOMENCIRCUMFERENCE_EXIT", 5,0,200,sWebLanguage) %> cm</td>
		<% if(activePatient.gender.toLowerCase().startsWith("f") && activePatient.getAge()>12){ %>
			...
			<td nowrap><%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_FOETAL_HEARTRATE_EXIT", 5,0,300,sWebLanguage) %> /min</td>
			<td width='1%' nowrap><b><%=getTran(request,"web","armcircumferenceshort",sWebLanguage)%>:</b></td>
			<td nowrap><%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_ARM_CIRCUMFERENCE_EXIT", 5,0,50,sWebLanguage) %> cm</td>
		<% }else{ %>
			<td width='1%' nowrap><b><%=getTran(request,"web","armcircumferenceshort",sWebLanguage)%>:</b></td>
			<td nowrap>
				<%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_ARM_CIRCUMFERENCE_EXIT", 5,0,50,sWebLanguage) %> cm
				<%=SH.writeDefaultHiddenInput(tran, "ITEM_TYPE_FOETAL_HEARTRATE_EXIT") %>
			</td>
	...
	<tr>
		...
		<td colspan='1' nowrap><%=ScreenHelper.writeDefaultTextInput(session, (TransactionVO) transaction, "ITEM_TYPE_BIOMETRY_GLYCEMIA_EXIT", 5) %> g/dl</td>
		<td width='1%' nowrap><b><%=getTran(request,"web","perimetrecranien",sWebLanguage) %></b></td>
			<td nowrap>
				<%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_PERIMETRE_CRANIEN_EXIT", 5, 10, 150, sWebLanguage) %> cm
			</td>
	</tr>
	<tr>
	...
	<tr>
		<td nowrap colspan='2'><%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_SYSTOLIC_PRESSURE_RIGHT_EXIT", 3,0,300,sWebLanguage, "setBP(this,\"ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_SYSTOLIC_PRESSURE_RIGHT_EXIT\",\"ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_DIASTOLIC_PRESSURE_RIGHT_EXIT\")") %> / <%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_DIASTOLIC_PRESSURE_RIGHT_EXIT", 3,0,300,sWebLanguage, "setBP(this,\"ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_SYSTOLIC_PRESSURE_RIGHT_EXIT\",\"ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_DIASTOLIC_PRESSURE_RIGHT_EXIT\")") %> mmHg</td>
		<td nowrap colspan='2'><%=SH.writeDefaultNumericInput(session, tran, "[GENERAL.ANAMNESE]ITEM_TYPE_RESPIRATORY_FRENQUENCY_EXIT", 5, 0, 100, sWebLanguage) %> /min</td>
		<td nowrap colspan='2'><%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_CARDIAL_CLINICAL_EXAMINATION_HEARTH_FREQUENCY_EXIT", 5, 0, 300, sWebLanguage,"setHF(this);") %> /min</td>
		<td nowrap colspan='2'>
		<% if(activePatient.getAgeInDaysOnDate(tran.getUpdateTime())<1857){ %>
			<table width='100%'>
				<tr>
					<td width='33%' nowrap>
						...
						<%=SH.writeDefaultHiddenInput(tran, "ITEM_TYPE_WEIGHT4HEIGHT_ZSCORE_EXIT") %>
					</td>
					<td width='33%' nowrap>
						...
						<%=SH.writeDefaultHiddenInput(tran, "ITEM_TYPE_WEIGHT4AGE_ZSCORE_EXIT") %>
					</td>
					<td width='33%' nowrap>
						...
						<%=SH.writeDefaultHiddenInput(tran, "ITEM_TYPE_HEIGHT4AGE_ZSCORE_EXIT") %>
					</td>
				</tr>
			</table>
		<% }
...
<script>
	function calculateBMI(){
	    var _BMI = 0;
	    var heightInput = document.getElementById('ITEM_TYPE_BIOMETRY_HEIGHT_EXIT');
	    var weightInput = document.getElementById('ITEM_TYPE_BIOMETRY_WEIGHT_EXIT');
	...
	function checkWeightForHeight(height,weight){
	    ...
	    new Ajax.Request(url,{
	        ...
	        onSuccess: function(resp){
	            ...
	            document.getElementById("ITEM_TYPE_WEIGHT4HEIGHT_ZSCORE_EXIT").value='';
	    		if(label.zindex>-999){
	    			if(label.zindex<-4){
	    			...
	    			document.getElementById("ITEM_TYPE_WEIGHT4HEIGHT_ZSCORE_EXIT").value=(label.zindex*1).toFixed(2);
	...
	function checkWeightForAge(weight){
	    ...
	    new Ajax.Request(url,{
	        ...
	        onSuccess: function(resp){
	            var label = eval('('+resp.responseText+')');
	            document.getElementById("ITEM_TYPE_WEIGHT4AGE_ZSCORE_EXIT").value='';
	    		if(label.zindex>-999){
	    			if(label.zindex<-4){
	    			...
	    			document.getElementById("ITEM_TYPE_WEIGHT4AGE_ZSCORE_EXIT").value=(label.zindex*1).toFixed(2);
	...
	function checkHeightForAge(height){
	    ...
	    new Ajax.Request(url,{
	        ...
	        onSuccess: function(resp){
	            var label = eval('('+resp.responseText+')');
	            document.getElementById("ITEM_TYPE_HEIGHT4AGE_ZSCORE_EXIT").value='';
	    		if(label.zindex>-999){
	    			if(label.zindex<-4){
	    			...
	    			document.getElementById("ITEM_TYPE_HEIGHT4AGE_ZSCORE_EXIT").value=(label.zindex*1).toFixed(2);
