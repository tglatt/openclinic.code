        ...
        <tr>
        	<td width="100%" valign='top'>
	        	...
	        	<tr>
        				...
        				<td class='admin2' ><%=SH.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_MSAS_PRENATAL_YEARNUM",10, 1)%></td>
			            ...
			            <td class="admin2" >
			                ...
			                <span id='fullcpn' style='display: none'><%=SH.writeDefaultCheckBox((TransactionVO)transaction, request, "medwan.common.true", "ITEM_TYPE_MSAS_PRENATAL_CPNCOMPLETE", "") %><%=getTran(request,"web","cpn.complete.8",sWebLanguage)%></span>
	        		...
	        		<tr>
			            ...
			            <td class='admin2'>
			            	<%=SH.writeDefaultTextInput(session, (TransactionVO)transaction, "ITEM_TYPE_MSAS_PRENATAL_CHILDRENALIVE", 5) %>
						</td>
			            <td class='admin'><%=getTran(request,"web","deadchildren",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultTextInput(session, (TransactionVO)transaction, "ITEM_TYPE_MSAS_PRENATAL_CHILDRENDEAD", 5) %>
						</td>
	        		</tr>
	        		<tr>
			            <td class='admin'><%=getTran(request,"web","doblastchild",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultLastDateInput(request, tran, "ITEM_TYPE_DOBLASTCHILD", sWebLanguage, sCONTEXTPATH) %>
						</td>
			            <td class='admin'><%=getTran(request,"web","statuslastchild",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultLastRadioButtons(tran, request, "deadalive", "ITEM_TYPE_STATUSLASTCHILD", sWebLanguage, false, "", "") %>
						</td>
	        		</tr>
	        		<tr>
			            <td class='admin'><%=getTran(request,"web","fpafterlastbirth",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultLastTextArea(request, tran, "ITEM_TYPE_FPAFTERLASTBIRTH", 40, 1) %>
						</td>
			            <td class='admin'><%=getTran(request,"web","gestationalage",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_GESTATIONALAGE", sWebLanguage, "", 3, 43) %>
						</td>
	        		</tr>
	        		<tr>
			            <td class='admin'><%=getTran(request,"web","riskfactorsfirstcontact",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultLastTextArea(request, tran, "ITEL_TYPE_RISKFACTORSLASTCONTACT", 40, 1) %>
						</td>
			            <td class='admin'><%=getTran(request,"web","riskfactorsthiscontact",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultLastTextArea(request, tran, "ITEL_TYPE_RISKFACTORSTHISCONTACT", 40, 1) %>
						</td>
	        		</tr>
			            ...
			            <script>
			            	...
			            	function checkCPNOrder(){
			            		if(document.getElementById('cpnorder').value=='8'){
		        	...
		        	<tr>
			            ...
			            <td class='admin2' colspan='3'>
			                <%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_MSAS_PRENATAL_BREASTEXAMINATION", 80, 1) %>
						</td>
			        </tr>
		        	<tr>
			        ...
			        <tr>
			            ...
			            <td class='admin2'>
			            	<%=SH.writeDefaultNumericInput(session, (TransactionVO)transaction, "ITEM_TYPE_MSAS_PRENATAL_HB", 10, 0, 50, sWebLanguage)%> mg/dl
						</td>
			            <td class="admin"><%=getTran(request,"web","hepatitisb",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_MSAS_PRENATAL_HEPB", "posneg", sWebLanguage, "")%> 
						</td>
			        </tr>
			        <tr>
			            <td class="admin"><%=getTran(request,"web","sicklecell",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultTextInput(session, tran, "ITEM_TYPE_SICKLECELL", 40)%>
						</td>
			            <td class="admin"><%=getTran(request,"web","urine",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultTextInput(session, tran, "ITEM_TYPE_URINE", 40)%>
						</td>
			        </tr>
			        ...
			        <tr>
			            ...
			            <td class='admin2'>
	                        ...
	                        <script>writeMyDate("vatdate", "<c:url value="/_img/icons/icon_agenda.png"/>", "<%=getTran(null,"Web","PutToday",sWebLanguage)%>");</script><br/>
	                        <%=getTran(request,"web","vaccinationsite",sWebLanguage) %>: <%=SH.writeDefaultRadioButtons(tran, request, "vaccinationsite", "!ITEM_TYPE_MSAS_CONS_VACCINATIONSITE", sWebLanguage, false,"","") %>
			        	</td>
	                        
	                        
			            </td>
			            <td class='admin'><%=getTran(request,"web","iva",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'><%=SH.writeDefaultRadioButtons((TransactionVO)transaction, request, "posnegnot", "ITEM_TYPE_MSAS_PRENATAL_IVA",sWebLanguage,false,"","") %></td>
			        </tr>
			        <tr>
			        ...
			        <tr>
			            ...
			            <td class="admin2">
			            	<%=SH.writeDefaultSelect(request, (TransactionVO)transaction, "ITEM_TYPE_MSAS_PRENATAL_MALNUTRITION", "msas.nutrition", sWebLanguage, "") %>
			            	<%=SH.writeDefaultCheckBox(tran, request, "medwan.common.true", "ITEM_TYPE_MSAS_PRENATAL_MALNUTRITION_SCREENED", "") %><%=getTran(request,"web","screened",sWebLanguage) %>
			            </td>
			        </tr>
			        <tr>
		        	...
		        	<tr>
			            ...
			            <td class='admin2' colspan='3'>
			                <%=SH.writeDefaultRadioButtons((TransactionVO)transaction, request, "yesno", "ITEM_TYPE_MSAS_PRENATAL_PF",sWebLanguage,false,"","") %>
						</td>
			        </tr>
		        	...
		        	<tr>
			            <td class="admin"><%=getTran(request,"web","nutritioninformation",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			                <%=SH.writeDefaultCheckBoxes(tran, request, "cpn.nutritioninformation", "ITEM_TYPE_MSAS_PRENATAL_AME", sWebLanguage, false) %>
						</td>
			            <td class='admin'><%=getTran(request,"anc","malariatreatment",sWebLanguage)%>&nbsp;</td>
			            <td class="admin2">
			            	<%=SH.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_MSAS_PRENATAL_MALARIATREATMENT", 40, 1) %>
			            </td>
			        </tr>
		        	<tr>
			            <td class="admin"><%=getTran(request,"web","sti",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_MSAS_PRENATAL_STI", 40, 1) %>
						</td>
			            <td class='admin'><%=getTran(request,"anc","stipartner",sWebLanguage)%>&nbsp;</td>
			            <td class="admin2">
			            	<%=SH.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_MSAS_PRENATAL_STIPARTNER", 40, 1) %>
			            </td>
			        </tr>
		        	<tr>
			            <td class="admin"><%=getTran(request,"web","syphilisindex",sWebLanguage)%>&nbsp;</td>
			            <td class='admin2'>
			            	<%=SH.writeDefaultDateInput(session, tran, "ITEM_TYPE_SYPHILIS_TREATMENT_DATE", sWebLanguage, sCONTEXTPATH) %>
						</td>
			            <td class='admin'><%=getTran(request,"anc","syphilisindex",sWebLanguage)%>&nbsp;</td>
			            <td class="admin2">
			            	<%=SH.writeDefaultDateInput(session, tran, "ITEM_TYPE_SYPHILIS_PARTNERTREATMENT_DATE", sWebLanguage, sCONTEXTPATH) %>
			            </td>
			        </tr>
			        <tr>
			            <td class="admin"><%=getTran(request,"web", "consultation.observations", sWebLanguage)%></td>
			            <td class="admin2">
			                <textarea rows="1" onKeyup="resizeTextarea(this,10);" class="text" cols="40" name="currentTransactionVO.items.<ItemVO[hashCode=<mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_MSAS_PRENATAL_OBSERVATIONS" property="itemId"/>]>.value"><mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_MSAS_PRENATAL_OBSERVATIONS" property="value"/></textarea>
			            </td>
			            <td class="admin"><%=getTran(request,"web", "consultation.mgf", sWebLanguage)%></td>
			            <td class="admin2">
			                <%=SH.writeDefaultRadioButtons(tran, request, "msas.mgf", "ITEM_TYPE_MSAS_PRENATAL_MGF", sWebLanguage, false, "", "") %>
			            </td>
			        </tr>
					<tr>
						<td colspan="4">
