...
<%
    ...
    if((activePatient!=null)&&(request.getParameter("SavePatientEditForm")==null)) {
        ...
        out.print(inputRow(request,"Web","Lastname","Lastname","Admin",activePatient.lastname,"T",true, true,sWebLanguage)
            ...
            +inputRow(request,"Web","nativetown","NativeTown","Admin",activePatient.nativeTown,"T",true,false,sWebLanguage)
      ...
      String sCivilStatus = "<select name='CivilStatus' select-one class='text'><option value=''/>"
		...
		int nBloodGifts=0;
		try{
			MedwanQuery.getInstance().getTransactionsByType(Integer.parseInt(activePatient.personid), "be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_CNTS_BLOODGIFT").size();
		}
		catch(Exception e){}
      ...
      out.print(normalRow("Web","gender","Gender","Admin",sWebLanguage)+sGender+"</select></td></tr>"
	      ...
	      //+inputRow(request,"Web","numberofbloodgifts","bloodgifts","Admin",nBloodGifts+"","T",false, false,sWebLanguage)
