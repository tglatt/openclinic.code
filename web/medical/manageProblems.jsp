        ...
        while (iter.hasNext()){
            ...
            if (activeUser.getAccessRight("occup.restricteddiagnosis.select") || !MedwanQuery.getInstance().isRestrictedDiagnosis(tmpProblem.getCodeType(), tmpProblem.getCode())){
                sOutput = "<tr class=\"list"+sClass+"\"" +
                        ...
                        "<td>&nbsp;<img src='http://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/_img/icons/icon_delete.png' alt='"+getTranNoLink("web", "delete", sWebLanguage)+"' border='0' onclick='doDelete(\""+tmpProblem.getUid()+"\");'>&nbsp;</td>" +
...
<table class="list" width="100%" cellspacing="0">
    ...
    <input type="hidden" name="reloadFunction" id="reloadFunction" value="<%=checkString(request.getParameter("reloadFunction"))%>">
    ...
    <table class="list" width="100%" cellspacing="1">
        ...
        <tr>
            ...
            <td class="admin2">
                ...
                <img src="<c:url value="/_img/icons/icon_search.png"/>" class="link" alt="<%=getTranNoLink("Web","select",sWebLanguage)%>" onclick="searchICPC('eCode','eLabel','eType');">
                <img src="<c:url value="/_img/icons/icon_delete.png"/>" class="link" alt="<%=getTranNoLink("Web","clear",sWebLanguage)%>" onclick="EditProblemForm.EditProblemCode.value='';EditProblemForm.EditProblemCodeLabel.value='';EditProblemForm.EditProblemCodeType.value='';">
            </td>
...
<%
    ...
    if (sAction.equals("SAVE")){
		%>
		<script>
		  window.location.href = '<c:url value="/"/>popup.jsp?Page=medical/manageProblems.jsp&reloadFunction='+document.getElementById('reloadFunction').value+'&ts=<%=getTs()%>&PopupWidth=700&PopupHeight=500';
		</script>
		<%
    }
%>
<script>
  ...
  function doNew(){
    window.location.href = "<c:url value='/popup.jsp'/>?Page=medical/manageProblems.jsp&reloadFunction="+document.getElementById('reloadFunction').value+"&Action=NEW&ts=<%=getTs()%>&PopupWidth=700&PopupHeight=300";
  }
  ...
  function doClose(){
    <%if(request.getParameter("reloadFunction")==null || request.getParameter("reloadFunction").length()==0){%>
	    var URL = unescape(window.opener.location);
	    window.opener.location.href = URL;
	<%}
    else{%>
    	window.opener.<%=request.getParameter("reloadFunction")%>;
    <%}%>
    window.close();
  }

  function doSearchBack(){
    window.location.href = "<c:url value='/popup.jsp'/>?Page=medical/manageProblems.jsp&reloadFunction="+document.getElementById('reloadFunction').value+"&ts=<%=getTs()%>";
  }
  ...
  function doSelect(problem){
    window.location = "<c:url value='/popup.jsp'/>?Page=medical/manageProblems.jsp&reloadFunction="+document.getElementById('reloadFunction').value+"&Action=SELECT&EditProblemUID="+problem+"&ts=<%=getTs()%>";
  }

  function doDelete(problem){
      if(yesnoDeleteDialog()){
      window.location = "<c:url value='/popup.jsp'/>?Page=medical/manageProblems.jsp&reloadFunction="+document.getElementById('reloadFunction').value+"&Action=DELETE&DeleteUID="+problem+"&ts=<%=getTs()%>";
    }
  }
</script>