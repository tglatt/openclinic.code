...
<%
	...
	if(MedwanQuery.getInstance().getConfigInt("enableMPI",0)==1){
		...
		if(sMPIID.length()>0){
			sMPI=" | "+SH.cs("mpiServerName","KHIN")+": &nbsp;&nbsp;<a style='text-decoration: none;' href='javascript:showMPI()'><font style='padding: 2px;border: 2px solid;font-size:14px;font-weight: bolder'>"+sMPIID+"</font></a> <span id='MPIconnected'></span>";
		}
		else{
			sMPI=" | <a href='javascript:showMPI()'><font style='font-size:12px'>"+SH.cs("mpiServerName","KHIN")+": </font><img height='14px' style='vertical-align: middle' src='"+sCONTEXTPATH+"/_img/icons/icon_erase.gif'/></a> <span id='MPIconnected'></span>";
		}
	}
	String sOpenIMIS="";
...
<script>
  ...
  function mpiget(mpiid){
	    ...
	    document.getElementById('divmpisearch').innerHTML = "<img height='12px' src='<c:url value="/_img/themes/default/ajax-loader.gif"/>'/>";
  ...
  function creatempi(){
	    ...
	    document.getElementById('creatempi').innerHTML = "<br/><img height='12px' src='<c:url value="/_img/themes/default/ajax-loader.gif"/>'/>";
		new Ajax.Request(url,{
		method: "POST",
		parameters: params,
		onSuccess: function(resp){
  ...
  function analyzeIkirezi(){
	    var params = "";
		var url = "<%=sCONTEXTPATH%>/curative/analyzeIkirezi.jsp";
		new Ajax.Request(url,{
		method: "POST",
		parameters: params,
		onSuccess: function(resp){
			var label = eval('('+resp.responseText+')');
			if(label.message.indexOf("<empty/>")<0){
			    alert(label.message);
			}
		}
		});
  ...
  function checkMPI(){
	    var params = "";
		var url = "<%=sCONTEXTPATH%>/curative/checkMPI.jsp";
		new Ajax.Request(url,{
		method: "POST",
		parameters: params,
		onSuccess: function(resp){
			if(resp.responseText.trim()*1>0){
				val1=resp.responseText.trim()*1;
				val=Math.round(100-Math.log10(10-(5000-val1)*0.9/500)*100);
				document.getElementById('MPIconnected').innerHTML="&nbsp;&nbsp;&nbsp;&nbsp;<img style='vertical-align: middle' height='18px' src='<%=sCONTEXTPATH%>/_img/icons/icon_connected.png'/> <font style='font-size: 7px'>"+val+"</font><font style='font-size: 5px'>%</font>";
				missed=0;
				delay=new Date().getTime();
			}
			else if(resp.responseText.trim()*1==-1 && (new Date().getTime()-delay)><%=SH.ci("intervalMPICheck", 10000)%>){
				delay=new Date().getTime();
				missed++;
				if(missed==1){
					document.getElementById('MPIconnected').innerHTML="&nbsp;&nbsp;&nbsp;&nbsp;<img style='vertical-align: middle' height='18px' src='<%=sCONTEXTPATH%>/_img/icons/icon_connected.png'/> <font style='font-size: 7px'>.</font>";
				}
				else if(missed==2){
					document.getElementById('MPIconnected').innerHTML="&nbsp;&nbsp;&nbsp;&nbsp;<img style='vertical-align: middle' height='18px' src='<%=sCONTEXTPATH%>/_img/icons/icon_connected.png'/> <font style='font-size: 7px'>..</font>";
				}
					...
					document.getElementById('MPIconnected').innerHTML="&nbsp;&nbsp;&nbsp;&nbsp;<img style='vertical-align: middle' height='18px' src='<%=sCONTEXTPATH%>/_img/icons/icon_disconnected.png'/>";
				}
			}
			window.setTimeout("checkMPI();",2000);
		}
		});
  }

  <%if(SH.ci("enableMPI",0)==1){
	  if(SH.ci("MPIServerResponseDelay",-1)>0){
		  %>
			val=Math.round(100-Math.log10(10-(5000-<%=SH.ci("MPIServerResponseDelay",-1)%>)*0.9/500)*100);
			document.getElementById('MPIconnected').innerHTML="&nbsp;&nbsp;&nbsp;&nbsp;<img style='vertical-align: middle' height='18px' src='<%=sCONTEXTPATH%>/_img/icons/icon_connected.png'/> <font style='font-size: 7px'>"+val+"</font><font style='font-size: 5px'>%</font>";
		  <%
	  }
	  %>  
