        ...
        if(lResults.size() > 0){
            ...
            while(it.hasNext() && lines++<iMaxRows){
                ...
                if(SH.c(prestation.getServiceUid()).length()>0){
               	...
               	out.write("<span style='display:none'>"+prestation.getUid()+";"+(insurance.getExtraInsurarUid().length()>0 && SH.p(request,"defaultExtraInsurar").equals("1")?"0":SH.getPriceFormat(prestation.getPatientPrice(insurance,insurance.getInsurar().getType(),sEncounterType)))+"-idcache</span>");
                out.write("</li>");
            }
        }
