...
<table width='100%'>
    ...
    <tr>
    	<td colspan='2'>
            <input type="hidden" name="mobilewicketuid" id="mobilewicketuid"/>
	...
	<tr id='payment_tr'>
		<td colspan='2'>
			<center>
				<input type='button' class='button' name='paymentButton' id='paymentButton' value='<%=getTranNoLink("web","paymentreceived",sWebLanguage) %>' onclick='doPayment();'/>
				<%
			    	if(SH.ci("enableOrangeMoney",0)==1 && activeUser.getAccessRight("financial.mobilepayment.select")){
			            %><button type='button' class='button' name="buttonOrangeMoney" id="buttonOrangeMoney" onclick="doMobilePaymentOrange();"><img height='16px' style='vertical-align: middle' src='<%=sCONTEXTPATH%>/_img/themes/default/orangemoney.png'/></button><%
			    	}
	            	if(SH.ci("enableMoovMoney",0)==1 && activeUser.getAccessRight("financial.mobilepayment.select")){
	                    %><button type='button' style='vertical-align: middle' class='button' name="buttonMoovMoney" onclick="doMobilePaymentMoov();"><img height='16px' style='vertical-align: middle' src='<%=sCONTEXTPATH%>/_img/themes/default/moovmoney.png'/></button><%
	            	}
				%>
			</center>
		</td>
...
<script>
	...
	function doPayment(comment,financialTransactionId){
		if((document.getElementById('invoiceuid').value=='' || (document.getElementById("wicketuid").value.length==0 && document.getElementById("mobilewicketuid").value.length==0)) && (document.getElementById("prestationlist").innerHTML.length<40 ||
			...
			if(document.getElementById("buttonOrangeMoney")){
				document.getElementById("buttonOrangeMoney").disabled= true;
			}
			if(document.getElementById("buttonMoovMoney")){
				document.getElementById("buttonMoovMoney").disabled= true;
			}
			var wicketuid=document.getElementById('wicketuid').value;
			if(document.getElementById('mobilewicketuid').value.length>0){
				wicketuid=document.getElementById('mobilewicketuid').value;
			}
		    var params = 	"prestations="+prestationlist+
		    				...
		    				"&invoiceuid="+document.getElementById('invoiceuid').value+
		   	...
		   	if(financialTransactionId){
		   		params+="&financialtransactionid="+financialTransactionId;
		   	}
		   	if(document.getElementById('invoiceuid')){
		   		params+="&invoiceuid="+document.getElementById('invoiceuid').value;
		   	}
	...
	function doMobilePaymentOrange(){
		if(document.getElementById('invoiceuid').value==''){
			var url= '<c:url value="/curative/ajax/getInvoiceUid.jsp"/>?ts='+new Date();
			new Ajax.Request(url,{
				method: "POST",
			   	parameters: "",
			   	onSuccess: function(resp){
			    	var label = eval('('+resp.responseText+')');
			    	document.getElementById('invoiceuid').value=label.invoiceuid;
					var url = "financial/mobilemoney/mobileMoneyPayment.jsp&invoiceid="+document.getElementById("invoiceuid").value+"&operator=Orange&amount="+document.getElementById("amount").value+"&currency=<%=SH.cs("currency","RWF")%>&ts=<%=getTs()%>";
					openPopup(url);
			    },
				onFailure: function(){
			  		alert('error');
			...
			new Ajax.Request(url,{
				method: "POST",
			   	...
			   	onSuccess: function(resp){
			    	var label = eval('('+resp.responseText+')');
			    	document.getElementById('invoiceuid').value=label.invoiceuid;
					var url = "financial/mobilemoney/mobileMoneyPayment.jsp&invoiceid="+document.getElementById("invoiceuid").value+"&operator=Moov&amount="+document.getElementById("amount").value+"&currency=<%=SH.cs("currency","RWF")%>&ts=<%=getTs()%>";
					openPopup(url);
			    },
				onFailure: function(){
			  		alert('error');
	...
	function registerMomoPayment(cashdesk,comment,financialTransactionId){
		document.getElementById("mobilewicketuid").value=cashdesk;
		doPayment(comment,financialTransactionId);
	}
