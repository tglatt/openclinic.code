...
function addICD11(code ="CODE 1" , label = "LABEL CODE 1"){
    openPopup("/_common/search/diagnoseInfoIcd11.jsp&AuthorUID=<%=sAuthorId%>&ts=<%=getTs()%>&showpatientencounters=<%=sShowPatientEncounters%>&Type=ICD11&Code="+code+"&Value="+codeICD11+"&Label="+label+"&returnField=<%=sReturnField%>&returnField2=<%=sReturnField%>&patientuid=<%=sPatientUid%>",800,600);
}
...
saveIcd11Button.addEventListener("click", function(e){
    ...
    // check if the input doesn't contain the value 
    
    var content_Code = varMyOpener.document.getElementById('<%=sReturnField %>').value;
    if(!content_Code.includes(codeICD11)){
    	varMyOpener.document.getElementById('<%=sReturnField %>').value += codeICD11 + " | ";
    	varMyOpener.document.getElementById('<%=sReturnField %>' + '_LABEL').value +=   label +" | ";
    	
    	
    	var contentLine = "<img src='<%=sCONTEXTPATH%>/_img/icons/icon_delete.png' onclick='document.getElementById(\"ICD11<%=sCode%>\").innerHTML=\"\";'/>";
    	//var   service = document.getElementById("serviceUid").value;
    	   contentLine +=  "<span>";
          contentLine += " <i><b>ICD11</b></i> &nbsp;&nbsp;";
          contentLine += " <b>"+codeICD11+"</b>  &nbsp;&nbsp; <b>"+label+"</b>&nbsp; &nbsp; &nbsp; &nbsp; "
          
          // var to be saved
          contentLine += "<input type='hidden' name='ICD11Code"+codeICD11+"' value='-' />";
          contentLine += "<input type='hidden' name='UserICD11Code"+codeICD11+"' value='<%=authorid%>'/>";
          //contentLine += "<input type='hidden' name='ICD10Code"+sicd10Code+"' value='-' />";
          
          
    	varMyOpener.document.getElementById('icd11Code').innerHTML+= contentLine;
    	window.close();
    	
    }else{
    	alert("La valeur existe déjà");
    }
});
