...
<%
	...
	if(transactionId.length()>0 && serverId.length()>0){
		...
		else if(transaction!=null && destinationid<0){
			//Try to get destination ID from transaction
			try{
				destinationid=Integer.parseInt(transaction.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_DESTINATIONID"));
				destinationname=GHBNetwork.getServerNameById(destinationid);
			}
			catch(Exception e){}
		}
