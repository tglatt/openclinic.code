...
<!-- 
 ...
 -->
<%
	String accessright="ne.dressing";
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
        ...
        %>
        <tr>
			<td class='admin'><%=getTran(request,"web","receiptnumber",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultTextInput(session, tran, "ITEM_TYPE_DRESSING_RECEIPT", 30) %></td>
			<td class='admin'><%=getTran(request,"web","surgerydiagnosis",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_DRESSING_DIAGNOSIS", "niger.dressing.diagnosis", sWebLanguage, "") %></td>
        </tr>
        <tr>
			<td class='admin'><%=getTran(request,"web","returnvisit",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultDateInput(session, tran, "ITEM_TYPE_DRESSING_RETURNVISIT", sWebLanguage, sCONTEXTPATH) %></td>
			<td class='admin'><%=getTran(request,"dressing","comment",sWebLanguage) %></td>
			<td class='admin'><%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_DRESSING_COMMENT", 40, 2) %></td>
        </tr>
    </table>
