...
<!-- 
 ...
 -->
<%
	String accessright="imported.doc";
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
		...
		<%
			String title = tran.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_TITLE");
			String[] lines = tran.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CONTENT").split("~");
		%>
		<tr class='admin'>
		...
		<%
			for(int n=0;n<lines.length; n++){
				...
				for(int i=0;i<cols.length;i+=2){
					if(cols[i].length()>0){
						if(cols[i].equalsIgnoreCase("title")){
							out.println("<td class='adminblack' colspan='8'>"+cols[i+1].toUpperCase()+"</td>");
						}
						else{
							if(cols.length>i+1 && cols[i+1].length()>0){
								out.println("<td class='admin'>"+cols[i]+"</td>");
								out.println("<td class='admin2'>"+cols[i+1]+"</td>");
							}
							else{
