		    ...
		    if(activeEncounterUid.length() > 0){
		        ...
		        for(int n=0; n<d.size(); n++){
			        ...
			        if(diag.getCodeType().equalsIgnoreCase("icpc")){
		    		    ...
		    		    %><tr class="list<%=sClass%>"><td width="1%">ICD11</td><td width="1%"><b><%=diag.getCode()%></b></td><td><b><%=MedwanQuery.getInstance().getCodeTran("icd11code"+diag.getCode(),sWebLanguage)%> <%=diag.getLateralisation()%>&nbsp;<i>G:<%=sGravity%>/C:<%=sCertainty%><%=POA.length()>0?" POA":""%><%=NC.length()>0?" N":""%><%=flags.length()==0?"":" ("+flags+flagsuffix+")" %></i></b></td></tr><%
		            }
		        }
		    }
	    %>
