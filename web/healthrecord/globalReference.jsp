...
<%=checkPermission(out,"occup.reference","select",activeUser)%>
<%
    // supported languages
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <%=ScreenHelper.alignButtonsStart()%>
        <%=getButtonsHtml(request,activeUser,activePatient,"occup.reference",sWebLanguage)%>     
...
<script>
  ...
  function doSave(printDocument,tranSubType){
    ...
    if(printDocument){
      ...
      else printLang = transactionForm.PrintLanguage.value;
