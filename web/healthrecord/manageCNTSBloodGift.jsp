...
<%
	...
	if(tran.getTransactionId()<0){
		...
		if(activePatient.getAge()<SH.ci("bloodDonorMinimumAge",17)){
			out.println("<script>bRejected=true;alert('"+ScreenHelper.getTranNoLink("cnts","donoristooyoung",sWebLanguage)+"');window.history.go(-1);</script>");
			out.flush();
		}
		if(activePatient.getAge()>SH.ci("bloodDonorMaximumAge",65)){
			out.println("<script>bRejected=true;alert('"+ScreenHelper.getTranNoLink("cnts","donoristooold",sWebLanguage)+"');window.history.go(-1);</script>");
			out.flush();
		}
	        	...
	        	<table width='100%'>
			            ...
			            <td class="admin2">
			            	<table width='100%' cellspacing='0' cellpadding='0'>
			            		<tr>
			                		...
			                		<td class='admin2'>
			                			<%=SH.writeDefaultHiddenInput(tran, "ITEM_TYPE_CNTSBLOODGIFT_OLDGIFTS") %>
		        	...
		        	<tr>
			            ...
			            <td class="admin2">
			                ...
			                <img src="<c:url value="/_img/icons/icon_delete.png"/>" class="link" alt="<%=getTranNoLink("Web","clear",sWebLanguage)%>" onclick="document.getElementById('searchServiceUid').value='';document.getElementById('searchServiceName').value='';"><br/>
			                ...
			                <%=SH.writeDefaultSelect(request, tran, "!ITEM_TYPE_CNTSBLOODGIFT_ENVIRONMENT", "cnts.environment", sWebLanguage, "") %>
			            </td>
...
function searchEncounter(){
  ...
  function submitForm(){
	  ...
	  if(document.getElementById("oldblooddonations").innerHTML=='?'){
		  alert('<%=getTranNoLink("web","oldblooddonationsismandatory",sWebLanguage)%>');
	  }
  ...
  function countBloodDonations(setVal){
	  ...
	  new Ajax.Request(url,{
			...
			onSuccess: function(resp){
				...
				if(label.old != '?'){
					document.getElementById("ITEM_TYPE_CNTSBLOODGIFT_OLDGIFTS").value = label.old;
				}
		  }
	  });
  }
  ...
  function checkCriteria(bReset){
	  ...
	  <% if(activePatient.gender.equalsIgnoreCase("m")){%>
	  	...
	  	document.getElementById('ITEM_TYPE_CNTSBLOODGIFT_REJECTIONCRITERIA.13').checked=false;
	  	document.getElementById('ITEM_TYPE_CNTSBLOODGIFT_REJECTIONCRITERIA.13').disabled=true;
	  	document.getElementById('ITEM_TYPE_CNTSBLOODGIFT_REJECTIONCRITERIA.13').parentElement.style.color='white';
	  	document.getElementById('ITEM_TYPE_CNTSBLOODGIFT_REJECTIONCRITERIA.15').checked=false;
	  	document.getElementById('ITEM_TYPE_CNTSBLOODGIFT_REJECTIONCRITERIA.15').disabled=true;
	  	document.getElementById('ITEM_TYPE_CNTSBLOODGIFT_REJECTIONCRITERIA.15').parentElement.style.color='white';
	  <% }%>
	  //Last donation check
