...
<!-- 
 ...
 -->
<%
	String accessright="mshp.transfusion";
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
        ...
        <!-- 
        	...
        	******************************************
        -->
        <tr class='admin'><td colspan='4'><%=getTran(request,"web","receiver",sWebLanguage) %></td></tr>
        <tr>
		...
		<tr>
			...
			<td class='admin2'><%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_WEIGHT", 5, 0, 300, sWebLanguage) %></td>
			<td class='admin'><%=getTran(request,"web","diagnosis",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultTextArea(session, tran, "ITEM_TYPE_DIAGNOSIS", 40, 1) %></td>
		</tr>
        <tr class='admin'><td colspan='4'><%=getTran(request,"web","transfusion",sWebLanguage) %></td></tr>
		<tr>
			...
			<td class='admin'><%=getTran(request,"web","responsabletransfusion",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultTextInput(session, tran, "ITEM_TYPE_RESPONSABLE", 40) %></td>
		</tr>
		<tr>
			...
			<td class='admin'><%=getTran(request,"web","receivedvolume",sWebLanguage) %></td>
			<td class='admin2'>
				<%=SH.writeDefaultNumericInput(session, tran, "ITEM_TYPE_BLOODVOLUME", 10, 0, 2000, sWebLanguage) %>ml | &nbsp;&nbsp;
				<%=getTran(request,"web","pockets",sWebLanguage) %>: <%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_BLOODPOCKETS", sWebLanguage, "", 1, 20) %>
			</td>
		</tr>
		<tr>
			<td class='admin'><%=getTran(request,"web","bloodproduct",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultSelect(request, tran, "ITEM_TYPE_BLOODPRODUCT", "gn.bloodproducts", sWebLanguage, "") %></td>
			<td class='admin'><%=getTran(request,"web","compatibilitytest",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultTextInput(session, tran, "ITEM_TYPE_COMPATIBILITYTEST", 20) %></td>
		</tr>
		<tr>
			<td class='admin'><%=getTran(request,"web","begintime",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultTimeInput(session, tran, "ITEM_TYPE_BEGIN", sWebLanguage, sCONTEXTPATH) %></td>
			<td class='admin'><%=getTran(request,"web","endtime",sWebLanguage) %></td>
			<td class='admin2'><%=SH.writeDefaultTimeInput(session, tran, "ITEM_TYPE_END", sWebLanguage, sCONTEXTPATH) %></td>
		</tr>
		<tr>
