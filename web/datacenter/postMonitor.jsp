...
<%
	...
	Long t = new Double(Math.random()*5000).longValue();
	Thread.sleep(t);
	...
	if(messageType.equalsIgnoreCase("systeminfo")){
		...
		try{
			...
			if(rs.next()){
				...
				sSql="select * from dc_monitorservers where dc_monitorserver_name=?";
				ps = conn.prepareStatement(sSql);
				ps.setString(1,centerName);
				rs = ps.executeQuery();
				if(!rs.next()){
					SH.syslog("Inserting new server "+centerName);
					rs.close();
					ps.close();
			...
			if(doinsert){
				sSql = "select * from dc_monitorvalues where dc_monitorvalue_serverid=? and dc_monitorvalue_date>?";
				ps=conn.prepareStatement(sSql);
				ps.setInt(1,serverid);
				ps.setTimestamp(2,new java.sql.Timestamp(date.getTime()-SH.getTimeHour()));
				rs = ps.executeQuery();
				doinsert = !rs.next();
				rs.close();
				ps.close();
			...
			if(doinsert){
				sSql = "select * from dc_monitorvalues,dc_monitorservers where dc_monitorvalue_serverid=dc_monitorserver_serverid and dc_monitorserver_name=? and dc_monitorvalue_date>?";
				ps=conn.prepareStatement(sSql);
				ps.setString(1,centerName);
				ps.setTimestamp(2,new java.sql.Timestamp(date.getTime()-SH.getTimeHour()));
				rs = ps.executeQuery();
				...
				ps.close();
				if(!doinsert){
					Debug.println("Duplicate data from "+centerName);
				}
			}
			else{
				Debug.println("Duplicate data from "+serverid);
			}
			if(doinsert){
