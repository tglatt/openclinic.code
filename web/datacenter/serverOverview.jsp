...
<div class="wrap-smallcontainer leftcontainer">
    <div id="diagnostics" class="container diagnostics">
        <h3 id="diagnostics_title">
            <span class="icon diagnostics"><%=getTranNoLink("datacenter","covid19",sWebLanguage)%></span>
        </h3>
        <div class="subcontent">
        <%
        	java.util.Date end= DatacenterHelper.getLastDate(Integer.parseInt(serverid),"covid19");
        	if(end!=null){
	        	long day = 24*3600*1000;
	        	java.util.Date begin = new java.util.Date(end.getTime()-7*day);
	        	%>
	            <a class="togglecontent" href="javascript:void(0)" onclick="loadCovid19()"><span class="icon down">&nbsp;</span></a>
				<table width='90%'>
					<tr style='text-align: center'>
						<td/>
						<td>#<%=getTran(request,"web","triage",sWebLanguage) %></td>
						<td>#<%=getTran(request,"web","confirmed",sWebLanguage) %></td>
						<td>#<%=getTran(request,"web","admitted",sWebLanguage) %></td>
						<td>#<%=getTran(request,"web","discharged",sWebLanguage) %></td>
						<td>#<%=getTran(request,"web","mortality",sWebLanguage) %></td>
					</tr>
					<tr style='text-align: center'>
						<td><b><%=SH.formatDate(end) %></b></td>
						<td><b><a href='javascript:simpleValueGraph("<%=serverid%>","covid19.1");'><%=DatacenterHelper.getLastSimpleValue(Integer.parseInt(serverid), "covid19.1").replaceAll("\\?","0") %></a></b></td>
						<td><b><a href='javascript:simpleValueGraph("<%=serverid%>","covid19.5");'><%=DatacenterHelper.getLastSimpleValue(Integer.parseInt(serverid), "covid19.5").replaceAll("\\?","0") %></a></b></td>
						<td><b><a href='javascript:simpleValueGraph("<%=serverid%>","covid19.2a");'><%=DatacenterHelper.getLastSimpleValue(Integer.parseInt(serverid), "covid19.2a").replaceAll("\\?","0") %></a></b></td>
						<td><b><a href='javascript:simpleValueGraph("<%=serverid%>","covid19.2b");'><%=DatacenterHelper.getLastSimpleValue(Integer.parseInt(serverid), "covid19.2b").replaceAll("\\?","0") %></a></b></td>
						<td><b><a href='javascript:simpleValueGraph("<%=serverid%>","covid19.3");'><%=DatacenterHelper.getLastSimpleValue(Integer.parseInt(serverid), "covid19.3").replaceAll("\\?","0") %></a></b></td>
					</tr>
					<tr style='text-align: center'>
						<td><b><%=getTran(request,"web","week",sWebLanguage) %></b></td>
						<td><b><a href='javascript:loadCovid19("covid19.1");'><%=new Double(DatacenterHelper.getTotalSimpleValue(Integer.parseInt(serverid), "covid19.1",begin,end).replaceAll("\\?","0")).intValue() %></a></b></td>
						<td><b><a href='javascript:loadCovid19("covid19.5");'><%=new Double(DatacenterHelper.getTotalSimpleValue(Integer.parseInt(serverid), "covid19.5",begin,end).replaceAll("\\?","0")).intValue() %></a></b></td>
						<td><b><a href='javascript:loadCovid19("covid19.2a");'><%=new Double(DatacenterHelper.getTotalSimpleValue(Integer.parseInt(serverid), "covid19.2a",begin,end).replaceAll("\\?","0")).intValue() %></a></b></td>
						<td><b><a href='javascript:loadCovid19("covid19.2b");'><%=new Double(DatacenterHelper.getTotalSimpleValue(Integer.parseInt(serverid), "covid19.2b",begin,end).replaceAll("\\?","0")).intValue() %></a></b></td>
						<td><b><a href='javascript:loadCovid19("covid19.3");'><%=new Double(DatacenterHelper.getTotalSimpleValue(Integer.parseInt(serverid), "covid19.3",begin,end).replaceAll("\\?","0")).intValue() %></a></b></td>
					</tr>
				</table>
				<%
        	}
...
<script>
<%-- OPEN POPUP --%>
	function loadCovid19(parameterid){
		<%
			String serverzone="";
			Connection conn = SH.getOpenclinicConnection();
			PreparedStatement ps = conn.prepareStatement("select dc_server_zone from dc_servers where dc_server_serverid=?");
			ps.setString(1,serverid);
			ResultSet rs = ps.executeQuery();
			if(rs.next()){
				serverzone=rs.getString("dc_server_zone");
			}
			rs.close();
			ps.close();
			conn.close();
		%>
		var zoneid = window.prompt('<%=getTranNoLink("web","zone",sWebLanguage)%>','<%=serverzone%>');
		simpleZoneValueGraph(zoneid,parameterid);
	}
    ...
    function simpleZoneValueGraph(zoneid,parameterid){
        openPopupWindow("/datacenter/simpleValueGraph.jsp?zoneid="+zoneid+"&parameterid="+parameterid+"&ts=<%=getTs()%>");
    }
