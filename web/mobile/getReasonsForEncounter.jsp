	...
	if(encounter!=null){
		%>
		    ...
		    <table class="list" padding="0" cellspacing="0" width="<%=sTABLE_WIDTH%>">
				<tr class="admin"><td colspan="3"><%=getTran(request,"web","active_encounter",activeUser)%></td></tr>
				    <tr><td><%=encounter.getService().getLabel(activeUser.person.language)%></td></tr>
				    <tr><td><%=getTran(request,"encountertype",encounter.getType(),activeUser)%>, <%=stdDateFormat.format(encounter.getBegin())%></td></tr>
			</table>		
			<div style="padding-top:3px"></div>
			
            <%-- 1 - reasons for the active encounter --%>
		    <table class="list" padding="0" cellspacing="0" width="<%=sTABLE_WIDTH%>">
				<tr class="admin"><td><%=getTran(request,"openclinic.chuk","rfe",activeUser)%></td></tr>
		    ...
		    <table class="list" padding="0" cellspacing="0" width="<%=sTABLE_WIDTH%>">
				<tr class="admin"><td><%=getTran(request,"openclinic.chuk","dfe",activeUser)%></td></tr>
