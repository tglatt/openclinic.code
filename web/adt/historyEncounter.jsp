...
<%=checkPermission(out,"adt.encounterhistory","select",activeUser)%>
<%=sJSSORTTABLE%>
...
<%
    ...
    String sClass = "", sInactive = "", sInactiveSelect = "", sUpdateUser = "", diags="";
    ...
    while(encIter.hasNext()){
        ...
        for(int n=0; n<tranHist.size(); n++){
        	...
        	diags ="";
            if(activeUser.getAccessRight("diagnoses.patientDiagnoses.select")){
            	Vector diagnoses = Diagnosis.selectDiagnoses("", "", tmpEnc.getUid(), "", "", "", "", "", "", "", "", "", "");
            	for(int q=0; q<diagnoses.size();q++){
            		Diagnosis diagnosis = (Diagnosis)diagnoses.elementAt(q);
            		if(diags.length()>0){
            			diags+=", ";
            		}
            		diags+=" ["+diagnosis.getCode()+" - "+diagnosis.getCodeLabel(diagnosis.getCode(), diagnosis.getCodeType(), sWebLanguage)+"]";
            	}
            }
        ...
        sbResults.append("<tr id='"+(bFinished?"finished":"")+"' class='list"+sInactive+sClass+"'")
                 .append("onmouseover=\"this.style.cursor='pointer';\" onmouseout=\"this.style.cursor='default';\">")
                  ...
                  .append("<td onclick=\"doSelect('"+tmpEnc.getUid()+"');\">"+diags+"</td>")
        ...
        %>
			<table width="100%" cellspacing="0" cellpadding="0" class="sortable" id="searchresults">
			    ...
			    <tr class="gray">
			        ...
			        <td><%=getTran(request,"web","diagnosis",sWebLanguage)%>&nbsp;</td>
