...
<%
if(SH.cs("loginredirect","").length()>0 && request.getParameter("stay")==null){
	response.sendRedirect(SH.cs("loginredirect",""));
}
if(!request.isSecure() && MedwanQuery.getInstance(true).getConfigString("forceInsecureRedirect","").length()>0){
	response.sendRedirect(MedwanQuery.getInstance(true).getConfigString("forceInsecureRedirect",""));
}
if(MedwanQuery.getInstance(true).getConfigString("edition","openclinic").equalsIgnoreCase("mpi")){
	response.sendRedirect(sCONTEXTPATH+"/mpiLogin.jsp");
}
...
<head>
    ...
    try{
		...
		//First check existance of templateSource server
		URL url = new URL(MedwanQuery.getInstance().getConfigString("templateSource"));
		if(!SH.isServerListening(url.getHost(), url.getPort())){
			MedwanQuery.getInstance().setConfigString("templateSource", (request.isSecure()?"https":"http")+"://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/_common/xml/");
		}
    ...
    //Remove impossible birthdates from patients and impossible updatetimes from transactions
    ...
    try{ 
		PreparedStatement ps = conn.prepareStatement("select * from transactions where updatetime='0000-00-00 00:00:00'");
		ps.execute();
		ps.close();
    }
    catch(Exception e){
